...
### <a name="add-hello-relay-nuget-package"></a>Hello 릴레이 NuGet 패키지 추가

1. Hello 새로 만든 프로젝트를 마우스 오른쪽 단추로 누른 **NuGet 패키지 관리**합니다.
2. Hello 클릭 **찾아보기** 누른 "Microsoft.Azure.Relay" 및 선택 hello에 대 한 검색 **Microsoft Azure 릴레이** 항목입니다. 클릭 **설치** toocomplete hello 설치의 경우 다음이 대화 상자를 닫습니다.

### <a name="write-some-code-toosend-messages"></a>Toosend 메시지 일부 코드를 작성 합니다.

1. Hello 기존 항목 바꾸기 `using` hello 다음과 같이 hello Program.cs 파일의 hello 위쪽 문을 `using` 문:
    ...
    ```
2. 상수 toohello 추가 `Program` hello 하이브리드 연결 세부 정보에 대 한 클래스입니다. Hello 값 hello 하이브리드 연결을 만들 때 대괄호의 hello 자리 표시자를 바꿉니다. 있는지 toouse hello 정규화 된 네임 스페이스 이름 이어야 합니다.
    ...
    ```
3. 다음 메서드 toohello hello 추가 `Program` 클래스:
    ...
    ```
4. 다음 줄의 코드 toohello hello 추가 `Main` hello에 대 한 메서드 `Program` 클래스입니다.
