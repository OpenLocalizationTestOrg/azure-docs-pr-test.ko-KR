...
2. 상수 toohello 추가 `Program` hello 하이브리드 연결 세부 정보에 대 한 클래스입니다. Hello 값 hello 하이브리드 연결을 만들 때 대괄호의 hello 자리 표시자를 바꿉니다. 있는지 toouse hello 정규화 된 네임 스페이스 이름 이어야 합니다.
    ...
    ```
3. 다음 메서드를 호출 하는 hello 추가 `ProcessMessagesOnConnection` toohello `Program` 클래스:
    ...
    ```
4. 라는 다른 메서드를 추가 `RunAsync` toohello `Program` 클래스 다음과 같습니다.
    ...
    ```
5. 다음 줄의 코드 toohello hello 추가 `Main` hello에 대 한 메서드 `Program` 클래스:
