...
ExpressRoute를 사용 하 여 tooconnect tooMicrosoft 클라우드 서비스, tooset 필요한 하 라우팅을 관리 합니다. 일부 연결 공급자는 라우팅을 관리 서비스로 설치하고 관리해 줍니다. 이 서비스를 제공 하는 경우 연결 공급자 toosee 확인 하십시오. 그렇지 않으면 요구 사항을 준수 toohello를 따라야 합니다.
...
* /29 서브넷을 사용한 경우 두 개의 /30 서브넷으로 분할됩니다. 
  * 먼저 hello/30 서브넷 hello 기본 링크에 사용 되 고 hello 두 번째/30 서브넷 hello 보조 링크에 사용 됩니다.
  * 각 hello/30 서브넷에 대 한 라우터에서 hello hello/30 서브넷의 첫 번째 IP 주소를 사용 해야 합니다. Microsoft는 hello BGP 세션을 hello/30 서브넷 tooset의 두 번째 IP 주소를 사용 합니다.
...
Toouse a.b.c.d/29 tooset hello 피어 링을 선택 하면 두 개의/30 서브넷으로 분할 됩니다. Hello 아래 예제에서는 hello a.b.c.d/29 서브넷 사용 되는 방법을 살펴봅니다. 
...
IP 주소와 마찬가지로 번호는 확인 된 등록 된 tooyou hello 중 하나에서 다음 레지스트리:
