...
## <a name="download-and-install-azcopy"></a><span data-ttu-id="9ca53-112">AzCopy 다운로드 및 설치</span><span class="sxs-lookup"><span data-stu-id="9ca53-112">Download and install AzCopy</span></span>
...
### <a name="download-blobs-with-specified-prefix"></a><span data-ttu-id="9ca53-132">지정된 접두사가 있는 Blob 다운로드</span><span class="sxs-lookup"><span data-stu-id="9ca53-132">Download blobs with specified prefix</span></span>
...
### <a name="download-all-files"></a><span data-ttu-id="9ca53-183">모든 파일 다운로드</span><span class="sxs-lookup"><span data-stu-id="9ca53-183">Download all files</span></span>
...
## <a name="table-import"></a><span data-ttu-id="9ca53-235">테이블: 가져오기</span><span class="sxs-lookup"><span data-stu-id="9ca53-235">Table: Import</span></span>
...
* <span data-ttu-id="9ca53-303">AzCopy에 대한 자세한 명령줄 도움말: `AzCopy /?`</span><span class="sxs-lookup"><span data-stu-id="9ca53-303">For detailed command-line help for AzCopy: `AzCopy /?`</span></span>
...
<span data-ttu-id="9ca53-351">**적용 대상:** Blob, 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-351">**Applicable to:** Blobs, Files</span></span>
...
<span data-ttu-id="9ca53-357">**적용 대상:** Blob</span><span class="sxs-lookup"><span data-stu-id="9ca53-357">**Applicable to:** Blobs</span></span>

### <a name="checkmd5"></a><span data-ttu-id="9ca53-358">/CheckMD5</span><span class="sxs-lookup"><span data-stu-id="9ca53-358">/CheckMD5</span></span>
<span data-ttu-id="9ca53-359">다운로드 한 데이터에 대 한 MD5 해시를 계산 하 고 hello blob에 저장 된 MD5 해시 hello 또는 파일의 content-md5 속성 hello 계산 된 해시와 일치 여부를 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-359">Calculates an MD5 hash for downloaded data and verifies that hello MD5 hash stored in hello blob or file's Content-MD5 property matches hello calculated hash.</span></span> <span data-ttu-id="9ca53-360">hello MD5 검사 꺼져 기본적으로 데이터를 다운로드할 때이 옵션 tooperform hello MD5 검사를 지정 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-360">hello MD5 check is turned off by default, so you must specify this option tooperform hello MD5 check when downloading data.</span></span>

<span data-ttu-id="9ca53-361">Azure 저장소 않습니다 보장 하는 해당 hello hello blob에 대해 저장 된 MD5 해시 또는 파일이 최신 상태입니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-361">Note that Azure Storage doesn't guarantee that hello MD5 hash stored for hello blob or file is up-to-date.</span></span> <span data-ttu-id="9ca53-362">클라이언트의 책임 tooupdate hello MD5 hello blob 또는 파일 수정 될 때마다</span><span class="sxs-lookup"><span data-stu-id="9ca53-362">It is client's responsibility tooupdate hello MD5 whenever hello blob or file is modified.</span></span>

<span data-ttu-id="9ca53-363">항상 AzCopy는 Azure blob 또는 파일에 대 한 content-md5 속성 hello toohello 서비스 업로드 한 후 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-363">AzCopy always sets hello Content-MD5 property for an Azure blob or file after uploading it toohello service.</span></span>  

<span data-ttu-id="9ca53-364">**적용 대상:** Blob, 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-364">**Applicable to:** Blobs, Files</span></span>

### <a name="snapshot"></a><span data-ttu-id="9ca53-365">/Snapshot</span><span class="sxs-lookup"><span data-stu-id="9ca53-365">/Snapshot</span></span>
<span data-ttu-id="9ca53-366">표시 여부를 tootransfer 스냅숏을 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-366">Indicates whether tootransfer snapshots.</span></span> <span data-ttu-id="9ca53-367">이 옵션은 hello 원본 blob 인 경우에 유효 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-367">This option is only valid when hello source is a blob.</span></span>

<span data-ttu-id="9ca53-368">hello 전송된 blob 스냅숏 이름이이 형식의:.extension blob 이름 (스냅숏 시간)</span><span class="sxs-lookup"><span data-stu-id="9ca53-368">hello transferred blob snapshots are renamed in this format: blob-name (snapshot-time).extension</span></span>

<span data-ttu-id="9ca53-369">기본적으로 스냅숏은 복사되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-369">By default, snapshots are not copied.</span></span>

<span data-ttu-id="9ca53-370">**적용 대상:** Blob</span><span class="sxs-lookup"><span data-stu-id="9ca53-370">**Applicable to:** Blobs</span></span>

### <a name="vverbose-log-file"></a><span data-ttu-id="9ca53-371">/V:[verbose-log-file]</span><span class="sxs-lookup"><span data-stu-id="9ca53-371">/V:[verbose-log-file]</span></span>
<span data-ttu-id="9ca53-372">세부 정보 표시 상태 메시지를 로그 파일로 출력합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-372">Outputs verbose status messages into a log file.</span></span>

<span data-ttu-id="9ca53-373">기본적으로 hello 자세한 로그 파일에서 AzCopyVerbose.log 이름은 `%LocalAppData%\Microsoft\Azure\AzCopy`합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-373">By default, hello verbose log file is named AzCopyVerbose.log in `%LocalAppData%\Microsoft\Azure\AzCopy`.</span></span> <span data-ttu-id="9ca53-374">이 옵션에 대 한 기존 파일 위치를 지정 하면 hello 자세한 로그 파일이 추가 된 toothat입니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-374">If you specify an existing file location for this option, hello verbose log is appended toothat file.</span></span>  

<span data-ttu-id="9ca53-375">**적용 대상:** Blob, 파일, 테이블</span><span class="sxs-lookup"><span data-stu-id="9ca53-375">**Applicable to:** Blobs, Files, Tables</span></span>

### <a name="zjournal-file-folder"></a><span data-ttu-id="9ca53-376">/Z:[journal-file-folder]</span><span class="sxs-lookup"><span data-stu-id="9ca53-376">/Z:[journal-file-folder]</span></span>
<span data-ttu-id="9ca53-377">작업을 다시 시작하기 위한 저널 파일 폴더를 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-377">Specifies a journal file folder for resuming an operation.</span></span>

<span data-ttu-id="9ca53-378">AzCopy는 작업이 중단된 경우 항상 다시 시작을 지원합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-378">AzCopy always supports resuming if an operation has been interrupted.</span></span>

<span data-ttu-id="9ca53-379">이 옵션을 지정 하지 않거나 없이 폴더 경로 지정 된, AzCopy hello 저널 파일은 % LocalAppData%\Microsoft\Azure\AzCopy hello 기본 위치에 만듭니다 다음.</span><span class="sxs-lookup"><span data-stu-id="9ca53-379">If this option is not specified, or it is specified without a folder path, then AzCopy creates hello journal file in hello default location, which is %LocalAppData%\Microsoft\Azure\AzCopy.</span></span>

<span data-ttu-id="9ca53-380">저널 파일 hello 기본 폴더에 있는지 여부 또는이 옵션을 통해 지정 하는 폴더에 존재 하는지 여부 명령 tooAzCopy 실행 될 때마다 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-380">Each time you issue a command tooAzCopy, it checks whether a journal file exists in hello default folder, or whether it exists in a folder that you specified via this option.</span></span> <span data-ttu-id="9ca53-381">Hello 저널 파일이 어느 위치에 없는 경우 AzCopy 새으로 hello 작업을 처리 하 고 새 저널 파일을 생성 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-381">If hello journal file does not exist in either place, AzCopy treats hello operation as new and generates a new journal file.</span></span>

<span data-ttu-id="9ca53-382">Hello 저널 파일이 AzCopy hello 명령줄 입력을 hello 명령줄 hello 저널 파일에서 일치 하는지 여부를 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-382">If hello journal file does exist, AzCopy checks whether hello command line that you input matches hello command line in hello journal file.</span></span> <span data-ttu-id="9ca53-383">Hello 두 명령줄 일치 AzCopy hello 불완전 한 작업을 다시 시작 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-383">If hello two command lines match, AzCopy resumes hello incomplete operation.</span></span> <span data-ttu-id="9ca53-384">일치 하지 않으면 증명된 tooeither 덮어쓰기 hello 저널 파일 toostart 새 작업 또는 toocancel hello 현재 작업이 됩니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-384">If they do not match, you are prompted tooeither overwrite hello journal file toostart a new operation, or toocancel hello current operation.</span></span>

<span data-ttu-id="9ca53-385">hello 작업이 성공적으로 완료 되 면 hello 저널 파일은 삭제 됩니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-385">hello journal file is deleted upon successful completion of hello operation.</span></span>

<span data-ttu-id="9ca53-386">이전 버전의 AzCopy에서 만들어진 저널 파일에서 작업을 다시 시작하는 것은 지원되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-386">Note that resuming an operation from a journal file created by a previous version of AzCopy is not supported.</span></span>

<span data-ttu-id="9ca53-387">**적용 대상:** Blob, 파일, 테이블</span><span class="sxs-lookup"><span data-stu-id="9ca53-387">**Applicable to:** Blobs, Files, Tables</span></span>

### <a name="parameter-file"></a><span data-ttu-id="9ca53-388">/@:"parameter-file"</span><span class="sxs-lookup"><span data-stu-id="9ca53-388">/@:"parameter-file"</span></span>
<span data-ttu-id="9ca53-389">매개 변수를 포함하는 파일을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-389">Specifies a file that contains parameters.</span></span> <span data-ttu-id="9ca53-390">AzCopy 프로세스 경우 hello 명령줄에 지정 된 것 처럼 hello 파일의 매개 변수를 hello 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-390">AzCopy processes hello parameters in hello file just as if they had been specified on hello command line.</span></span>

<span data-ttu-id="9ca53-391">지시 파일에서 단일 줄에 여러 매개 변수를 지정하거나 한 줄에 매개 변수를 하나씩 지정할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-391">In a response file, you can either specify multiple parameters on a single line, or specify each parameter on its own line.</span></span> <span data-ttu-id="9ca53-392">하나의 매개 변수가 여러 줄에 걸쳐 있을 수 없습니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-392">Note that an individual parameter cannot span multiple lines.</span></span>

<span data-ttu-id="9ca53-393">지시 파일 hello # 기호로 시작 하는 주석 줄을 포함할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-393">Response files can include comments lines that begin with hello # symbol.</span></span>

<span data-ttu-id="9ca53-394">여러 지시 파일을 지정할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-394">You can specify multiple response files.</span></span> <span data-ttu-id="9ca53-395">그렇지만 AzCopy는 중첩된지시 파일을 지원하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-395">However, note that AzCopy does not support nested response files.</span></span>

<span data-ttu-id="9ca53-396">**적용 대상:** Blob, 파일, 테이블</span><span class="sxs-lookup"><span data-stu-id="9ca53-396">**Applicable to:** Blobs, Files, Tables</span></span>

### <a name="y"></a><span data-ttu-id="9ca53-397">/Y</span><span class="sxs-lookup"><span data-stu-id="9ca53-397">/Y</span></span>
<span data-ttu-id="9ca53-398">모든 AzCopy 확인 프롬프트를 표시하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-398">Suppresses all AzCopy confirmation prompts.</span></span>

<span data-ttu-id="9ca53-399">**적용 대상:** Blob, 파일, 테이블</span><span class="sxs-lookup"><span data-stu-id="9ca53-399">**Applicable to:** Blobs, Files, Tables</span></span>

### <a name="l"></a><span data-ttu-id="9ca53-400">/L</span><span class="sxs-lookup"><span data-stu-id="9ca53-400">/L</span></span>
<span data-ttu-id="9ca53-401">열거 작업만 지정하고 데이터는 복사되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-401">Specifies a listing operation only; no data is copied.</span></span>

<span data-ttu-id="9ca53-402">AzCopy 해석의 hello를 사용 하 여이 옵션을 수동으로 구성 하지 않으면 hello 명령줄을 실행 중인에 대 한 시뮬레이션 옵션 /L 및 개체 수를 지정할 수 있습니다 복사 되는 개수 /V hello toocheck 시간이 hello 자세한 로그에서 어떤 개체를 복사 하는 옵션입니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-402">AzCopy interprets hello using of this option as a simulation for running hello command line without this option /L and counts how many objects are copied, you can specify option /V at hello same time toocheck which objects are copied in hello verbose log.</span></span>

<span data-ttu-id="9ca53-403">이 옵션의 동작은 hello hello 원본 데이터의 hello 위치와 hello 재귀 모드 옵션 /S 및 파일 패턴 옵션 /Pattern의 hello 존재 하 여도 결정 됩니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-403">hello behavior of this option is also determined by hello location of hello source data and hello presence of hello recursive mode option /S and file pattern option /Pattern.</span></span>

<span data-ttu-id="9ca53-404">AzCopy는 이 옵션을 사용하는 경우 이 원본 위치에 대한 목록 및 읽기 권한이 필요합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-404">AzCopy requires LIST and READ permission of this source location when using this option.</span></span>

<span data-ttu-id="9ca53-405">**적용 대상:** Blob, 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-405">**Applicable to:** Blobs, Files</span></span>

### <a name="mt"></a><span data-ttu-id="9ca53-406">/MT</span><span class="sxs-lookup"><span data-stu-id="9ca53-406">/MT</span></span>
<span data-ttu-id="9ca53-407">Toobe hello 원본 blob 또는 파일의 동일 hello hello 다운로드 된 파일의 마지막 수정 시간을 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-407">Sets hello downloaded file's last-modified time toobe hello same as hello source blob or file's.</span></span>

<span data-ttu-id="9ca53-408">**적용 대상:** Blob, 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-408">**Applicable to:** Blobs, Files</span></span>

### <a name="xn"></a><span data-ttu-id="9ca53-409">/XN</span><span class="sxs-lookup"><span data-stu-id="9ca53-409">/XN</span></span>
<span data-ttu-id="9ca53-410">최신 소스 리소스를 제외합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-410">Excludes a newer source resource.</span></span> <span data-ttu-id="9ca53-411">hello 리소스 복사 되지 않습니다 이면 hello hello 소스의 마지막 수정된 시간 hello 같거나 대상 보다 최신입니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-411">hello resource is not copied if hello last modified time of hello source is hello same or newer than destination.</span></span>

<span data-ttu-id="9ca53-412">**적용 대상:** Blob, 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-412">**Applicable to:** Blobs, Files</span></span>

### <a name="xo"></a><span data-ttu-id="9ca53-413">/XO</span><span class="sxs-lookup"><span data-stu-id="9ca53-413">/XO</span></span>
<span data-ttu-id="9ca53-414">오래된 소스 리소스를 제외합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-414">Excludes an older source resource.</span></span> <span data-ttu-id="9ca53-415">hello 리소스 복사 되지 않습니다 hello hello 소스의 마지막 수정된 시간은 hello 같거나 대상 보다 오래 된 경우.</span><span class="sxs-lookup"><span data-stu-id="9ca53-415">hello resource is not copied if hello last modified time of hello source is hello same or older than destination.</span></span>

<span data-ttu-id="9ca53-416">**적용 대상:** Blob, 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-416">**Applicable to:** Blobs, Files</span></span>

### <a name="a"></a><span data-ttu-id="9ca53-417">/A</span><span class="sxs-lookup"><span data-stu-id="9ca53-417">/A</span></span>
<span data-ttu-id="9ca53-418">Hello 보관 특성이 설정 되어 있는 파일만 업로드 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-418">Uploads only files that have hello Archive attribute set.</span></span>

<span data-ttu-id="9ca53-419">**적용 대상:** Blob, 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-419">**Applicable to:** Blobs, Files</span></span>

### <a name="iarashcnetoi"></a><span data-ttu-id="9ca53-420">/IA:[RASHCNETOI]</span><span class="sxs-lookup"><span data-stu-id="9ca53-420">/IA:[RASHCNETOI]</span></span>
<span data-ttu-id="9ca53-421">업로드만 포함 된 파일의 hello 특성 집합을 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-421">Uploads only files that have any of hello specified attributes set.</span></span>

<span data-ttu-id="9ca53-422">사용 가능한 특성에는 다음이 포함됩니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-422">Available attributes include:</span></span>

* <span data-ttu-id="9ca53-423">R = 읽기 전용 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-423">R = Read-only files</span></span>
* <span data-ttu-id="9ca53-424">A = 보관 준비가 된 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-424">A = Files ready for archiving</span></span>
* <span data-ttu-id="9ca53-425">S = 시스템 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-425">S = System files</span></span>
* <span data-ttu-id="9ca53-426">H = 숨김 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-426">H = Hidden files</span></span>
* <span data-ttu-id="9ca53-427">C = 압축 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-427">C = Compressed files</span></span>
* <span data-ttu-id="9ca53-428">N = 일반 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-428">N = Normal files</span></span>
* <span data-ttu-id="9ca53-429">E = 암호화된 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-429">E = Encrypted files</span></span>
* <span data-ttu-id="9ca53-430">T = 임시 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-430">T = Temporary files</span></span>
* <span data-ttu-id="9ca53-431">O = 오프라인 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-431">O = Offline files</span></span>
* <span data-ttu-id="9ca53-432">I = 인덱싱되지 않은 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-432">I = Non-indexed files</span></span>

<span data-ttu-id="9ca53-433">**적용 대상:** Blob, 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-433">**Applicable to:** Blobs, Files</span></span>

### <a name="xarashcnetoi"></a><span data-ttu-id="9ca53-434">/XA:[RASHCNETOI]</span><span class="sxs-lookup"><span data-stu-id="9ca53-434">/XA:[RASHCNETOI]</span></span>
<span data-ttu-id="9ca53-435">지정 된 hello 포함 된 파일 제외 특성이 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-435">Excludes files that have any of hello specified attributes set.</span></span>

<span data-ttu-id="9ca53-436">사용 가능한 특성에는 다음이 포함됩니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-436">Available attributes include:</span></span>

* <span data-ttu-id="9ca53-437">R = 읽기 전용 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-437">R = Read-only files</span></span>
* <span data-ttu-id="9ca53-438">A = 보관 준비가 된 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-438">A = Files ready for archiving</span></span>
* <span data-ttu-id="9ca53-439">S = 시스템 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-439">S = System files</span></span>
* <span data-ttu-id="9ca53-440">H = 숨김 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-440">H = Hidden files</span></span>
* <span data-ttu-id="9ca53-441">C = 압축 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-441">C = Compressed files</span></span>
* <span data-ttu-id="9ca53-442">N = 일반 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-442">N = Normal files</span></span>
* <span data-ttu-id="9ca53-443">E = 암호화된 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-443">E = Encrypted files</span></span>
* <span data-ttu-id="9ca53-444">T = 임시 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-444">T = Temporary files</span></span>
* <span data-ttu-id="9ca53-445">O = 오프라인 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-445">O = Offline files</span></span>
* <span data-ttu-id="9ca53-446">I = 인덱싱되지 않은 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-446">I = Non-indexed files</span></span>

<span data-ttu-id="9ca53-447">**적용 대상:** Blob, 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-447">**Applicable to:** Blobs, Files</span></span>

### <a name="delimiterdelimiter"></a><span data-ttu-id="9ca53-448">/Delimiter:"delimiter"</span><span class="sxs-lookup"><span data-stu-id="9ca53-448">/Delimiter:"delimiter"</span></span>
<span data-ttu-id="9ca53-449">Blob 이름에 toodelimit 가상 디렉터리를 사용 하는 hello 구분 기호 문자를 나타냅니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-449">Indicates hello delimiter character used toodelimit virtual directories in a blob name.</span></span>

<span data-ttu-id="9ca53-450">기본적으로 사용 하 여 AzCopy / hello 구분 기호 문자로 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-450">By default, AzCopy uses / as hello delimiter character.</span></span> <span data-ttu-id="9ca53-451">그렇지만 AzCopy는 아무 일반 문자(예: @, # 또는 %)를 구분 문자로 사용할 수 있게 지원합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-451">However, AzCopy supports using any common character (such as @, #, or %) as a delimiter.</span></span> <span data-ttu-id="9ca53-452">Hello 명령줄에서 이러한 특수 문자 중 하나는 tooinclude 해야 할 경우 hello 파일 이름을 큰따옴표로 묶습니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-452">If you need tooinclude one of these special characters on hello command line, enclose hello file name with double quotes.</span></span>

<span data-ttu-id="9ca53-453">이 옵션은 Blob을 다운로드하는 데만 적용됩니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-453">This option is only applicable for downloading blobs.</span></span>

<span data-ttu-id="9ca53-454">**적용 대상:** Blob</span><span class="sxs-lookup"><span data-stu-id="9ca53-454">**Applicable to:** Blobs</span></span>

### <a name="ncnumber-of-concurrent-operations"></a><span data-ttu-id="9ca53-455">/NC:"number-of-concurrent-operations"</span><span class="sxs-lookup"><span data-stu-id="9ca53-455">/NC:"number-of-concurrent-operations"</span></span>
<span data-ttu-id="9ca53-456">Hello 동시 작업 수를 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-456">Specifies hello number of concurrent operations.</span></span>

<span data-ttu-id="9ca53-457">기본적으로 AzCopy 특정 수의 동시 작업 tooincrease hello 데이터 전송 처리량을 시작합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-457">AzCopy by default starts a certain number of concurrent operations tooincrease hello data transfer throughput.</span></span> <span data-ttu-id="9ca53-458">에 불과하며 많은 수의 대역폭이 낮은 환경에서 동시 작업 수 hello 네트워크 연결에 과부하가 hello 작업이 완전히 완료를 방지 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-458">Note that large number of concurrent operations in a low-bandwidth environment may overwhelm hello network connection and prevent hello operations from fully completing.</span></span> <span data-ttu-id="9ca53-459">사용 가능한 실제 네트워크 대역폭에 따라 동시 작업을 조절하세요.</span><span class="sxs-lookup"><span data-stu-id="9ca53-459">Throttle concurrent operations based on actual available network bandwidth.</span></span>

<span data-ttu-id="9ca53-460">동시 작업에 대 한 hello 상한값은 512입니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-460">hello upper limit for concurrent operations is 512.</span></span>

<span data-ttu-id="9ca53-461">**적용 대상:** Blob, 파일, 테이블</span><span class="sxs-lookup"><span data-stu-id="9ca53-461">**Applicable to:** Blobs, Files, Tables</span></span>

### <a name="sourcetypeblob--table"></a><span data-ttu-id="9ca53-462">/SourceType:"Blob" | "Table"</span><span class="sxs-lookup"><span data-stu-id="9ca53-462">/SourceType:"Blob" | "Table"</span></span>
<span data-ttu-id="9ca53-463">해당 hello 지정 `source` 리소스는 blob hello 저장소 에뮬레이터에서 실행 되는 hello 로컬 개발 환경에서 사용할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-463">Specifies that hello `source` resource is a blob available in hello local development environment, running in hello storage emulator.</span></span>

<span data-ttu-id="9ca53-464">**적용 대상:** Blob, 테이블</span><span class="sxs-lookup"><span data-stu-id="9ca53-464">**Applicable to:** Blobs, Tables</span></span>

### <a name="desttypeblob--table"></a><span data-ttu-id="9ca53-465">/DestType:"Blob" | "Table"</span><span class="sxs-lookup"><span data-stu-id="9ca53-465">/DestType:"Blob" | "Table"</span></span>
<span data-ttu-id="9ca53-466">해당 hello 지정 `destination` 리소스는 blob hello 저장소 에뮬레이터에서 실행 되는 hello 로컬 개발 환경에서 사용할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-466">Specifies that hello `destination` resource is a blob available in hello local development environment, running in hello storage emulator.</span></span>

<span data-ttu-id="9ca53-467">**적용 대상:** Blob, 테이블</span><span class="sxs-lookup"><span data-stu-id="9ca53-467">**Applicable to:** Blobs, Tables</span></span>

### <a name="pkrskey1key2key3"></a><span data-ttu-id="9ca53-468">/PKRS:"key1#key2#key3#..."</span><span class="sxs-lookup"><span data-stu-id="9ca53-468">/PKRS:"key1#key2#key3#..."</span></span>
<span data-ttu-id="9ca53-469">분할 hello 파티션 키 범위 tooenable hello 내보내기 작업의 hello 속도 향상 시키는 동시에 테이블 데이터 내보내기.</span><span class="sxs-lookup"><span data-stu-id="9ca53-469">Splits hello partition key range tooenable exporting table data in parallel, which increases hello speed of hello export operation.</span></span>

<span data-ttu-id="9ca53-470">이 옵션을 지정 하지 않으면 AzCopy 단일 스레드 tooexport 테이블 엔터티를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-470">If this option is not specified, then AzCopy uses a single thread tooexport table entities.</span></span> <span data-ttu-id="9ca53-471">예를 들어 hello 사용자 지정 /PKRS: "aa #bb" 다음 AzCopy 3 개의 동시 작업을 시작 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-471">For example, if hello user specifies /PKRS:"aa#bb", then AzCopy starts three concurrent operations.</span></span>

<span data-ttu-id="9ca53-472">각 작업에서는 아래에 나와 있는 것처럼 3개 파티션 키 범위 중 하나를 내보냅니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-472">Each operation exports one of three partition key ranges, as shown below:</span></span>

  <span data-ttu-id="9ca53-473">[첫 번째 파티션 키, aa)</span><span class="sxs-lookup"><span data-stu-id="9ca53-473">[first-partition-key, aa)</span></span>

  <span data-ttu-id="9ca53-474">[aa, bb)</span><span class="sxs-lookup"><span data-stu-id="9ca53-474">[aa, bb)</span></span>

  <span data-ttu-id="9ca53-475">[bb, 마지막 파티션 키]</span><span class="sxs-lookup"><span data-stu-id="9ca53-475">[bb, last-partition-key]</span></span>

<span data-ttu-id="9ca53-476">**적용 대상:** 테이블</span><span class="sxs-lookup"><span data-stu-id="9ca53-476">**Applicable to:** Tables</span></span>

### <a name="splitsizefile-size"></a><span data-ttu-id="9ca53-477">/SplitSize:"file-size"</span><span class="sxs-lookup"><span data-stu-id="9ca53-477">/SplitSize:"file-size"</span></span>
<span data-ttu-id="9ca53-478">내보낸된 파일 크기 (MB) 분할 hello, 허용 된 hello 최소 값은 32를 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-478">Specifies hello exported file split size in MB, hello minimal value allowed is 32.</span></span>

<span data-ttu-id="9ca53-479">이 옵션을 지정 하지 않으면 AzCopy 테이블 데이터 tooa을 단일 파일 내보냅니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-479">If this option is not specified, AzCopy exports table data tooa single file.</span></span>

<span data-ttu-id="9ca53-480">Hello 테이블 데이터 내보낸된 tooa blob이 고 hello 내보낸된 파일 크기 제한에 도달 하면 hello 200GB blob 크기에 대 한 다음 AzCopy 분할 hello 내보낸된 파일을이 옵션이 지정 되지 않은 경우에 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-480">If hello table data is exported tooa blob, and hello exported file size reaches hello 200 GB limit for blob size, then AzCopy splits hello exported file, even if this option is not specified.</span></span>

<span data-ttu-id="9ca53-481">**적용 대상:** 테이블</span><span class="sxs-lookup"><span data-stu-id="9ca53-481">**Applicable to:** Tables</span></span>

### <a name="entityoperationinsertorskip--insertormerge--insertorreplace"></a><span data-ttu-id="9ca53-482">/EntityOperation:"InsertOrSkip" | "InsertOrMerge" | "InsertOrReplace"</span><span class="sxs-lookup"><span data-stu-id="9ca53-482">/EntityOperation:"InsertOrSkip" | "InsertOrMerge" | "InsertOrReplace"</span></span>
<span data-ttu-id="9ca53-483">Hello 테이블 데이터 가져오기 동작을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-483">Specifies hello table data import behavior.</span></span>

* <span data-ttu-id="9ca53-484">InsertOrSkip-기존 엔터티를 생략 하거나 hello 테이블에 존재 하지 않는 경우 새 엔터티를 삽입 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-484">InsertOrSkip - Skips an existing entity or inserts a new entity if it does not exist in hello table.</span></span>
* <span data-ttu-id="9ca53-485">InsertOrMerge-기존 엔터티를 병합 하거나 hello 테이블에 존재 하지 않는 경우 새 엔터티를 삽입 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-485">InsertOrMerge - Merges an existing entity or inserts a new entity if it does not exist in hello table.</span></span>
* <span data-ttu-id="9ca53-486">InsertOrReplace-기존 엔터티를 바꾸거나 hello 테이블에 존재 하지 않는 경우 새 엔터티를 삽입 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-486">InsertOrReplace - Replaces an existing entity or inserts a new entity if it does not exist in hello table.</span></span>

<span data-ttu-id="9ca53-487">**적용 대상:** 테이블</span><span class="sxs-lookup"><span data-stu-id="9ca53-487">**Applicable to:** Tables</span></span>

### <a name="manifestmanifest-file"></a><span data-ttu-id="9ca53-488">/Manifest:"manifest-file"</span><span class="sxs-lookup"><span data-stu-id="9ca53-488">/Manifest:"manifest-file"</span></span>
<span data-ttu-id="9ca53-489">Hello 테이블 내보내기 및 가져오기 작업에 대 한 hello 매니페스트 파일을 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-489">Specifies hello manifest file for hello table export and import operation.</span></span>

<span data-ttu-id="9ca53-490">Hello 내보내기 작업 중에이 옵션은 선택 사항, AzCopy이이 옵션을 지정 하지 않으면 미리 정의 된 이름으로 매니페스트 파일을 생성 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-490">This option is optional during hello export operation, AzCopy generates a manifest file with predefined name if this option is not specified.</span></span>

<span data-ttu-id="9ca53-491">이 옵션은 hello 데이터 파일을 찾기 위한 hello 가져오기 작업 동안 필요 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-491">This option is required during hello import operation for locating hello data files.</span></span>

<span data-ttu-id="9ca53-492">**적용 대상:** 테이블</span><span class="sxs-lookup"><span data-stu-id="9ca53-492">**Applicable to:** Tables</span></span>

### <a name="synccopy"></a><span data-ttu-id="9ca53-493">/SyncCopy</span><span class="sxs-lookup"><span data-stu-id="9ca53-493">/SyncCopy</span></span>
<span data-ttu-id="9ca53-494">나타냅니다 toosynchronously blob 또는 Azure 저장소는 두 개의 끝점 사이 파일 복사 여부.</span><span class="sxs-lookup"><span data-stu-id="9ca53-494">Indicates whether toosynchronously copy blobs or files between two Azure Storage endpoints.</span></span>

<span data-ttu-id="9ca53-495">기본적으로 AzCopy에서는 서버 쪽 비동기 복사를 사용합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-495">AzCopy by default uses server-side asynchronous copy.</span></span> <span data-ttu-id="9ca53-496">이 옵션 tooperform 동기 지정 blob 다운로드 또는 toolocal 메모리 파일 및 다음 저장소 tooAzure 업로드를 복사 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-496">Specify this option tooperform a synchronous copy, which downloads blobs or files toolocal memory and then uploads them tooAzure Storage.</span></span>

<span data-ttu-id="9ca53-497">파일 저장소 또는 Blob 저장소 tooFile 저장소 만들거나 기본 Blob 저장소 내의 파일을 복사할 때이 옵션을 사용할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-497">You can use this option when copying files within Blob storage, within File storage, or from Blob storage tooFile storage or vice versa.</span></span>

<span data-ttu-id="9ca53-498">**적용 대상:** Blob, 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-498">**Applicable to:** Blobs, Files</span></span>

### <a name="setcontenttypecontent-type"></a><span data-ttu-id="9ca53-499">/SetContentType:"content-type"</span><span class="sxs-lookup"><span data-stu-id="9ca53-499">/SetContentType:"content-type"</span></span>
<span data-ttu-id="9ca53-500">대상 blob 또는 파일에 대 한 hello MIME 콘텐츠 형식을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-500">Specifies hello MIME content type for destination blobs or files.</span></span>

<span data-ttu-id="9ca53-501">Blob에 대해 콘텐츠 형식 hello 또는 기본적으로 tooapplication/옥텟 스트림 파일 하는 AzCopy 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-501">AzCopy sets hello content type for a blob or file tooapplication/octet-stream by default.</span></span> <span data-ttu-id="9ca53-502">이 옵션에 대 한 값을 명시적으로 지정 하 여 모든 blob 또는 파일에 대 한 hello 콘텐츠 형식을 설정할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-502">You can set hello content type for all blobs or files by explicitly specifying a value for this option.</span></span>

<span data-ttu-id="9ca53-503">값이 없는이 옵션을 지정 하는 경우 각 blob 또는 파일의 콘텐츠 형식을 tooits 파일 확장명에 따라 AzCopy 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-503">If you specify this option without a value, then AzCopy sets each blob or file's content type according tooits file extension.</span></span>

<span data-ttu-id="9ca53-504">**적용 대상:** Blob, 파일</span><span class="sxs-lookup"><span data-stu-id="9ca53-504">**Applicable to:** Blobs, Files</span></span>

### <a name="payloadformatjson--csv"></a><span data-ttu-id="9ca53-505">/PayloadFormat:"JSON" | "CSV"</span><span class="sxs-lookup"><span data-stu-id="9ca53-505">/PayloadFormat:"JSON" | "CSV"</span></span>
<span data-ttu-id="9ca53-506">Hello 테이블 내보낸된 데이터 파일의 hello 형식을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-506">Specifies hello format of hello table exported data file.</span></span>

<span data-ttu-id="9ca53-507">이 옵션을 지정하지 않으면 기본적으로 AzCopy는 JSON 형식으로 테이블 데이터 파일을 내보냅니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-507">If this option is not specified, by default AzCopy exports table data file in JSON format.</span></span>

<span data-ttu-id="9ca53-508">**적용 대상:** 테이블</span><span class="sxs-lookup"><span data-stu-id="9ca53-508">**Applicable to:** Tables</span></span>

## <a name="known-issues-and-best-practices"></a><span data-ttu-id="9ca53-509">알려진 문제 및 모범 사례</span><span class="sxs-lookup"><span data-stu-id="9ca53-509">Known Issues and Best Practices</span></span>
### <a name="limit-concurrent-writes-while-copying-data"></a><span data-ttu-id="9ca53-510">데이터를 복사하는 동안 동시 쓰기 제한</span><span class="sxs-lookup"><span data-stu-id="9ca53-510">Limit concurrent writes while copying data</span></span>
<span data-ttu-id="9ca53-511">Blob 또는 AzCopy 사용 하 여 파일을 복사할 때는 다른 응용 프로그램 수정 하 있습니다 hello 데이터 복사 하려는 동안 염두에서에 둬야 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-511">When you copy blobs or files with AzCopy, keep in mind that another application may be modifying hello data while you are copying it.</span></span> <span data-ttu-id="9ca53-512">가능 하면 hello 복사 작업 중 hello 데이터를 복사 하는 수정 되지 않은 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-512">If possible, ensure that hello data you are copying is not being modified during hello copy operation.</span></span> <span data-ttu-id="9ca53-513">예를 들어 Azure 가상 컴퓨터와 연결 된 VHD를 복사 하는 경우 다른 응용 프로그램이 toohello VHD을 작성 하는 현재 선택 되어 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-513">For example, when copying a VHD associated with an Azure virtual machine, make sure that no other applications are currently writing toohello VHD.</span></span> <span data-ttu-id="9ca53-514">복사 hello 리소스 toobe 임대는 좋은 방법 toodo입니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-514">A good way toodo this is by leasing hello resource toobe copied.</span></span> <span data-ttu-id="9ca53-515">또는 먼저 hello VHD의 스냅샷을 만들 수 있으며 hello 스냅숏 복사 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-515">Alternately, you can create a snapshot of hello VHD first and then copy hello snapshot.</span></span>

<span data-ttu-id="9ca53-516">다른 응용 프로그램에서 복사 하는 것 다음 hello 타이머 hello 작업이 완료 된 것을 기억 하는 동안 tooblobs 또는 파일 쓰기를 방지할 수, 하는 경우 hello 복사 된 리소스에 실패 했습니다 hello 원본 리소스와 함께 전체 패리티 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-516">If you cannot prevent other applications from writing tooblobs or files while they are being copied, then keep in mind that by hello time hello job finishes, hello copied resources may no longer have full parity with hello source resources.</span></span>

### <a name="run-one-azcopy-instance-on-one-machine"></a><span data-ttu-id="9ca53-517">한 컴퓨터에서 AzCopy 인스턴스 하나를 실행합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-517">Run one AzCopy instance on one machine.</span></span>
<span data-ttu-id="9ca53-518">AzCopy는 컴퓨터 리소스 tooaccelerate hello 데이터 전송의 디자인 된 toomaximize hello 사용률, 한 컴퓨터에서 하나만 AzCopy 인스턴스를 실행 하 고 hello 옵션 지정 좋습니다 `/NC` 더 많은 동시 작업을 필요 합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-518">AzCopy is designed toomaximize hello utilization of your machine resource tooaccelerate hello data transfer, we recommend you run only one AzCopy instance on one machine, and specify hello option `/NC` if you need more concurrent operations.</span></span> <span data-ttu-id="9ca53-519">자세한 내용은 입력 `AzCopy /?:NC` hello 명령줄에서.</span><span class="sxs-lookup"><span data-stu-id="9ca53-519">For more details, type `AzCopy /?:NC` at hello command line.</span></span>

### <a name="enable-fips-compliant-md5-algorithms-for-azcopy-when-you-use-fips-compliant-algorithms-for-encryption-hashing-and-signing"></a><span data-ttu-id="9ca53-520">"암호화, 해시 및 서명에 FIPS 호환 알고리즘을 사용"할 경우 AzCopy에 대해 FIPS 규격 MD5 알고리즘을 사용하도록 설정합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-520">Enable FIPS compliant MD5 algorithms for AzCopy when you "Use FIPS compliant algorithms for encryption, hashing and signing".</span></span>
<span data-ttu-id="9ca53-521">기본적으로 AzCopy는 개체를 복사 하는 경우.NET MD5 구현 toocalculate hello MD5 사용 하 여 있지만 AzCopy tooenable FIPS 규격 MD5 설정 해야 하는 몇 가지 보안 요구 사항이 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-521">AzCopy by default uses .NET MD5 implementation toocalculate hello MD5 when copying objects, but there are some security requirements that need AzCopy tooenable FIPS compliant MD5 setting.</span></span>

<span data-ttu-id="9ca53-522">`AzureStorageUseV1MD5` 속성을 사용하여 app.config 파일(`AzCopy.exe.config`)을 만들고 AzCopy.exe를 통해 일단 사용을 보류할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-522">You can create an app.config file `AzCopy.exe.config` with property `AzureStorageUseV1MD5` and put it aside with AzCopy.exe.</span></span>
...
<span data-ttu-id="9ca53-523">True-"AzureStorageUseV1MD5" • 속성에 대 한 hello 기본 값, AzCopy이.NET MD5 구현의 사용합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-523">For property "AzureStorageUseV1MD5" • True - hello default value, AzCopy uses .NET MD5 implementation.</span></span>
<span data-ttu-id="9ca53-524">• False - AzCopy는 FIPS 규격 MD5 알고리즘을 사용합니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-524">• False – AzCopy uses FIPS compliant MD5 algorithm.</span></span>

<span data-ttu-id="9ca53-525">FIPS 규격 알고리즘은 Windows 컴퓨터에는 기본적으로 사용되지 않도록 설정되어 있으며 실행 창에 secpol.msc를 입력하고 보안 설정-> 로컬 정책-> 보안 옵션->시스템 암호화: 암호화, 해시, 서명에 FIPS 규격 알고리즘 사용에서 이 스위치를 선택할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9ca53-525">Note that FIPS compliant algorithms is disabled by default on your Windows machine, you can type secpol.msc in your Run window and check this switch at Security Setting->Local Policy->Security Options->System cryptography: Use FIPS compliant algorithms for encryption, hashing and signing.</span></span>

## <a name="next-steps"></a><span data-ttu-id="9ca53-526">다음 단계</span><span class="sxs-lookup"><span data-stu-id="9ca53-526">Next steps</span></span>
<span data-ttu-id="9ca53-527">Azure 저장소 및 AzCopy에 대 한 자세한 내용은 다음 리소스는 hello 참조:</span><span class="sxs-lookup"><span data-stu-id="9ca53-527">For more information about Azure Storage and AzCopy, see hello following resources:</span></span>

### <a name="azure-storage-documentation"></a><span data-ttu-id="9ca53-528">Azure 저장소 설명서</span><span class="sxs-lookup"><span data-stu-id="9ca53-528">Azure Storage documentation:</span></span>
* [<span data-ttu-id="9ca53-529">소개 tooAzure 저장소</span><span class="sxs-lookup"><span data-stu-id="9ca53-529">Introduction tooAzure Storage</span></span>](../storage-introduction.md)
* [<span data-ttu-id="9ca53-530">어떻게 toouse.NET에서 Blob 저장소</span><span class="sxs-lookup"><span data-stu-id="9ca53-530">How toouse Blob storage from .NET</span></span>](../blobs/storage-dotnet-how-to-use-blobs.md)
* [<span data-ttu-id="9ca53-531">어떻게 toouse.NET에서 파일 저장소</span><span class="sxs-lookup"><span data-stu-id="9ca53-531">How toouse File storage from .NET</span></span>](../storage-dotnet-how-to-use-files.md)
* [<span data-ttu-id="9ca53-532">어떻게 toouse.NET에서 테이블 저장소</span><span class="sxs-lookup"><span data-stu-id="9ca53-532">How toouse Table storage from .NET</span></span>](../../cosmos-db/table-storage-how-to-use-dotnet.md)
* [<span data-ttu-id="9ca53-533">Toocreate, 관리 또는 저장소 계정을 삭제 방법</span><span class="sxs-lookup"><span data-stu-id="9ca53-533">How toocreate, manage, or delete a storage account</span></span>](../storage-create-storage-account.md)
* [<span data-ttu-id="9ca53-534">Linux에서 AzCopy를 사용하여 데이터 전송</span><span class="sxs-lookup"><span data-stu-id="9ca53-534">Transfer data with AzCopy on Linux</span></span>](storage-use-azcopy-linux.md)

### <a name="azure-storage-blog-posts"></a><span data-ttu-id="9ca53-535">Azure 저장소 블로그 게시물:</span><span class="sxs-lookup"><span data-stu-id="9ca53-535">Azure Storage blog posts:</span></span>
* [<span data-ttu-id="9ca53-536">Azure 저장소 데이터 이동 라이브러리 미리 보기 소개</span><span class="sxs-lookup"><span data-stu-id="9ca53-536">Introducing Azure Storage Data Movement Library Preview</span></span>](https://azure.microsoft.com/blog/introducing-azure-storage-data-movement-library-preview-2/)
* [<span data-ttu-id="9ca53-537">AzCopy: 동기 복사본 및 사용자 지정 콘텐츠 형식 소개(영문)</span><span class="sxs-lookup"><span data-stu-id="9ca53-537">AzCopy: Introducing synchronous copy and customized content type</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2015/01/13/azcopy-introducing-synchronous-copy-and-customized-content-type.aspx)
* [<span data-ttu-id="9ca53-538">AzCopy: AzCopy 3.0의 일반 공급 및 테이블 및 파일을 지원하는 AzCopy 4.0의 미리 보기 릴리스 발표(영문)</span><span class="sxs-lookup"><span data-stu-id="9ca53-538">AzCopy: Announcing General Availability of AzCopy 3.0 plus preview release of AzCopy 4.0 with Table and File support</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2014/10/29/azcopy-announcing-general-availability-of-azcopy-3-0-plus-preview-release-of-azcopy-4-0-with-table-and-file-support.aspx)
* [<span data-ttu-id="9ca53-539">AzCopy: 대량 복사 시나리오에 맞게 최적화(영문)</span><span class="sxs-lookup"><span data-stu-id="9ca53-539">AzCopy: Optimized for Large-Scale Copy Scenarios</span></span>](http://go.microsoft.com/fwlink/?LinkId=507682)
* [<span data-ttu-id="9ca53-540">AzCopy: 읽기 액세스 지역 중복 저장소 지원(영문)</span><span class="sxs-lookup"><span data-stu-id="9ca53-540">AzCopy: Support for read-access geo-redundant storage</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2014/04/07/azcopy-support-for-read-access-geo-redundant-account.aspx)
* [<span data-ttu-id="9ca53-541">AzCopy: 다시 시작 가능 모드 및 SAS 토큰으로 데이터 전송(영문)</span><span class="sxs-lookup"><span data-stu-id="9ca53-541">AzCopy: Transfer data with re-startable mode and SAS token</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2013/09/07/azcopy-transfer-data-with-re-startable-mode-and-sas-token.aspx)
* [<span data-ttu-id="9ca53-542">AzCopy: 크로스 계정 Blob 복사 사용(영문)</span><span class="sxs-lookup"><span data-stu-id="9ca53-542">AzCopy: Using cross-account Copy Blob</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2013/04/01/azcopy-using-cross-account-copy-blob.aspx)
* [<span data-ttu-id="9ca53-543">AzCopy: Azure Blob 파일 업로드/다운로드(영문)</span><span class="sxs-lookup"><span data-stu-id="9ca53-543">AzCopy: Uploading/downloading files for Azure Blobs</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2012/12/03/azcopy-uploading-downloading-files-for-windows-azure-blobs.aspx)
