...
---
# <a name="implement-failover-streaming-with-azure-media-services"></a><span data-ttu-id="ea30c-103">Azure Media Services를 사용하여 장애 조치 스트리밍 구현</span><span class="sxs-lookup"><span data-stu-id="ea30c-103">Implement failover streaming with Azure Media Services</span></span>

<span data-ttu-id="ea30c-104">이 연습에서는 주문형 스트리밍에 대한 중복성을 처리 하기 위해 한 자산에서 다른 자산으로 콘텐츠(Blob)를 복사하는 방법을 보여줍니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-104">This walkthrough demonstrates how to copy content (blobs) from one asset into another in order to handle redundancy for on-demand streaming.</span></span> <span data-ttu-id="ea30c-105">이 시나리오는 데이터 센터에서 가동이 중단되는 경우 두 데이터 센터 간에 장애 조치할 Azure Content Delivery Network를 설정하려는 경우에 유용합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-105">This scenario is useful if you want to set up Azure Content Delivery Network to fail over between two datacenters, in case of an outage in one datacenter.</span></span> <span data-ttu-id="ea30c-106">이 연습에서는 Azure Media Services SDK, Azure Media Services REST API 및 Azure Storage SDK를 사용하여 다음 작업을 설명합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-106">This walkthrough uses the Azure Media Services SDK, the Azure Media Services REST API, and the Azure Storage SDK to demonstrate the following tasks:</span></span>

1. <span data-ttu-id="ea30c-107">"데이터 센터 A"에서 Media Services 계정을 설정합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-107">Set up a Media Services account in "Data Center A."</span></span>
2. <span data-ttu-id="ea30c-108">mezzanine 파일을 원본 자산에 업로드합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-108">Upload a mezzanine file into a source asset.</span></span>
3. <span data-ttu-id="ea30c-109">자산을 다중 비트 전송률 MP4 파일로 인코딩합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-109">Encode the asset into multi-bit rate MP4 files.</span></span> 
4. <span data-ttu-id="ea30c-110">읽기 전용 공유 액세스 서명 로케이터를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-110">Create a read-only shared access signature locator.</span></span> <span data-ttu-id="ea30c-111">원본 자산에 원본 자산과 연관된 저장소 계정에서 컨테이너에 대한 읽기 액세스 권한을 부여합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-111">This is for the source asset to have read access to the container in the storage account that is associated with the source asset.</span></span>
5. <span data-ttu-id="ea30c-112">이전 단계에서 만든 읽기 전용 공유 액세스 서명 로케이터에서 원본 자산의 컨테이너 이름을 가져옵니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-112">Get the container name of the source asset from the read-only shared access signature locator created in the previous step.</span></span> <span data-ttu-id="ea30c-113">저장소 계정 간에 Blob을 복사하기 위해 필요합니다(이 항목의 뒷부분에서 설명함).</span><span class="sxs-lookup"><span data-stu-id="ea30c-113">This is necessary for copying blobs between storage accounts (explained later in the topic.)</span></span>
6. <span data-ttu-id="ea30c-114">인코딩 작업에서 만든 자산에 원본 로케이터를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-114">Create an origin locator for the asset that was created by the encoding task.</span></span> 

<span data-ttu-id="ea30c-115">그런 다음 장애 조치를 처리합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-115">Then, to handle the failover:</span></span>

1. <span data-ttu-id="ea30c-116">"데이터 센터 B"에서 Media Services 계정을 설정합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-116">Set up a Media Services account in "Data Center B."</span></span>
2. <span data-ttu-id="ea30c-117">대상 미디어 서비스 계정에서 빈 대상 자산을 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-117">Create a target empty asset in the target Media Services account.</span></span>
3. <span data-ttu-id="ea30c-118">쓰기 공유 액세스 서명 로케이터를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-118">Create a write shared access signature locator.</span></span> <span data-ttu-id="ea30c-119">빈 대상 자산에 대상 자산과 연관된 대상 저장소 계정에서 컨테이너에 대한 쓰기 액세스 권한을 부여합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-119">This is for the target empty asset to have write access to the container in the target storage account that is associated with the target asset.</span></span>
4. <span data-ttu-id="ea30c-120">Azure Storage SDK를 사용하여 "데이터 센터 A"의 원본 저장소 계정과 "데이터 센터 B"의 대상 저장소 계정 간에 Blob(자산 파일)을 복사합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-120">Use the Azure Storage SDK to copy blobs (asset files) between the source storage account in "Data Center A" and the target storage account in "Data Center B."</span></span> <span data-ttu-id="ea30c-121">이러한 저장소 계정은 관련 자산과 연결됩니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-121">These storage accounts are associated with the assets of interest.</span></span>
5. <span data-ttu-id="ea30c-122">대상 자산을 사용하여 대상 blob 컨테이너에 복사된 blob(자산 파일)를 연결합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-122">Associate blobs (asset files) that were copied to the target blob container with the target asset.</span></span> 
6. <span data-ttu-id="ea30c-123">"데이터 센터 B"의 자산에 원본 로케이터를 만들고 "데이터 센터 A"의 자산에 생성된 로케이터 ID를 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-123">Create an origin locator for the asset in "Data Center B", and specify the locator ID that was generated for the asset in "Data Center A."</span></span>

<span data-ttu-id="ea30c-124">이렇게 하면 URL의 상대 경로가 동일한 스트리밍 URL 위치를 제공합니다.(기본 URL만 다름)</span><span class="sxs-lookup"><span data-stu-id="ea30c-124">This gives you the streaming URLs where the relative paths of the URLs are the same (only the base URLs are different).</span></span> 

<span data-ttu-id="ea30c-125">그런 다음 정전 문제를 처리하려면 이러한 원본 로케이터 위에 Content Delivery Network를 만들 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-125">Then, to handle any outages, you can create a Content Delivery Network on top of these origin locators.</span></span> 

<span data-ttu-id="ea30c-126">고려 사항은 다음과 같습니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-126">The following considerations apply:</span></span>

* <span data-ttu-id="ea30c-127">미디어 서비스 SDK의 현재 버전은 자산 파일과 자산을 연결하는 IAssetFile 정보를 프로그래밍 방식으로 생성하도록 지원하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-127">The current version of Media Services SDK does not support programmatically generating IAssetFile information that would associate an asset with asset files.</span></span> <span data-ttu-id="ea30c-128">이를 수행하려면 대신 CreateFileInfos Media Services REST API를 사용합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-128">Instead, use the CreateFileInfos Media Services REST API to do this.</span></span> 
* <span data-ttu-id="ea30c-129">자산을 암호화한 저장소(AssetCreationOptions.StorageEncrypted)는 복제에 지원되지 않습니다(암호화 키가 Media Services 계정 모두에서 다르기 때문임).</span><span class="sxs-lookup"><span data-stu-id="ea30c-129">Storage encrypted assets (AssetCreationOptions.StorageEncrypted) are not supported for replication (because the encryption key is different in both Media Services accounts).</span></span> 
* <span data-ttu-id="ea30c-130">동적 패키징을 활용하려면 콘텐츠를 스트리밍하려는 스트리밍 끝점이 **실행** 상태인지 확인합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-130">If you want to take advantage of dynamic packaging, make sure the streaming endpoint from which you want to stream  your content is in the **Running** state.</span></span>

> [!NOTE]
> <span data-ttu-id="ea30c-131">수동으로 장애 조치 스트리밍 시나리오를 구현하는 대안으로 미디어 서비스 [복제기 도구](http://replicator.codeplex.com/) 를 사용하는 것이 좋습니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-131">Consider using the Media Services [Replicator Tool](http://replicator.codeplex.com/) as an alternative to implementing a failover streaming scenario manually.</span></span> <span data-ttu-id="ea30c-132">이 도구를 사용하면 두 미디어 서비스 계정 간에 자산을 복제할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-132">This tool allows you to replicate assets across two Media Services accounts.</span></span>
> 
> 

## <a name="prerequisites"></a><span data-ttu-id="ea30c-133">필수 조건</span><span class="sxs-lookup"><span data-stu-id="ea30c-133">Prerequisites</span></span>
* <span data-ttu-id="ea30c-134">신규 또는 기존 Azure 구독의 Media Services 계정 2개.</span><span class="sxs-lookup"><span data-stu-id="ea30c-134">Two Media Services accounts in a new or existing Azure subscription.</span></span> <span data-ttu-id="ea30c-135">[Media Services 계정을 만드는 방법](media-services-portal-create-account.md)을 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="ea30c-135">See [How to Create a Media Services Account](media-services-portal-create-account.md).</span></span>
* <span data-ttu-id="ea30c-136">운영 체제: Windows 7, Windows 2008 R2 또는 Windows 8.</span><span class="sxs-lookup"><span data-stu-id="ea30c-136">Operating system: Windows 7, Windows 2008 R2, or Windows 8.</span></span>
* <span data-ttu-id="ea30c-137">.NET Framework 4.5 또는 .NET Framework 4</span><span class="sxs-lookup"><span data-stu-id="ea30c-137">.NET Framework 4.5 or .NET Framework 4.</span></span>
* <span data-ttu-id="ea30c-138">Visual Studio 2010 SP1 또는 later version (Professional, Premium, Ultimate, 또는 Express).</span><span class="sxs-lookup"><span data-stu-id="ea30c-138">Visual Studio 2010 SP1 or later version (Professional, Premium, Ultimate, or Express).</span></span>

## <a name="set-up-your-project"></a><span data-ttu-id="ea30c-139">프로젝트 설정</span><span class="sxs-lookup"><span data-stu-id="ea30c-139">Set up your project</span></span>
<span data-ttu-id="ea30c-140">이 섹션에서는 C# 콘솔 응용 프로그램 프로젝트를 만들고 설정합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-140">In this section, you create and set up a C# Console Application project.</span></span>

1. <span data-ttu-id="ea30c-141">Visual Studio를 사용하여 C# 콘솔 응용 프로그램 프로젝트가 포함된 새 솔루션을 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-141">Use Visual Studio to create a new solution that contains the C# Console Application project.</span></span> <span data-ttu-id="ea30c-142">이름에 **HandleRedundancyForOnDemandStreaming**을 입력하고 **확인**을 클릭합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-142">Enter **HandleRedundancyForOnDemandStreaming** for the name, and then click **OK**.</span></span>
2. <span data-ttu-id="ea30c-143">**HandleRedundancyForOnDemandStreaming.csproj** 프로젝트 파일과 동일한 수준에 **SupportFiles** 폴더를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-143">Create the **SupportFiles** folder on the same level as the **HandleRedundancyForOnDemandStreaming.csproj** project file.</span></span> <span data-ttu-id="ea30c-144">**SupportFiles** 폴더에서 **OutputFiles** 및 **MP4Files** 폴더를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-144">Under the **SupportFiles** folder, create the **OutputFiles** and **MP4Files** folders.</span></span> <span data-ttu-id="ea30c-145">.mp4 파일을 **MP4Files** 폴더에 복사합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-145">Copy an .mp4 file into the **MP4Files** folder.</span></span> <span data-ttu-id="ea30c-146">(이 예제에서는 **BigBuckBunny.mp4** 파일을 사용합니다.)</span><span class="sxs-lookup"><span data-stu-id="ea30c-146">(In this example, the **BigBuckBunny.mp4** file is used.)</span></span> 
3. <span data-ttu-id="ea30c-147">**Nuget**을 사용하여 Media Services와 관련된 DLL에 참조를 추가합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-147">Use **Nuget** to add references to DLLs related to Media Services.</span></span> <span data-ttu-id="ea30c-148">**Visual Studio 주 메뉴**에서 **도구** > **라이브러리 패키지 관리자** > **패키지 관리자 콘솔**을 선택합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-148">In **Visual Studio Main Menu**, select **TOOLS** > **Library Package Manager** > **Package Manager Console**.</span></span> <span data-ttu-id="ea30c-149">콘솔 창에 **Install-Package windowsazure.mediaservices**를 입력하고 Enter를 누릅니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-149">In the console window, type **Install-Package windowsazure.mediaservices**, and press Enter.</span></span>
4. <span data-ttu-id="ea30c-150">System.Configuration, System.Runtime.Serialization 및 System.Web와 같이 이 프로젝트에 필요한 다른 참조를 추가합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-150">Add other references that are required for this project: System.Configuration, System.Runtime.Serialization, and System.Web.</span></span>
5. <span data-ttu-id="ea30c-151">기본적으로 **Programs.cs** 파일에 추가한 **using** 문을 다음 중 하나로 바꿉니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-151">Replace **using** statements that were added to the **Programs.cs** file by default with the following ones:</span></span>
...
6. <span data-ttu-id="ea30c-152">**appSettings** 섹션을 **.config** 파일에 추가하고 Media Services 및 저장소 키에 기반한 값과 이름 값을 업데이트합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-152">Add the **appSettings** section to the **.config** file, and update the values based on your Media Services and Storage key and name values.</span></span> 
...
## <a name="add-code-that-handles-redundancy-for-on-demand-streaming"></a><span data-ttu-id="ea30c-153">주문형 스트리밍에 대한 중복성을 처리하는 코드 추가</span><span class="sxs-lookup"><span data-stu-id="ea30c-153">Add code that handles redundancy for on-demand streaming</span></span>
<span data-ttu-id="ea30c-154">이 섹션에서는 중복성을 처리하는 기능을 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-154">In this section, you create the ability to handle redundancy.</span></span>

1. <span data-ttu-id="ea30c-155">Program 클래스에 다음 클래스 수준 필드를 추가합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-155">Add the following class-level fields to the Program class.</span></span>
...
2. <span data-ttu-id="ea30c-156">기본 Main 메서드 정의를 다음 정의로 바꿉니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-156">Replace the default Main method definition with the following one.</span></span> <span data-ttu-id="ea30c-157">Main에서 호출된 메서드 정의는 아래에 정의되어있습니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-157">Method definitions that are called from Main are defined below.</span></span>
...
3. <span data-ttu-id="ea30c-158">다음 메서드 정의는 Main에서 호출됩니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-158">The following method definitions are called from Main.</span></span>

    >[!NOTE]
    ><span data-ttu-id="ea30c-159">다른 Media Services 정책(예: 로케이터 정책 또는 ContentKeyAuthorizationPolicy의 경우)은 1,000,000개로 제한됩니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-159">There is a limit of 1,000,000 policies for different Media Services policies (for example, for Locator policy or ContentKeyAuthorizationPolicy).</span></span> <span data-ttu-id="ea30c-160">항상 동일한 날짜 및 액세스 권한을 사용하는 경우 동일한 정책 ID를 사용해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-160">You should use the same policy ID if you are always using the same days and access permissions.</span></span> <span data-ttu-id="ea30c-161">예를 들어, 장기간 위치에 유지하려는 로케이터의 정책에 동일한 ID를 사용합니다(비 업로드 정책).</span><span class="sxs-lookup"><span data-stu-id="ea30c-161">For example, use the same ID for policies for locators that are intended to remain in place for a long time (non-upload policies).</span></span> <span data-ttu-id="ea30c-162">자세한 내용은 [이 항목](media-services-dotnet-manage-entities.md#limit-access-policies) 을 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="ea30c-162">For more information, see [this topic](media-services-dotnet-manage-entities.md#limit-access-policies).</span></span>
...
## <a name="next-steps"></a><span data-ttu-id="ea30c-163">다음 단계</span><span class="sxs-lookup"><span data-stu-id="ea30c-163">Next steps</span></span>
<span data-ttu-id="ea30c-164">이제 두 데이터 센터 간의 경로 요청에 Traffic Manager를 사용하면 중단된 경우 장애 조치할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="ea30c-164">You can now use a traffic manager to route requests between the two datacenters, and thus fail over in case of any outages.</span></span>

## <a name="media-services-learning-paths"></a><span data-ttu-id="ea30c-165">미디어 서비스 학습 경로</span><span class="sxs-lookup"><span data-stu-id="ea30c-165">Media Services learning paths</span></span>
[!INCLUDE [media-services-learning-paths-include](../../includes/media-services-learning-paths-include.md)]

## <a name="provide-feedback"></a><span data-ttu-id="ea30c-166">피드백 제공</span><span class="sxs-lookup"><span data-stu-id="ea30c-166">Provide feedback</span></span>
