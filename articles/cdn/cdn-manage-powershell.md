...
PowerShell은 Azure CDN 프로필 및 끝점 hello 가장 유연한 방법 toomanage 중 하나를 제공합니다.  대화형으로 또는 tooautomate 관리 작업 스크립트를 작성 하 여 PowerShell을 사용할 수 있습니다.  이 자습서에서는 여러 설명 hello 가장 일반적인 작업을 수행할 수 있는 PowerShell toomanage Azure CDN 프로필 및 끝점입니다.

## <a name="prerequisites"></a>필수 조건
toouse PowerShell toomanage 사용자 프로필 Azure CDN과 끝점을 hello Azure PowerShell 모듈이 설치 되어 있어야 합니다.  toolearn 어떻게 tooinstall Azure PowerShell tooAzure hello를 사용 하 여 연결 `Login-AzureRmAccount` cmdlet 참조 [어떻게 tooinstall Azure PowerShell을 구성 하 고](/powershell/azure/overview)합니다.
...
## <a name="listing-hello-azure-cdn-cmdlets"></a>Hello Azure CDN cmdlet 목록
Hello를 사용 하 여 모든 hello Azure CDN cmdlet을 나열할 수 있습니다 `Get-Command` cmdlet.
...
Hello를 사용 하 여 이러한 cmdlet를 사용 하 여 도움말을 가져와서 `Get-Help` cmdlet.  `Get-Help` 는 사용량 및 구문을 제공하며 선택적으로 예제를 표시합니다.
...
hello `Get-AzureRmCdnProfile` cmdlet 매개 변수 없이 모든 기존 CDN 프로필을 검색 합니다.

```powershell
Get-AzureRmCdnProfile
```

이 출력에는 열거형에 대 한 파이프 된 toocmdlets 될 수 있습니다.
...
Hello 프로필 이름 및 리소스 그룹을 지정 하 여 단일 프로필을 반환할 수 있습니다.
...
> [!TIP]
> 가능한 toohave로 서로 다른 리소스 그룹에 이름과 같은 이름을 hello로 여러 CDN 프로필은  Hello 생략 `ResourceGroupName` 매개 변수 이름이 일치 하는 모든 프로필을 반환 합니다.
> 
> 

## <a name="listing-existing-cdn-endpoints"></a>기존 CDN 끝점 목록화
`Get-AzureRmCdnEndpoint`개별 끝점 또는 프로필에서 모든 hello 끝점을 검색할 수 있습니다.  
...
`New-AzureRmCdnProfile`및 `New-AzureRmCdnEndpoint` 은 사용 되는 toocreate CDN 프로필 및 끝점입니다.
...
`New-AzureRmCdnCustomDomain`사용자 지정 도메인 이름을 tooan 기존 끝점을 추가합니다.

> [!IMPORTANT]
> 에 설명 된 대로 DNS 공급자로 CNAME hello를 설정 해야 [어떻게 toomap 사용자 지정 도메인 tooContent CDN (배달 네트워크) 끝점](cdn-map-content-to-custom-domain.md)합니다.  Hello 매핑을 사용 하 여 끝점을 수정 하기 전에 테스트할 수 있습니다 `Test-AzureRmCdnCustomDomain`합니다.
> 
> 
...
`Start-AzureRmCdnEndpoint`및 `Stop-AzureRmCdnEndpoint` 사용된 toostart 수 있고 개별 끝점 또는 끝점 그룹을 중지 합니다.
...
`Remove-AzureRmCdnProfile`및 `Remove-AzureRmCdnEndpoint` 사용된 tooremove 프로필 및 끝점 될 수 있습니다.
...
toolearn CDN 기능에 대 한 참조 [CDN 개요](cdn-overview.md)합니다.
