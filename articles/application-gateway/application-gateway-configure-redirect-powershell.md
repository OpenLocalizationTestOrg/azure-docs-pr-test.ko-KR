...
---
# <a name="configure-redirection-on-application-gateway-with-powershell"></a><span data-ttu-id="5050d-103">PowerShell을 사용하여 Application Gateway에서 리디렉션 구성</span><span class="sxs-lookup"><span data-stu-id="5050d-103">Configure redirection on Application Gateway with PowerShell</span></span>

<span data-ttu-id="5050d-104">응용 프로그램 게이트웨이는 정의된 구성에 따라 트래픽을 리디렉션하는 기능을 지원합니다.</span><span class="sxs-lookup"><span data-stu-id="5050d-104">Application gateway supports the ability to redirect traffic based on a defined configuration.</span></span> <span data-ttu-id="5050d-105">일반 방문에서 리디렉션에 대해 자세히 알아보려면 [Application Gateway 리디렉션 개요](application-gateway-redirect-overview.md)를 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="5050d-105">To learn more about redirection in general visit, [Application Gateway redirect overview](application-gateway-redirect-overview.md).</span></span> <span data-ttu-id="5050d-106">이 문서에서는 HTTP에서 HTTPS로의 리디렉션, 경로 기반 리디렉션, 다중 사이트 리디렉션 및 외부 사이트로의 리디렉션에 대한 예제를 제공합니다.</span><span class="sxs-lookup"><span data-stu-id="5050d-106">This article provides examples of HTTP to HTTPS redirection, path based redirects, multi-site redirects, and redirects to external sites.</span></span>

## <a name="http-to-https-redirect-on-an-existing-application-gateway"></a><span data-ttu-id="5050d-107">기존 응용 프로그램 게이트웨이에서 HTTP에서 HTTPS로 리디렉션</span><span class="sxs-lookup"><span data-stu-id="5050d-107">HTTP to HTTPS redirect on an existing application gateway</span></span>

<span data-ttu-id="5050d-108">다음 예제에서는 80 포트에 새 HTTP 수신기를 만들어 443 포트에서 기존 수신기로 요청을 리디렉션합니다.</span><span class="sxs-lookup"><span data-stu-id="5050d-108">The following example creates a new HTTP listener on port 80 to redirect requests to the existing listener on port 443.</span></span>
...
<span data-ttu-id="5050d-109">응용 프로그램 게이트웨이가 업데이트되면 80 포트에서 새 수신기를 테스트합니다.</span><span class="sxs-lookup"><span data-stu-id="5050d-109">Once the application gateway is updated, test the new listener on port 80.</span></span>  <span data-ttu-id="5050d-110">다음 예제에서는 `curl`을 사용하여 리디렉션을 테스트합니다.</span><span class="sxs-lookup"><span data-stu-id="5050d-110">The following example shows using `curl` test the redirection.</span></span>
...
## <a name="path-based-redirect"></a><span data-ttu-id="5050d-111">경로 기반 리디렉션</span><span class="sxs-lookup"><span data-stu-id="5050d-111">Path based redirect</span></span>

<span data-ttu-id="5050d-112">다음 예제에서는 특정 URL 경로를 충족하는 트래픽만 리디렉션하는 기존 응용 프로그램 게이트웨이에 새 수신기 및 경로 기반 규칙을 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="5050d-112">The following example creates a new listener and a path based rule on an existing application gateway that redirects only traffic meeting the specific url path.</span></span>
...
## <a name="multi-site-redirect"></a><span data-ttu-id="5050d-113">다중 사이트 리디렉션</span><span class="sxs-lookup"><span data-stu-id="5050d-113">Multi-site redirect</span></span>

<span data-ttu-id="5050d-114">다음 예제에서는 80 포트에 2개의 다중 사이트 수신기가 있는 새 응용 프로그램 게이트웨이를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="5050d-114">The following example creates a new application gateway with 2 multi-site listeners on port 80.</span></span> <span data-ttu-id="5050d-115">수신기는 adatum.com 및 adatum.org입니다.</span><span class="sxs-lookup"><span data-stu-id="5050d-115">The listeners are for adatum.com and adatum.org.</span></span> <span data-ttu-id="5050d-116">adatum.org에서 adatum.com으로 트래픽을 리디렉션하는 리디렉션 규칙을 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="5050d-116">A redirect rule is created to redirect traffic from adatum.org to adatum.com.</span></span> <span data-ttu-id="5050d-117">CNAME 별칭을 응용 프로그램 게이트웨이 공용 IP 주소로 구성하려면 추가 구성이 필요합니다. 도메인을 Azure DNS에 위임하고 도메인에 대한 CNAME 레코드를 만드는 방법에 대한 자세한 내용은 [Azure DNS에 도메인 위임](../dns/dns-delegate-domain-azure-dns.md)을 방문하세요.</span><span class="sxs-lookup"><span data-stu-id="5050d-117">Additional configuration is required for configuring CNAME aliases to the application gateway public IP address, for more information on delegating your domain to Azure DNS and creating CNAME records for your domain visit, [Delegate a domain to Azure DNS](../dns/dns-delegate-domain-azure-dns.md).</span></span>
...
## <a name="redirect-to-an-external-site"></a><span data-ttu-id="5050d-118">외부 사이트로 리디렉션</span><span class="sxs-lookup"><span data-stu-id="5050d-118">Redirect to an external site</span></span>

<span data-ttu-id="5050d-119">다음 예제에서는 응용 프로그램 게이트웨이로 들어오는 트래픽을 외부 웹 사이트(bing.com)로 리디렉션합니다.</span><span class="sxs-lookup"><span data-stu-id="5050d-119">The following example redirects traffic coming into the application gateway to an external website (bing.com).</span></span> <span data-ttu-id="5050d-120">`TargetUrl` 스위치를 사용하면 `IncludePath` 및 `IncludeQuery` 스위치는 허용되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="5050d-120">When using the `TargetUrl` switch the `IncludePath` and `IncludeQuery` switches are not allowed.</span></span>
...
## <a name="next-steps"></a><span data-ttu-id="5050d-121">다음 단계</span><span class="sxs-lookup"><span data-stu-id="5050d-121">Next steps</span></span>

<span data-ttu-id="5050d-122">[PowerShell을 사용하여 Application Gateway에서 종단 간 SSL 구성](application-gateway-end-to-end-ssl-powershell.md)을 방문합니다.</span><span class="sxs-lookup"><span data-stu-id="5050d-122">Visit [configure end to end SSL with application gateway using PowerShell](application-gateway-end-to-end-ssl-powershell.md)</span></span>