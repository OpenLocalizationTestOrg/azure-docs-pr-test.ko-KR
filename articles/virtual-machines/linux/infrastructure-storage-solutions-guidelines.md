...
* 명명 규칙을 사용 하 여 저장소 계정의 hello 집합을 만듭니다. Hello Azure CLI 또는 hello 포털을 사용할 수 있습니다.
...
운영 체제 디스크 및 데이터 디스크의 최대 크기는 4TB입니다. 데이터 디스크 toopresent 논리 보다 큰 볼륨 1023GB tooyour VM 함께 풀링 하 여 논리 볼륨 관리자 (LVM) toosurpass이 한이도 사용할 수 있습니다.
...
을 사용 있으므로 toocreate 디스크 1,023 GB 보다 큰 대부분의 경우에서 외에도 데이터 디스크에 대 한 스트라이프를 사용 하 여 단일 볼륨에 대 한 tooback hello 저장소 여러 blob를 허용 하 여 성능이 향상 됩니다. 스트라이핑 기능을 hello I/O 필요한 toowrite 및 단일 논리적 디스크에서 읽은 데이터 동시에 진행 됩니다.

Azure는 데이터 디스크 개수 hello 및 hello VM 크기에 따라 사용할 수 있는 대역폭에 대 한 제한을 적용 됩니다. 자세한 내용은 [가상 컴퓨터 크기](sizes.md)를 참조하세요.
