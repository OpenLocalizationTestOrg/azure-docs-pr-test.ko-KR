...
* 웹 계층의 프런트 엔드 hello 클라이언트를 실행 하는 두 개의 nginx 서버
...
* 서브넷인에 속하는 모든 hello 서버:
  ...
  * hello 응용 프로그램 서버, MongoDB 클러스터 및 도메인 컨트롤러에 대 한 백 엔드 서브넷

![응용 프로그램 인프라의 여러 다른 계층 다이어그램](./media/infrastructure-example/example-tiers.png)

보안 들어오는 웹 트래픽의 같아야 hello 웹 서버 간에 부하 분산 된 고객 hello 온라인 스토어를 검색할 합니다. 주문 hello 형식의 HTTP 트래픽을 처리 hello 웹 서버 해야 수에 부하 분산 hello 응용 프로그램 서버에서에서 요청 합니다. 또한 고가용성을 위해 hello 인프라를 설계 해야 합니다.
...
* **프리미엄 저장소** hello MongoDB 분할 클러스터 서버 및 데이터 디스크에 대 한 합니다.
...
* **db로 azos 사용** hello 서버 hello MongoDB 분할 클러스터에 대 한
...
* **azos-사용-vm-d b를 01** hello 클러스터의 첫 번째 MongoDB 서버 hello에 대 한
* **사용 하 여 vm db02 azos** hello 클러스터의 두 번째 MongoDB 서버 hello에 대 한
