...
# <a name="task-fail-event"></a>태스크 실패 이벤트
 ...
 이 이벤트는 태스크가 오류와 함께 완료되면 내보내집니다. 현재 0이 아닌 종료 코드는 모두 오류로 간주됩니다. 이 이벤트는 발생 *외에* 작업 이벤트를 완료 하 고 작업에 실패 한 경우 사용 되는 toodetect 될 수 있습니다.


 hello 다음 예제에서는 작업의 본문 hello 실패 이벤트
