...
진단 로그는 기본적으로 해제되어 있습니다. tooenable 진단 로그:
...
* **ArchiveLogs**: 로그 관련 tooEvent 허브 아카이브 구체적으로, 관련된 tooarchive 오류를 기록 합니다.
* **OperationalLogs**: 특히 이벤트 허브 작업 하는 동안 프로세스에 대 한 내용은 작업 유형, 이벤트 허브 만들기를 사용 하는 리소스를 포함 하 여 hello 및 hello hello 작업의 상태입니다.
...
다음 표에 hello에 나열 된 요소를 포함 하는 보관 로그 JSON 문자열:

이름 | 설명
------- | -------
TaskName | Hello 작업에 실패 한 클라이언트의 설명입니다.
...
hello 다음 코드는 아카이브 로그 JSON 문자열의 예:
...
이름 | 설명
------- | -------
ActivityId | 내부 ID tootrack 목적을 사용 합니다.
EventName | 작업 이름입니다.  
resourceId | Azure Resource Manager 리소스 ID입니다.
...
hello 다음 코드는 작업 로그 JSON 문자열의 예:
...
* [소개 tooEvent 허브](event-hubs-what-is-event-hubs.md)
