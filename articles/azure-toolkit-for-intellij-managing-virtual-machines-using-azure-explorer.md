...
---
# <a name="manage-virtual-machines-by-using-hello-azure-explorer-for-intellij"></a>IntelliJ 용 hello Azure 탐색기를 사용 하 여 가상 컴퓨터를 관리 합니다.

hello hello IntelliJ 용 Azure 도구 키트의 일부인 Azure 탐색기의 Azure 계정과에서에서 가상 컴퓨터를 관리 하기 위한 사용 하기 쉬운 솔루션과 Java 개발자가 제공 hello IntelliJ 통합된 개발 환경 (IDE) 안에 있습니다.
...
1. Hello hello 단계를 사용 하 여 tooyour Azure 계정에에서 로그인 [로그인에 대 한 지침 Azure 도구 키트 hello IntelliJ] 문서.
...
4. Hello에 **가상 컴퓨터 이미지 선택** 창의 hello 다음 정보를 입력 합니다.
   ...
   * **사용자 지정 이미지**: 지정 hello 다음 정보를 제공 하 여 사용자 지정 이미지를 선택 합니다.

      * **게시자**: 가상 컴퓨터에 사용할 hello 이미지를 만든 hello 게시자를 지정 합니다 (예를 들어 *Microsoft*).
...
8. Hello에 **연결 된 리소스** 창 hello 다음 정보를 입력 합니다.

   * **리소스 그룹**: 가상 컴퓨터에 대 한 hello 리소스 그룹을 지정 합니다. Hello 다음 옵션 중 하나를 선택 합니다.
      * **새로 만들기**: toocreate 새 리소스 그룹 한다고 지정 합니다.
      * **기존 항목 사용**: Azure 계정에 연관 된 리소스 그룹의 목록에서 tooselect 한다고 지정 합니다.

       ![hello 연결 된 리소스 창][CR07]

   * **저장소 계정**: 가상 컴퓨터를 저장 하기 위한 저장소 계정 toouse hello를 지정 합니다. 기존 저장소 계정을 사용하거나 새 저장소 계정을 만들 수 있습니다. 선택 하면 **새로 만들기**, hello 다음 대화 상자가 나타납니다.

      ![hello 저장소 계정 만들기 대화 상자][CR05]

   * **가상 네트워크** 및 **서브넷**: hello 가상 네트워크 및 가상 컴퓨터에 연결 하는 서브넷을 지정 합니다. 기존 네트워크 및 서브넷을 사용하거나 새 네트워크 및 서브넷을 만들 수 있습니다. 선택 하는 경우 **새로 만들기**, hello 다음 대화 상자가 나타납니다.
   ...
   * **가용성 집합**: 가상 컴퓨터가 속할 선택적 가용성 집합을 지정합니다. 수 기존 가용성 집합, 새 가용성 집합 만들기를 선택 하거나 가상 컴퓨터 tooan 가용성 집합에 속하지 것입니다 선택 **(없음)**합니다.

9. **마침**을 클릭합니다.  
    새 가상 컴퓨터에 hello Azure 탐색기 도구 창에 나타납니다. 

   ![Hello Azure 탐색기 보기에서에서 새 가상 컴퓨터][CR08]

## <a name="restart-a-virtual-machine-in-intellij"></a>IntelliJ에서 가상 컴퓨터 다시 시작

toorestart IntelliJ의 hello Azure 탐색기를 사용 하 여 가상 컴퓨터는 다음 hello지 않습니다.
...
IntelliJ의 hello Azure 탐색기를 사용 하 여 실행 중인 가상 컴퓨터를 tooshut 다음 hello지 않습니다.
...
toodelete IntelliJ의 hello Azure 탐색기를 사용 하 여 가상 컴퓨터는 다음 hello지 않습니다.
