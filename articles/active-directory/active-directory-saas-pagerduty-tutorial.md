...
이 자습서에 설명 어떻게 toointegrate Azure Active Directory (Azure AD)와 PagerDuty 합니다.

Azure AD와 PagerDuty 통합 이점을 다음 hello로 제공 합니다.

- 액세스 tooPagerDuty을 지닌 Azure AD에서 제어할 수 있습니다.
- 프로그램 사용자 tooautomatically get 로그온 tooPagerDuty (Single Sign-on)와 Azure AD 계정 사용 하도록 설정할 수 있습니다.
...
Azure AD 및 PagerDuty 통합 tooconfigure 다음 항목 hello가 필요 합니다.
...
1. PagerDuty는 hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-pagerduty-from-hello-gallery"></a>PagerDuty는 hello 갤러리 추가
tooconfigure hello와의 통합 PagerDuty Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 PagerDuty tooadd가 필요합니다.

**hello 갤러리에서 PagerDuty tooadd hello 다음 단계를 수행 합니다.**
...
4. Hello 검색 상자에 입력 **PagerDuty**선택, **PagerDuty** 결과 패널에서 클릭 **추가** tooadd hello 응용 프로그램 단추입니다.
...
Single sign on toowork에 대 한 Azure AD는 tooknow PagerDuty에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자와 PagerDuty에 hello 관련된 사용자 간 링크 관계를 설정 하는 toobe가 필요 합니다.

PagerDuty에 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure와 PagerDuty 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.
...
3. **[PagerDuty 테스트 사용자 만들기](#create-a-pagerduty-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 PagerDuty에 해당 하는 도구입니다.
...
이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 PagerDuty 응용 프로그램에서 single sign on 구성 합니다.

**Azure AD tooconfigure single sign on와 PagerDuty를 hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **PagerDuty** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **PagerDuty 도메인 및 Url** 섹션를 hello 다음 단계를 수행 합니다.

    ![PagerDuty 도메인 및 URL Single Sign-On 정보](./media/active-directory-saas-pagerduty-tutorial/tutorial_pagerduty_url.png)

    a. Hello에 **로그온 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<tenant-name>.pagerduty.com`

    b. Hello에 **식별자** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<tenant-name>.pagerduty.com`

    > [!NOTE] 
    > 이러한 값은 실제 값이 아닙니다. 이러한 항목을 업데이트 로그온 URL과 식별자 실제 hello로 값입니다. 연락처 [PagerDuty 클라이언트 지원 팀](https://www.pagerduty.com/support/) tooget 이러한 값입니다. 
...
6. Hello에 **PagerDuty 구성** 섹션에서 클릭 **구성 PagerDuty** tooopen **sign on 구성** 창. 복사 hello **Sign-Out URL 및 SAML Single Sign-on 서비스 URL** hello에서 **빠른 참조 섹션.**
...
10. Hello에 **사용 Single sign-on (SSO)** 페이지 hello 다음 단계를 수행 합니다.
   
    ![Single Sign-On 사용](./media/active-directory-saas-pagerduty-tutorial/ic778537.png "Single Sign-On 사용")
   
    a. 메모장에서 콘텐츠를 클립보드에 복사 hello Azure 포털에서 다운로드 한 base 64로 인코딩된 인증서를 열고 다음 toohello **X.509 인증서** textbox
  
    b. Hello에 **로그인 URL** 붙여넣습니다 **SAML Single Sign-on 서비스 URL** Azure 포털에서 복사한입니다.
  
    c. Hello에 **로그 아웃 URL** 붙여넣습니다 **Sign-Out URL** Azure 포털에서 복사한입니다.
...
tooenable Azure AD 사용자가 toolog tooPagerDuty에서 프로 비전 해야 PagerDuty에 있습니다.  
Hello PagerDuty의 경우에서 프로 비전은 수동 작업입니다.

>[!NOTE]
>다른 Pagerduty 사용자 계정 만들기 도구를 사용할 수 있습니다 또는 사용자 계정을 Pagerduty tooprovision Azure Active Directory에서 제공 된 Api입니다.

**tooprovision 사용자 계정을 hello 다음 단계를 수행 합니다.**

1. Tooyour 로그인 **Pagerduty** 테 넌 트입니다.

2. Hello 메뉴에서 hello 위에 표시를 클릭 **사용자**합니다.

3. **사용자 추가**를 클릭합니다.
   
    ![사용자 추가](./media/active-directory-saas-pagerduty-tutorial/ic778539.png "사용자 추가")

4.  Hello에 **팀 초대** 대화 상자에서 hello 다음 단계를 수행 합니다.
   
    ![팀 초대](./media/active-directory-saas-pagerduty-tutorial/ic778540.png "팀 초대")

    a. 형식 hello **이름 및 성** 같은 사용자의 **Britta Simon**합니다. 
    ...
    >[!NOTE]
    >추가 된 모든 사용자는 초대 toocreate PagerDuty 계정을 받게 됩니다.

### <a name="assign-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당 합니다.

이 섹션에서는 tooPagerDuty 액세스 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![Hello 사용자 역할 할당][200]

**tooassign Britta Simon tooPagerDuty hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **PagerDuty**합니다.
...
클릭할 때 hello PagerDuty 타일 액세스 Panelyou hello에 자동으로 로그온 tooyour PagerDuty 응용 프로그램을 구해야 합니다.
