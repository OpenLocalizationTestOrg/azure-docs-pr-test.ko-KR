...
이 자습서에 설명 어떻게 toointegrate PurelyHR Azure Active directory (Azure AD).

다음 이점을 hello로 제공 PurelyHR Azure AD와 통합:

- 액세스 tooPurelyHR을 지닌 Azure AD에서 제어할 수 있습니다.
- 프로그램 사용자 tooautomatically get 로그온 tooPurelyHR (Single Sign-on)와 Azure AD 계정 사용 하도록 설정할 수 있습니다.
...
다음 항목 hello가 필요 tooconfigure PurelyHR와 Azure AD 통합 합니다.
...
1. PurelyHR은 hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-purelyhr-from-hello-gallery"></a>PurelyHR은 hello 갤러리 추가
tooconfigure hello와의 통합 PurelyHR Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 PurelyHR tooadd가 필요합니다.

**hello 갤러리에서 PurelyHR tooadd hello 다음 단계를 수행 합니다.**
...
Single sign on toowork에 대 한 Azure AD는 tooknow PurelyHR에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자 및 PurelyHR에 hello 관련된 사용자 간 링크 관계를 설정할 toobe가 필요 합니다.

PurelyHR에서 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure 및 PurelyHR 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.

1. **[Azure AD Single Sign-on 구성](#configuring-azure-ad-single-sign-on)**  -tooenable 사용자 toouse이이 기능입니다.
2. **[Azure AD 테스트 사용자 만들기](#creating-an-azure-ad-test-user)**  -tootest Azure AD single sign on Britta Simon 사용 합니다.
3. **[PurelyHR 테스트 사용자 만들기](#creating-a-purelyhr-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 PurelyHR에 해당 하는 도구입니다.
4. **[Azure AD hello 테스트 사용자를 할당](#assigning-the-azure-ad-test-user)**  -tooenable Britta Simon toouse Azure AD에서 single sign-on입니다.
5. **[Single Sign-on 테스트](#testing-single-sign-on)**  -tooverify 구성 works를 hello 여부.

### <a name="configuring-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성

이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 PurelyHR 응용 프로그램에서 single sign on 구성 합니다.

**tooconfigure Azure AD single sign on, PurelyHR와 hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **PurelyHR** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **PurelyHR 도메인 및 Url** 섹션를 hello tooconfigure hello 응용 프로그램에 필요한 경우 다음 단계를 수행 **IDP** 시작 모드:

    ![Single Sign-on 구성](./media/active-directory-saas-purelyhr-tutorial/tutorial_purelyhr_url.png)
   
    Hello에 **회신 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<companyID>.purelyhr.com/sso-consume`
...
4. 확인 **고급 URL 설정 표시**tooconfigure hello 응용 프로그램에 필요한 경우, **SP** 시작 모드:
    ...
    Hello에 **로그온 URL** hello 패턴을 사용 하 여 형식 hello 값 텍스트 상자:`https://<companyID>.purelyhr.com/sso-initiate`
     
    > [!NOTE]
    > 이러한 값은 실제 hello 되지 않습니다. Hello 실제 회신 URL 및 로그온 URL 사용 하 여 이러한 값을 업데이트 합니다. 연락처 [PurelyHR 클라이언트 지원 팀](http://support.purelyhr.com/) tooget 이러한 값입니다. 
...
7. Hello에 **PurelyHR 구성** 섹션에서 클릭 **구성 PurelyHR** tooopen **sign on 구성** 창. 복사 hello **SAML Single Sign-on 서비스 URL 및 엔터티 ID가 SAML** hello에서 **빠른 참조 섹션.**

    ![Single Sign-on 구성](./media/active-directory-saas-purelyhr-tutorial/tutorial_purelyhr_configure.png) 

8. tooconfigure single sign on에서 **PurelyHR** 쪽, 관리자 권한으로 로그인 tootheir 웹 사이트입니다.

9. 열기 hello **대시보드** hello 도구 모음 및 클릭 hello 옵션 중에서 **SSO 설정**합니다.

10. Hello 상자에 붙여넣기 hello 값 설명 아래-

    ![Single Sign-on 구성](./media/active-directory-saas-purelyhr-tutorial/purelyhr-dashboard-sso-settings.png)    

    a. 열기 hello **Certificate(Bas64)** 메모장 및 복사 hello 인증서 값에서 Azure 포털 hello에서 다운로드 합니다. 붙여넣기 hello hello에 값 복사 **X.509 인증서** 상자입니다.

    b. Hello에 **Idp 발급자 URL** 상자, 붙여넣기 hello **SAML 엔터티 ID** hello Azure 포털에서에서 복사 합니다.

    c. Hello에 **Idp 끝점 URL** 상자, 붙여넣기 hello **SAML Single Sign-on 서비스 URL** hello Azure 포털에서에서 복사 합니다. 

    d. Hello 확인 **사용자 자동 만들기** PurelyHR에서 tooenable 자동 사용자 프로비저닝을 확인란을 선택 합니다.

    e. 클릭 **변경 내용 저장** toosave hello 설정 합니다.
...
tooenable Azure AD 사용자가 toolog tooPurelyHR에서 이러한 해야에 프로 비전 PurelyHR 합니다. PurelyHR에서 프로비전은 자동 작업이고 자동 사용자 프로비전이 사용되면 수동 단계가 필요하지 않습니다.

### <a name="assigning-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당합니다.

이 섹션에서는 tooPurelyHR 액세스 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![사용자 할당][200] 

**tooassign Britta Simon tooPurelyHR hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **PurelyHR**합니다.
...
Hello 흡수 LMS hello 액세스 패널에서에서 타일을 클릭 합니다. 응용 프로그램을 자동으로 로그온 tooyour LMS 습득 합니다.

액세스 패널 hello에 대 한 자세한 내용은 다음을 참조 하세요. [액세스 패널 소개 toohello](https://msdn.microsoft.com/library/dn308586)합니다.
