...
이 자습서에 설명 어떻게 toointegrate TargetProcess Azure Active directory (Azure AD).

다음 이점을 hello로 제공 TargetProcess Azure AD와 통합:

- 액세스 tooTargetProcess을 지닌 Azure AD에서 제어할 수 있습니다.
- 프로그램 사용자 tooautomatically get 로그온 tooTargetProcess (Single Sign-on)와 Azure AD 계정 사용 하도록 설정할 수 있습니다.
...
다음 항목 hello가 필요 tooconfigure TargetProcess와 Azure AD 통합 합니다.
...
1. TargetProcess hello 갤러리 추가
2. Azure AD Single Sign-On 구성 및 테스트

## <a name="add-targetprocess-from-hello-gallery"></a>TargetProcess hello 갤러리 추가
tooconfigure hello와의 통합 TargetProcess Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 TargetProcess tooadd가 필요합니다.

**hello 갤러리에서 TargetProcess tooadd hello 다음 단계를 수행 합니다.**
...
4. Hello 검색 상자에 입력 **TargetProcess**선택, **TargetProcess** 결과 패널에서 클릭 **추가** tooadd hello 응용 프로그램 단추입니다.
...
Single sign on toowork에 대 한 Azure AD는 tooknow TargetProcess에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자 및 TargetProcess에 hello 관련된 사용자 간 링크 관계를 설정할 toobe가 필요 합니다.

TargetProcess에서 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure 및 TargetProcess 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.
...
3. **[TargetProcess 테스트 사용자 만들기](#create-a-targetprocess-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 TargetProcess에 해당 하는 도구입니다.
...
이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 TargetProcess 응용 프로그램에서 single sign on 구성 합니다.

**tooconfigure Azure AD single sign on, TargetProcess와 hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **TargetProcess** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **TargetProcess 도메인 및 Url** 섹션를 hello 다음 단계를 수행 합니다.

    ![TargetProcess 도메인 및 URL 섹션](./media/active-directory-saas-target-process-tutorial/tutorial_target-process_url.png)

    a. Hello에 **로그온 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<subdomain>.tpondemand.com/`

    b. Hello에 **식별자** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<subdomain>.tpondemand.com/`

    > [!NOTE] 
    > 이러한 값은 실제 값이 아닙니다. 이러한 항목을 업데이트 로그온 URL과 식별자 실제 hello로 값입니다. 연락처 [TargetProcess 클라이언트 지원 팀](mailto:support@targetprocess.com) tooget 이러한 값입니다. 
...
6. Hello에 **TargetProcess 구성** 섹션에서 클릭 **구성 TargetProcess** tooopen **sign on 구성** 창. 복사 hello **SAML Single Sign-on 서비스 URL** hello에서 **빠른 참조 섹션.**

    ![TargetProcess 구성 섹션](./media/active-directory-saas-target-process-tutorial/tutorial_target-process_configure.png) 

7. 로그온 tooyour 관리자 권한으로 TargetProcess 응용 프로그램입니다.

8. Hello 메뉴에서 hello 위에 표시를 클릭 **설치**합니다.
...
11. Hello Single Sign on 설정 대화 상자에서 hello 다음 단계를 수행 합니다.
    ...
    c. 콘텐츠 복사 hello 메모장에서 다운로드 한 인증서를 열고 hello에 붙여 넣습니다 **인증서** 텍스트 상자에 붙여넣습니다.
...
hello이이 섹션의 목적은 toocreate Britta Simon TargetProcess에서 호출 하는 사용자입니다.
...
이 섹션에서는 tooTargetProcess 액세스 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![사용자 할당][200] 

**tooassign Britta Simon tooTargetProcess hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **TargetProcess**합니다.
...
Hello TargetProcess hello 액세스 패널에서에서 타일을 클릭할 때 자동으로 로그온 tooyour TargetProcess 응용 프로그램을 구해야 합니다. 액세스 패널 hello에 대 한 자세한 내용은 참조 [액세스 패널 소개 toohello](active-directory-saas-access-panel-introduction.md)합니다.
