...
이 자습서에 설명 어떻게 toointegrate Azure Active Directory (Azure AD)와 이미지 릴레이 합니다.

다음 이점을 hello로 제공 이미지 릴레이 Azure AD와 통합:

- Azure ad 액세스 tooImage 릴레이 가진 제어할 수 있습니다.
- 프로그램 사용자 tooautomatically get 로그온 tooImage 릴레이 (Single Sign-on)와 Azure AD 계정 사용 하도록 설정할 수 있습니다.
...
이미지 릴레이와 Azure AD 통합 tooconfigure 다음 항목 hello가 필요 합니다.
...
1. Hello 갤러리에서 이미지 릴레이 추가합니다.
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-image-relay-from-hello-gallery"></a>Hello 갤러리에서 이미지 릴레이 추가합니다.
tooconfigure hello와의 통합 이미지 릴레이 Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 이미지 릴레이 tooadd가 필요합니다.

**hello 갤러리에서 이미지 릴레이 tooadd hello 다음 단계를 수행 합니다.**
...
Single sign on toowork에 대 한 Azure AD는 tooknow 이미지 릴레이에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자 및 이미지 릴레이에 hello 관련된 사용자 간 링크 관계를 설정 하는 toobe가 필요 합니다.

이미지 릴레이에 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure 및 이미지 릴레이 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.

1. **[Azure AD Single Sign-on 구성](#configuring-azure-ad-single-sign-on)**  -tooenable 사용자 toouse이이 기능입니다.
2. **[Azure AD 테스트 사용자 만들기](#creating-an-azure-ad-test-user)**  -tootest Azure AD single sign on Britta Simon 사용 합니다.
3. **[이미지 릴레이 테스트 사용자 만들기](#creating-an-image-relay-test-user)**  -toohave Britta Simon 표현인 연결 된 toohello Azure AD 사용자의 이미지 릴레이에 해당 하는 도구입니다.
4. **[Azure AD hello 테스트 사용자를 할당](#assigning-the-azure-ad-test-user)**  -tooenable Britta Simon toouse Azure AD에서 single sign-on입니다.
5. **[Single Sign-on 테스트](#testing-single-sign-on)**  -tooverify 구성 works를 hello 여부.

### <a name="configuring-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성

이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 이미지 릴레이 응용 프로그램에서 single sign on 구성 합니다.

**Azure AD tooconfigure single sign on 이미지 릴레이와 hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **이미지 릴레이** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **이미지 릴레이 도메인 및 Url** 섹션를 hello 다음 단계를 수행 합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-imagerelay-tutorial/tutorial_imagerelay_url.png)

    a. Hello에 **로그온 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<companyname>.imagerelay.com/`

    b. Hello에 **식별자** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<companyname>.imagerelay.com/sso/metadata`

    > [!NOTE] 
    > 이러한 값은 실제 값이 아닙니다. 이러한 항목을 업데이트 로그온 URL과 식별자 실제 hello로 값입니다. 연락처 [이미지 릴레이 클라이언트 지원 팀](http://support.imagerelay.com/) tooget 이러한 값입니다. 
...
6. Hello에 **이미지 릴레이 구성** 섹션에서 클릭 **구성 이미지 릴레이** tooopen **sign on 구성** 창. 복사 hello **Sign-Out 서비스 URL 및 SAML Single Sign-on 서비스 URL** hello에서 **빠른 참조 섹션.**

    ![Single Sign-on 구성](./media/active-directory-saas-imagerelay-tutorial/tutorial_imagerelay_configure.png) 

7. 다른 브라우저 창에서 관리자 권한으로 tooyour 이미지 릴레이 회사 사이트에 로그인 합니다.

8. 도구 모음의 hello hello 위쪽에 클릭 hello **사용자 및 사용 권한** 작업 합니다.
...
10. Hello에 **Single Sign-on 설정** , 선택 hello **만 통해 로그인 Single Sign On이 그룹 수** 확인란을 선택한 다음 클릭 **저장**합니다.
...
12. Toohello 이동 **Single Sign-on 설정** 작업 합니다.
    
     ![Single Sign-on 구성](./media/active-directory-saas-imagerelay-tutorial/tutorial_imagerelay_11.png)

13. Hello에 **SAML 설정** 대화 상자에서 hello 다음 단계를 수행 합니다.
    
    ![Single Sign-on 구성](./media/active-directory-saas-imagerelay-tutorial/tutorial_imagerelay_12.png)
    
    a. **로그인 URL** 붙여넣기 hello 값의 텍스트 상자 **Single Sign-on 서비스 URL** Azure 포털에서 복사한입니다.

    b. **로그 아웃 URL** 붙여넣기 hello 값의 텍스트 상자 **Single Sign-Out 서비스 URL** Azure 포털에서 복사한입니다.

    c. **이름 ID 형식**으로 **urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress**를 선택합니다.

    d. 으로 **hello 서비스 공급자 (이미지 릴레이)의 요청에 대 한 바인딩 옵션**선택, **POST 바인딩**합니다.
    ...
    f. 메모장에서 다운로드 한 hello 인증서 열고 hello 콘텐츠 복사 hello x.509 인증서 텍스트 상자에 붙여 넣습니다.

    ![Single Sign-on 구성](./media/active-directory-saas-imagerelay-tutorial/tutorial_imagerelay_18.png)

    g. **Just-In-Time 사용자 프로 비전** 섹션을 선택 하는 hello **Just-In-Time 사용자 프로비저닝 사용**합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-imagerelay-tutorial/tutorial_imagerelay_19.png)

    h. 선택 hello 권한 그룹 (예를 들어 **SSO 기본**)를 통한 single sign on에서 toosign ´ ù.
...
hello이이 섹션의 목적은 toocreate Britta Simon 이미지 릴레이에서 호출 하는 사용자입니다.

**toocreate Britta Simon 이미지 릴레이의 라는 사용자를 만들면 hello 다음 단계를 수행 합니다.**

1. 이미지 릴레이 회사 사이트에 관리자로 로그온 tooyour 합니다.

2. 너무 이동**사용자 및 사용 권한** 선택 **SSO 사용자 만들기**합니다.
   
    ![Single Sign-on 구성](./media/active-directory-saas-imagerelay-tutorial/tutorial_imagerelay_21.png) 

3. Hello 입력 **전자 메일**, **이름**, **성**, 및 **회사** tooprovision 및 선택 hello 권한 그룹 원하는 hello 사용자의 (예를 들어 SSO 기본) 하는 hello 그룹 통한 single sign-on 로그인 할 수입니다.
...
이 섹션에서는 액세스 tooImage 릴레이 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![사용자 할당][200] 

**tooassign Britta Simon tooImage 릴레이 hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **이미지 릴레이**합니다.
...
Hello 액세스 패널에서에서 hello 이미지 릴레이 타일을 클릭할 때 자동으로 로그온 tooyour 이미지 릴레이 응용 프로그램을 구해야 합니다.
