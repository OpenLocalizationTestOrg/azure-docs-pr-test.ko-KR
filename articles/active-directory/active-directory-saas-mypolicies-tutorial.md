...
이 자습서에 설명 어떻게 toointegrate myPolicies Azure Active directory (Azure AD).

다음 이점을 hello로 제공 myPolicies Azure AD와 통합:

- 액세스 toomyPolicies을 지닌 Azure AD에서 제어할 수 있습니다.
- 프로그램 사용자 tooautomatically get 로그온 toomyPolicies (Single Sign-on)와 Azure AD 계정 사용 하도록 설정할 수 있습니다.
...
다음 항목 hello가 필요 tooconfigure myPolicies와 Azure AD 통합 합니다.
...
1. MyPolicies hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-mypolicies-from-hello-gallery"></a>MyPolicies hello 갤러리 추가
tooconfigure hello와의 통합 myPolicies Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 tooadd myPolicies 필요합니다.

**hello 갤러리에서 tooadd myPolicies hello 다음 단계를 수행 합니다.**
...
Single sign on toowork에 대 한 Azure AD는 tooknow myPolicies에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자 및 myPolicies에 hello 관련된 사용자 간 링크 관계를 설정할 toobe가 필요 합니다.

MyPolicies에서 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure 및 myPolicies 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.

1. **[Azure AD Single Sign-on 구성](#configuring-azure-ad-single-sign-on) ** -tooenable 사용자 toouse이이 기능입니다.
2. **[Azure AD 테스트 사용자 만들기](#creating-an-azure-ad-test-user) ** -tootest Azure AD single sign on Britta Simon 사용 합니다.
3. **[MyPolicies 테스트 사용자 만들기](#creating-a-mypolicies-test-user) ** -toohave에서 사용자의 연결 된 Azure AD toohello 표현인 myPolicies Britta Simon 해당 하는 도구입니다.
4. **[Azure AD hello 테스트 사용자를 할당](#assigning-the-azure-ad-test-user) ** -tooenable Britta Simon toouse Azure AD에서 single sign-on입니다.
5. **[Single Sign-on 테스트](#testing-single-sign-on) ** -tooverify 구성 works를 hello 여부.

### <a name="configuring-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성

이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 myPolicies 응용 프로그램에서 single sign on 구성 합니다.

**tooconfigure Azure AD single sign on, myPolicies와 hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **myPolicies** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **도메인 및 Url myPolicies** 섹션를 hello 다음 단계를 수행 합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-mypolicies-tutorial/tutorial_mypolicies_url.png)

    a. Hello에 **식별자** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<tenantname>.mypolicies.com/`

    b. Hello에 **회신 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<tenantname>.mypolicies.com/users/auth/saml/callback`

    > [!NOTE] 
    > 이러한 값은 실제 값이 아닙니다. Hello 실제 식별자 및 회신 URL로이 값을 업데이트 합니다. 연락처 [myPolicies 지원 팀](mailto:support@mypolicies.com) tooget 이러한 값입니다.

4. hello myPolicies 응용 프로그램 구성을 수행 해야 하면 tooadd 사용자 지정 특성 매핑을 tooyour SAML 토큰 특성을 특정 형식에서 hello SAML 어설션이 필요 합니다. 이 응용 프로그램에 대 한 클레임을 따라 hello를 구성 합니다. Hello에서 이러한 특성의 hello 값을 관리할 수 있습니다 "**사용자 특성**" 응용 프로그램 통합 페이지에서 섹션. 다음 스크린 샷 hello이에 대 한 예가 나와 있습니다. 

    ![Single Sign-on 구성](./media/active-directory-saas-mypolicies-tutorial/tutorial_mypolicies_attribute.png)

5. 클릭 **보기 및 다른 모든 사용자 특성 편집** hello 확인란을 선택 **사용자 특성** 섹션 tooexpand hello 특성입니다. Hello hello 표시 특성-각에서 다음 단계를 수행 합니다.
    ...
    a. Hello 특성 tooopen hello 클릭 **특성 편집** 대화 상자.
    
    ![Single Sign-on 구성](./media/active-directory-saas-mypolicies-tutorial/tutorial_attribute_05.png)
    
    b. Hello에서 hello URL 값을 삭제 **Namespace**합니다.
    
    c. 클릭 **확인** toosave hello 설정 합니다.
    
6. Hello에 **SAML 서명 인증서** 섹션에서 클릭 **Certificate(Base64)** hello 인증서 파일을 컴퓨터에 저장 합니다.
...
8. Hello에 **myPolicies 구성** 섹션에서 클릭 **myPolicies 구성** tooopen **sign on 구성** 창. 복사 hello **SAML Single Sign-on 서비스 URL** hello에서 **빠른 참조 섹션.**

    ![Single Sign-on 구성](./media/active-directory-saas-mypolicies-tutorial/tutorial_mypolicies_configure.png) 

9. tooconfigure single sign on에서 **myPolicies** toosend hello 다운로드 해야 쪽에서는 **Certificate(Base64)** 및 **SAML Single Sign-on 서비스 URL** 너무[myPolicies 지원 팀](mailto:support@mypolicies.com)합니다. 
...
이 섹션에서는 myPolicies에서 Britta Simon이라는 사용자를 만듭니다. 작업할 [myPolicies 지원 팀](mailto:support@mypolicies.com) hello 사용자 hello myPolicies 플랫폼을 추가할 수 있습니다. Single Sign-On을 사용하려면 먼저 사용자를 만들고 활성화해야 합니다.

### <a name="assigning-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당합니다.

이 섹션에서는 toomyPolicies 액세스 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![사용자 할당][200] 

**tooassign Britta Simon toomyPolicies hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **myPolicies**합니다.
...
Hello myPolicies hello 액세스 패널에서에서 타일을 클릭할 때 자동으로 로그온 tooyour myPolicies 응용 프로그램을 구해야 합니다.
