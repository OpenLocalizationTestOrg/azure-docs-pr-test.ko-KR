...
이 자습서에 설명 Azure Active Directory (Azure AD)와 toointegrate 캔버스 하는 방법입니다.

Azure AD와 Canvas를 통합 이점을 다음 hello 제공:

- 액세스 tooCanvas을 지닌 Azure AD에서 제어할 수 있습니다.
- 프로그램 사용자 tooautomatically get 로그온 tooCanvas (Single Sign-on)와 Azure AD 계정 사용 하도록 설정할 수 있습니다.
...
캔버스와 Azure AD 통합 tooconfigure 다음 항목 hello가 필요 합니다.
...
## <a name="adding-canvas-from-hello-gallery"></a>캔버스는 hello 갤러리 추가
tooconfigure hello 통합 캔버스의 Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 캔버스 tooadd가 필요합니다.

**hello 갤러리에서 캔버스 tooadd hello 다음 단계를 수행 합니다.**
...
Single sign on toowork에 대 한 Azure AD는 tooknow 캔버스에 있는 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자 및 hello 캔버스에 있는 관련된 사용자 간 링크 관계를 설정 하는 toobe가 필요 합니다.

캔버스의 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure 및 캔버스를 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.

1. **[Azure AD Single Sign-on 구성](#configuring-azure-ad-single-sign-on)**  -tooenable 사용자 toouse이이 기능입니다.
2. **[Azure AD 테스트 사용자 만들기](#creating-an-azure-ad-test-user)**  -tootest Azure AD single sign on Britta Simon 사용 합니다.
3. **[캔버스 테스트 사용자 만들기](#creating-a-canvas-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 캔버스에 해당 하는 도구입니다.
4. **[Azure AD hello 테스트 사용자를 할당](#assigning-the-azure-ad-test-user)**  -tooenable Britta Simon toouse Azure AD에서 single sign-on입니다.
5. **[Single Sign-on 테스트](#testing-single-sign-on)**  -tooverify 구성 works를 hello 여부.

### <a name="configuring-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성

이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 캔버스 응용 프로그램에서 single sign on 구성 합니다.

**tooconfigure Azure AD single sign on 캔버스와 hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **캔버스** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **캔버스 도메인 및 Url** 섹션를 hello 다음 단계를 수행 합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-canvas-lms-tutorial/tutorial_canvaslms_url.png)

    a. Hello에 **로그온 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<tenant-name>.instructure.com`

    b. Hello에 **식별자** hello 패턴을 사용 하 여 형식 hello 값 텍스트 상자:`https://<tenant-name>.instructure.com/saml2`

    > [!NOTE] 
    > 이러한 값은 실제 값이 아닙니다. 이러한 항목을 업데이트 로그온 URL과 식별자 실제 hello로 값입니다. 연락처 [캔버스 클라이언트 지원 팀](https://community.canvaslms.com/community/help) tooget 이러한 값입니다. 
...
6. Hello에 **캔버스 구성** 섹션에서 클릭 **구성 캔버스** tooopen **sign on 구성** 창. 복사 hello **암호 변경 URL, 로그 아웃 URL, SAML 엔터티 ID, 및 SAML Single Sign-on 서비스 URL** hello에서 **빠른 참조 섹션.**

    ![Single Sign-on 구성](./media/active-directory-saas-canvas-lms-tutorial/tutorial_canvaslms_configure.png) 
 
7. 다른 웹 브라우저 창에서 관리자 권한으로 tooyour Canvas 회사 사이트에 로그인 합니다.

8. 너무 이동**Courses \> 관리 계정 \> Microsoft**합니다.
   
    ![Canvas](./media/active-directory-saas-canvas-lms-tutorial/IC775990.png "Canvas")

9. Hello hello 왼쪽의 탐색 창에서 선택 **인증**, 클릭 하 고 **새 SAML 구성 추가**합니다.
   
    ![인증](./media/active-directory-saas-canvas-lms-tutorial/IC775991.png "인증")

10. Hello 현재 통합 페이지에서 다음 단계는 hello를 수행 합니다.
   
    ![현재 통합](./media/active-directory-saas-canvas-lms-tutorial/IC775992.png "현재 통합")

    a. **IdP 엔터티 ID** 붙여넣기 hello 값의 텍스트 상자 **SAML 엔터티 ID** Azure 포털에서 복사한입니다.

    b. **로그온 URL** 붙여넣기 hello 값의 텍스트 상자 **SAML Single Sign-on 서비스 URL** Azure 포털에서 복사한입니다.

    c. **로그 아웃 URL** 붙여넣기 hello 값의 텍스트 상자 **Sign-Out URL** Azure 포털에서 복사한입니다.

    d. **암호 변경 링크** 붙여넣기 hello 값의 텍스트 상자 **암호 변경 URL** Azure 포털에서 복사한입니다. 

    e. **인증서 지문** 텍스트 붙여넣기 hello **지문** Azure 포털에서 복사 하는 인증서의 값입니다.      
        
    f. Hello에서 **로그인 특성** 목록에서 **NameID**합니다.

    g. Hello에서 **식별자 형식** 목록에서 **emailAddress**합니다.
...
tooenable Azure AD 사용자가 toolog tooCanvas에서 이러한 해야에 프로 비전 캔버스입니다.

Canvas의 경우 사용자 프로비전은 수동 작업입니다.

**tooprovision 사용자 계정을 hello 다음 단계를 수행 합니다.**

1. Tooyour 로그인 **캔버스** 테 넌 트입니다.

2. 너무 이동**Courses \> 관리 계정 \> Microsoft**합니다.
...
5. 새 사용자 대화 상자 페이지 추가 hello hello 다음 단계를 수행 합니다.
   
   ![사용자 추가](./media/active-directory-saas-canvas-lms-tutorial/IC775997.png "사용자 추가")
   
   a. Hello에 **전체 이름을** 텍스트 상자와 같은 사용자의 hello 이름을 입력 **BrittaSimon**합니다.

   b. Hello에 **전자 메일** 텍스트 상자와 같은 사용자의 전자 메일을 hello 입력  **brittasimon@contoso.com** 합니다.

   c. Hello에 **로그인** 텍스트 상자 같은 hello 사용자의 Azure AD 메일 주소를 입력  **brittasimon@contoso.com** 합니다.

   d. 선택 **이 계정 만들기에 대 한 전자 메일 hello 사용자**합니다.

   e. **사용자 추가**를 클릭합니다.

>[!NOTE]
>다른 Canvas 사용자 계정 만들기 도구를 사용할 수 있습니다 또는 AAD 사용자 계정을 캔버스 tooprovision에서 제공 된 Api입니다.

### <a name="assigning-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당합니다.

이 섹션에서는 tooCanvas 액세스 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![사용자 할당][200] 

**tooassign Britta Simon tooCanvas hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **캔버스**합니다.
...
Hello 액세스 패널에서에서 hello 캔버스 타일을 클릭할 때 자동으로 로그온 tooyour 캔버스 응용 프로그램을 구해야 합니다.
