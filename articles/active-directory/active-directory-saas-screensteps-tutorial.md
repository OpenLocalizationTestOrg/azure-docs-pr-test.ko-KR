...
이 자습서에 설명 어떻게 toointegrate ScreenSteps와 Azure Active Directory (Azure AD).

Azure AD와 ScreenSteps 통합 이점을 다음 hello로 제공 합니다.

- 액세스 tooScreenSteps을 지닌 Azure AD에서 제어할 수 있습니다.
- 에 사용자가 tooautomatically get 로그온 tooScreenSteps (Single Sign-on)는 Azure AD 계정으로 설정할 수 있습니다.
...
ScreenSteps와 Azure AD 통합 tooconfigure 다음 항목 hello가 필요 합니다.
...
1. ScreenSteps는 hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-screensteps-from-hello-gallery"></a>ScreenSteps는 hello 갤러리 추가
tooconfigure hello와의 통합 ScreenSteps Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 ScreenSteps tooadd가 필요합니다.

**hello 갤러리에서 ScreenSteps tooadd hello 다음 단계를 수행 합니다.**
...
4. Hello 검색 상자에 입력 **ScreenSteps**선택, **ScreenSteps** 결과 패널에서 클릭 **추가** tooadd hello 응용 프로그램 단추입니다.
...
Single sign on toowork에 대 한 Azure AD는 tooknow ScreenSteps에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자와 ScreenSteps에 hello 관련된 사용자 간 링크 관계를 설정 하는 toobe가 필요 합니다.

Hello hello 값을 할당, ScreenSteps에서 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure와 ScreenSteps와 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.
...
3. **[ScreenSteps 테스트 사용자 만들기](#create-a-screensteps-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 ScreenSteps에 해당 하는 도구입니다.
...
이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 ScreenSteps 응용 프로그램에서 single sign on 구성 합니다.

**Azure AD tooconfigure single sign on와 ScreenSteps를 hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **ScreenSteps** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **ScreenSteps 도메인 및 Url** 섹션를 hello 다음 단계를 수행 합니다.

    ![ScreenSteps 도메인 및 URL Single Sign-On 정보](./media/active-directory-saas-screensteps-tutorial/tutorial_screensteps_url.png)

    Hello에 **로그온 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<tenantname>.ScreenSteps.com`

    > [!NOTE] 
    > 이 값은 실제 값이 아닙니다. Hello로이 값을 업데이트 합니다. 실제 로그온 URL,이 자습서의 뒷부분에서 설명 합니다. 
...
6. Hello에 **ScreenSteps 구성** 섹션에서 클릭 **구성 ScreenSteps** tooopen **sign on 구성** 창. 복사 hello **Sign-Out URL 및 SAML Single Sign-on 서비스 URL** hello에서 **빠른 참조 섹션.**
...
11. Hello에 **만들 Single sign-on 끝점** 섹션를 hello 다음 단계를 수행 합니다.

    ![인증 끝점 만들기](./media/active-directory-saas-screensteps-tutorial/ic778526.png "인증 끝점 만들기")
    
    a. Hello에 **제목** 텍스트 상자에 제목 입력 합니다.
    
    b. Hello에서 **모드** 목록에서 **SAML**합니다.
...
13. Hello에 **편집 Single sign-on 끝점** 섹션를 hello 다음 단계를 수행 합니다.

    ![원격 인증 끝점](./media/active-directory-saas-screensteps-tutorial/ic778527.png "원격 인증 끝점")

    a. 클릭 **새 SAML 인증서 파일 업로드**, 한 다음 업로드 hello 인증서를 Azure 포털에서 다운로드 합니다.
    
    b. 붙여넣기 **SAML Single Sign-on 서비스 URL** hello에 hello Azure 포털에서에서 복사한 값 **원격 로그인 URL** 텍스트 상자에 붙여넣습니다.
    
    c. 붙여넣기 **Sign-Out URL** hello에 hello Azure 포털에서에서 복사한 값 **로그 아웃 URL** 텍스트 상자에 붙여넣습니다.
    
    d. 선택 된 **그룹** tooassign 사용자 toowhen 프로 비전 됩니다.
    
    e. **업데이트**를 클릭합니다.

    f. 복사 hello **SAML 소비자 URL** toohello 클립보드 toohello에 붙여 **로그온 URL** 텍스트 상자로 **ScreenSteps 도메인 및 Url** 섹션.
    
    g. Toohello 반환 **편집 Single sign-on 끝점**합니다.
    
    h. Hello 클릭 **계정에 대 한 기본 서버로 만들기** 단추 toouse이이 끝점에 대 한 모든 사용자에 게 ScreenSteps에 로그인 합니다. Hello 클릭 해도 **tooSite 추가** toouse이이 끝점에서 특정 사이트에 대 한 단추 **ScreenSteps**합니다.
...
이 섹션에서는 ScreenSteps에서 Britta Simon이라는 사용자를 만듭니다. 작업할 [ScreenSteps 클라이언트 지원 팀](https://www.screensteps.com/contact) hello 사용자 hello ScreenSteps 플랫폼을 추가할 수 있습니다. Single Sign-On을 사용하려면 먼저 사용자를 만들고 활성화해야 합니다. 

### <a name="assign-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당 합니다.

이 섹션에서는 tooScreenSteps 액세스 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![Hello 사용자 역할 할당][200] 

**tooassign Britta Simon tooScreenSteps hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **ScreenSteps**합니다.
...
Hello ScreenSteps hello 액세스 패널에서에서 타일을 클릭할 때 자동으로 로그온 tooyour ScreenSteps 응용 프로그램을 구해야 합니다.
