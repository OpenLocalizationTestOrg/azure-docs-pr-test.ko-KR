...
이 자습서에 설명 어떻게 toointegrate Zscaler와 Azure Active Directory (Azure AD).

Azure AD와 Zscaler 통합 hello 다음 이점을 제공 합니다.

- 액세스 tooZscaler을 지닌 Azure AD에서 제어할 수 있습니다.
- 프로그램 사용자 tooautomatically get 로그온 tooZscaler (Single Sign-on)와 Azure AD 계정 사용 하도록 설정할 수 있습니다.
...
Zscaler와 Azure AD 통합 tooconfigure 다음 항목 hello가 필요 합니다.
...
1. Zscaler는 hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-zscaler-from-hello-gallery"></a>Zscaler는 hello 갤러리 추가
tooconfigure hello와의 통합 Zscaler Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 Zscaler tooadd가 필요합니다.

**Zscaler hello 갤러리에서 tooadd hello 다음 단계를 수행 합니다.**
...
Single sign on toowork에 대 한 Azure AD는 tooknow Zscaler에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자 및 Zscaler의 hello 관련된 사용자 간 링크 관계를 설정 하는 toobe가 필요 합니다.

Zscaler에서 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure 및 Zscaler 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.
...
4. **[Zscaler 테스트 사용자 만들기](#creating-a-zscaler-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 Zscaler에 해당 하는 도구입니다.
...
이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 Zscaler 응용 프로그램에서 single sign on 구성 합니다.

**tooconfigure Azure AD single sign on와 Zscaler, hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **Zscaler** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **Zscaler 도메인 및 Url** 섹션를 hello 다음 단계를 수행 합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-zscaler-tutorial/tutorial_zscaler_url.png)

    Hello에 **로그온 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<companyname>.zsclaer.net`

    > [!NOTE] 
    > 이 값은 실제 값이 아닙니다. Hello로이 값을 업데이트 합니다. 실제 로그온 URL입니다. 연락처 [Zscaler 클라이언트 지원 팀](https://www.zscaler.com/company/contact) tooget이이 값입니다. 
...
6. Hello에 **Zscaler 구성** 섹션에서 클릭 **Zscaler 구성** tooopen **sign on 구성** 창. 복사 hello **SAML Single Sign-on 서비스 URL** hello에서 **빠른 참조 섹션.**

    ![Single Sign-on 구성](./media/active-directory-saas-zscaler-tutorial/tutorial_zscaler_configure.png) 

7. 다른 웹 브라우저 창에서 관리자 권한으로 tooyour ZScaler 회사 사이트에 로그인 합니다.
...
5. Hello 모드 해제 프록시 서버 섹션에서에서 단계를 수행 하는 hello를 수행 합니다.   
    ...
    b. Hello 주소 텍스트 상자에 입력 **gateway.zscaler.net**합니다.
...
tooenable Azure AD 사용자가 toolog tooZScaler에 프로 비전 된 tooZScaler 이어야 합니다.  
Hello ZScaler의 경우에서 프로 비전은 수동 작업입니다.
...
> [!NOTE]
> 다른 ZScaler 사용자 계정 만들기 도구를 사용할 수 있습니다 또는 AAD 사용자 계정을 tooprovision ZScaler에서 제공 된 Api입니다.

### <a name="assigning-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당합니다.

이 섹션에서는 tooZscaler 액세스 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![사용자 할당][200] 

**tooassign Britta Simon tooZscaler hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **Zscaler**합니다.
...
Hello 액세스 패널에서에서 hello Zscaler 타일을 클릭할 때 자동으로 로그온 tooyour Zscaler 응용 프로그램을 구해야 합니다.
