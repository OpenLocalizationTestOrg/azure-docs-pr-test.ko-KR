...
이 자습서에 설명 어떻게 toointegrate Synergi Azure Active directory (Azure AD).

다음 이점을 hello로 제공 Synergi Azure AD와 통합:

- 액세스 tooSynergi을 지닌 Azure AD에서 제어할 수 있습니다.
- 에 사용자가 tooautomatically get 로그온 tooSynergi (Single Sign-on)는 Azure AD 계정으로 설정할 수 있습니다.
...
다음 항목 hello가 필요 tooconfigure Synergi와 Azure AD 통합 합니다.
...
1. Synergi는 hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-synergi-from-hello-gallery"></a>Synergi는 hello 갤러리 추가
tooconfigure hello와의 통합 Synergi Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 Synergi tooadd가 필요합니다.

**hello 갤러리에서 Synergi tooadd hello 다음 단계를 수행 합니다.**
...
4. Hello 검색 상자에 입력 **Synergi**선택, **Synergi** 결과 패널에서 클릭 **추가** tooadd hello 응용 프로그램 단추입니다.
...
Single sign on toowork에 대 한 Azure AD는 tooknow Synergi에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자 및 Synergi에 hello 관련된 사용자 간 링크 관계를 설정할 toobe가 필요 합니다.

Synergi에서 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure 및 Synergi 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.
...
3. **[Synergi 테스트 사용자 만들기](#create-a-synergi-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 Synergi에 해당 하는 도구입니다.
...
이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 Synergi 응용 프로그램에서 single sign on 구성 합니다.

**tooconfigure Azure AD single sign on, Synergi와 hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **Synergi** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **Synergi 도메인 및 Url** 섹션를 hello 다음 단계를 수행 합니다.

    ![Synergi 도메인 및 URL Single Sign-On 정보](./media/active-directory-saas-synergi-tutorial/tutorial_synergi_url.png)

    a. Hello에 **식별자** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<company name>.irmsecurity.com`

    b. Hello에 **회신 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<company name>.irmsecurity.com/sso/<organization id>`

    > [!NOTE] 
    > 이러한 값은 실제 값이 아닙니다. Hello 실제 식별자 및 회신 URL로이 값을 업데이트 합니다. 연락처 [Synergi 지원 팀](https://www.irmsecurity.com/contact/) tooget 이러한 값입니다.
...
6. Hello에 **Synergi 구성** 섹션에서 클릭 **구성 Synergi** tooopen **sign on 구성** 창. 복사 hello **Sign-Out URL 및 엔터티 ID가 SAML** hello에서 **빠른 참조 섹션.**

    ![Synergi 구성](./media/active-directory-saas-synergi-tutorial/tutorial_synergi_configure.png) 

7. tooconfigure single sign on에서 **Synergi** toosend hello 다운로드 해야 쪽에서는 **Certificate(Base64), 로그 아웃 URL 및 엔터티 ID가 SAML** 너무[Synergi 지원 팀](https://www.irmsecurity.com/contact/)합니다.
...
이 섹션에서는 Synergi에서 Britta Simon이라는 사용자를 만듭니다. 작업할 [Synergi 지원 팀](https://www.irmsecurity.com/contact/) hello 사용자 hello Synergi 플랫폼을 추가할 수 있습니다. Single Sign-On을 사용하려면 먼저 사용자를 만들고 활성화해야 합니다.

### <a name="assign-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당 합니다.

이 섹션에서는 tooSynergi 액세스 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![Hello 사용자 역할 할당][200] 

**tooassign Britta Simon tooSynergi hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **Synergi**합니다.
...
Hello 액세스 패널에서에서 hello Synergi 타일을 클릭할 때 자동으로 로그온 tooyour Synergi 응용 프로그램을 구해야 합니다.
