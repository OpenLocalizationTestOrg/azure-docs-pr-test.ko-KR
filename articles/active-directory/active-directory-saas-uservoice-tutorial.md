...
이 자습서에 설명 어떻게 toointegrate UserVoice Azure Active directory (Azure AD).

Azure AD와 UserVoice 통합 이점을 다음 hello로 제공 합니다.

- 액세스 tooUserVoice을 지닌 Azure AD에서 제어할 수 있습니다.
- 에 사용자가 tooautomatically get 로그온 tooUserVoice (Single Sign-on)는 Azure AD 계정으로 설정할 수 있습니다.
...
UserVoice와 Azure AD 통합 tooconfigure 다음 항목 hello가 필요 합니다.
...
1. UserVoice는 hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-uservoice-from-hello-gallery"></a>UserVoice는 hello 갤러리 추가
tooconfigure hello와의 통합 UserVoice Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 UserVoice tooadd가 필요합니다.

**hello 갤러리에서 UserVoice tooadd hello 다음 단계를 수행 합니다.**
...
4. Hello 검색 상자에 입력 **UserVoice**선택, **UserVoice** 결과 패널에서 클릭 **추가** tooadd hello 응용 프로그램 단추입니다.
...
Single sign on toowork에 대 한 Azure AD는 tooknow UserVoice에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자와 UserVoice에 hello 관련된 사용자 간 링크 관계를 설정 하는 toobe가 필요 합니다.

UserVoice에 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure와 UserVoice 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.
...
3. **[UserVoice 테스트 사용자 만들기](#create-a-uservoice-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 UserVoice에 해당 하는 도구입니다.
...
이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 UserVoice 응용 프로그램에서 single sign on 구성 합니다.

**Azure AD tooconfigure single sign on와 UserVoice를 hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **UserVoice** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **UserVoice 도메인 및 Url** 섹션를 hello 다음 단계를 수행 합니다.

    ![UserVoice 도메인 및 URL Single Sign-On 정보](./media/active-directory-saas-uservoice-tutorial/tutorial_uservoice_url.png)

    a. Hello에 **로그온 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<tenantname>.UserVoice.com`

    b. Hello에 **식별자** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<tenantname>.UserVoice.com`

    > [!NOTE] 
    > 이러한 값은 실제 값이 아닙니다. 이러한 항목을 업데이트 로그온 URL과 식별자 실제 hello로 값입니다. 연락처 [UserVoice 클라이언트 지원 팀](https://www.uservoice.com/) tooget 이러한 값입니다.
...
6. Hello에 **UserVoice 구성** 섹션에서 클릭 **구성 UserVoice** tooopen **sign on 구성** 창. 복사 hello **Sign-Out URL 및 SAML Single Sign-on 서비스 URL** hello에서 **빠른 참조 섹션.**

    ![UserVoice 구성](./media/active-directory-saas-uservoice-tutorial/tutorial_uservoice_configure.png) 

7. 다른 웹 브라우저 창에서 관리자 권한으로 tooyour UserVoice 회사 사이트에 로그인 합니다.

8. 도구 모음의 hello hello 위쪽에 클릭 **설정**를 선택한 후 **웹 포털** hello 메뉴에서 합니다.
   
    ![앱 쪽의 설정 섹션](./media/active-directory-saas-uservoice-tutorial/ic777519.png "설정")

9. Hello에 **웹 포털** 탭 hello **사용자 인증** 섹션에서 클릭 **편집** tooopen hello **사용자 인증 편집** 대화 상자 페이지입니다.
   
    ![웹 포털 탭](./media/active-directory-saas-uservoice-tutorial/ic777520.png "웹 포털")

10. Hello에 **사용자 인증 편집** 대화 상자 페이지를 hello 다음 단계를 수행 합니다.
   
    ![사용자 인증 편집](./media/active-directory-saas-uservoice-tutorial/ic777521.png "편집 사용자 인증")
   
    a. **SSO(Single Sign-On)**를 클릭합니다.
 
    b. 붙여넣기 hello **SAML Single Sign-on 서비스 URL** hello에 hello Azure 포털에서에서 복사한 값 **SSO 원격 로그인** 텍스트 상자에 붙여넣습니다.

    c. 붙여넣기 hello **Sign-Out URL** hello에 hello Azure 포털에서에서 복사한 값 **SSO 원격 로그 아웃 textbox**합니다.
 
    d. 붙여넣기 hello **지문** 에 Azure 포털에서 복사 하는 값은 **현재 인증서 SHA1 지문** 텍스트 상자에 붙여넣습니다.
...
tooenable Azure AD 사용자가 toolog tooUserVoice에서 프로 비전 해야 UserVoice에 있습니다. Hello UserVoice의 경우에서 프로 비전은 수동 작업입니다.

### <a name="tooprovision-a-user-account-perform-hello-following-steps"></a>tooprovision 사용자 계정을 hello 다음 단계를 수행 합니다.
1. Tooyour 로그인 **UserVoice** 테 넌 트입니다.
...
6. Hello에 **관리자 초대** 대화 상자에서 hello 다음 단계를 수행 합니다.
   
    ![관리자 초대](./media/active-directory-saas-uservoice-tutorial/ic777814.png "관리자 초대")
   
    a. Hello 전자 메일 텍스트 상자에 tooprovision을 차례로 클릭 hello 계정의 hello 전자 메일 주소를 입력 **추가**합니다.
   
    b. **초대**를 클릭합니다.

> [!NOTE]
> 다른 UserVoice 사용자 계정 만들기 도구를 사용할 수 있습니다 또는 AAD 사용자 계정을 UserVoice tooprovision에서 제공 된 Api입니다.

### <a name="assign-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당 합니다.

이 섹션에서는 tooUserVoice 액세스 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![Hello 사용자 역할 할당][200] 

**tooassign Britta Simon tooUserVoice hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **UserVoice**합니다.
...
Hello 액세스 패널에서에서 hello UserVoice 타일을 클릭할 때 자동으로 로그온 tooyour UserVoice 응용 프로그램을 구해야 합니다.
