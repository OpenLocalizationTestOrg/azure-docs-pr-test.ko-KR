...
이 자습서에 설명 어떻게 toointegrate Azure Active Directory (Azure AD)와 Druva 합니다.

Azure AD와 Druva 통합 이점을 다음 hello로 제공 합니다.

- 액세스 tooDruva을 지닌 Azure AD에서 제어할 수 있습니다.
- 에 사용자가 tooautomatically get 로그온 tooDruva (Single Sign-on)는 Azure AD 계정으로 설정할 수 있습니다.
...
Druva와 Azure AD 통합 tooconfigure 다음 항목 hello가 필요 합니다.
...
1. Druva는 hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-druva-from-hello-gallery"></a>Druva는 hello 갤러리 추가
tooconfigure hello와의 통합 Druva Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 Druva tooadd가 필요합니다.

**Druva hello 갤러리에서 tooadd hello 다음 단계를 수행 합니다.**
...
4. Hello 검색 상자에 입력 **Druva**선택, **Druva** 결과 패널에서 클릭 **추가** tooadd hello 응용 프로그램 단추입니다.
...
Single sign on toowork에 대 한 Azure AD는 tooknow Druva에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자와 Druva에 hello 관련된 사용자 간 링크 관계를 설정 하는 toobe가 필요 합니다.

Druva에 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure와 Druva 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.
...
3. **[Druva 테스트 사용자 만들기](#create-a-druva-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 Druva에 해당 하는 도구입니다.
...
이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 Druva 응용 프로그램에서 single sign on 구성 합니다.

**Azure AD tooconfigure single sign on와 Druva를 hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **Druva** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **Druva 도메인 및 Url** 섹션를 hello 다음 단계를 수행 합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-druva-tutorial/tutorial_druva_url.png)

    Hello에 **로그온 URL** textbox hello URL 입력:`https://cloud.druva.com/home`
...
5. Druva 응용 프로그램에 사용자 지정 특성 매핑을 tooyour tooadd 요구 하는 특정 형식으로 hello SAML 어설션이 **SAML 토큰 특성** 구성 합니다. 

    ![Single Sign-on 구성](./media/active-directory-saas-druva-tutorial/tutorial_druva_attribute.png)

6. Hello에 **사용자 특성** hello 섹션 **Single sign on** 대화 상자에서 hello 이미지 앞에 표시 된 대로 SAML 토큰 특성을 구성 하 고 hello 다음 단계를 수행 합니다.

    | 특성 이름      | 특성 값      |
    | ------------------- | -------------------- |
    | insync\_auth\_token |Hello 토큰 생성 된 값 입력 |
    ...
    c. Hello에서 **값** 목록, 해당 행에 대 한 표시 유형 hello 특성 값입니다. hello 토큰 생성 된 값이 자습서의 뒷부분에 설명 되어 있습니다.
...
8. Hello에 **Druva 구성** 섹션에서 클릭 **구성 Druva** tooopen **sign on 구성** 창. 복사 hello **Sign-Out URL 및 SAML Single Sign-on 서비스 URL** hello에서 **빠른 참조 섹션.**

    ![Single Sign-on 구성](./media/active-directory-saas-druva-tutorial/tutorial_druva_configure.png) 

9. 다른 웹 브라우저 창에서 관리자 권한으로 tooyour Druva 회사 사이트에 로그인 합니다.

10. 너무 이동**관리 \> 설정을**합니다.

    ![설정](./media/active-directory-saas-druva-tutorial/ic795091.png "설정")

11. Hello Single Sign-on 설정 대화 상자에서 hello 다음 단계를 수행 합니다.

    ![Single Sign-On 설정](./media/active-directory-saas-druva-tutorial/ic795092.png "Single Sign-On 설정")
    
    a. 붙여넣기 **SAML Single Sign-on 서비스 URL** hello에 hello Azure 포털에서에서 복사한 값 **ID 공급자 로그인 URL** 텍스트 상자에 붙여넣습니다.
    
    b. 붙여넣기 **Sign-Out URL** hello에 hello Azure 포털에서에서 복사한 값 **ID 공급자 로그 아웃 URL** 텍스트 상자에 붙여넣습니다.
    
     c. 콘텐츠를 클립보드에 복사 hello 메모장에서 e-64로 인코딩된 인증서를 열고 toohello 붙여 **ID 공급자 인증서** textbox
     
     d. tooopen hello **설정** 페이지 **저장**합니다.

12. Hello에 **설정** 페이지 **SSO 토큰 생성**합니다.

    ![설정](./media/active-directory-saas-druva-tutorial/ic795093.png "설정")

13. Hello에 **Single Sign on 인증 토큰** 대화 상자에서 hello 다음 단계를 수행 합니다.

    ![SSO 토큰](./media/active-directory-saas-druva-tutorial/ic795094.png "SSO 토큰")
    
    a. 클릭 **복사**, 붙여넣기 hello에서 값을 복사한 **값** hello 텍스트 상자로 **특성 추가** 섹션.
...
Tooenable Azure AD 사용자가 toolog tooDruva에 주문 하 고에 Druva에 이들 프로 비전 해야 합니다. Hello Druva의 경우에서 프로 비전은 수동 작업입니다.

**tooconfigure 사용자 프로비저닝, hello 다음 단계를 수행:**

1. Tooyour 로그인 **Druva** 회사 사이트에서 관리자 권한으로 로그인 합니다.
...
4. Hello 새 사용자 만들기 대화 상자에서 hello 다음 단계를 수행 합니다.
   
   ![NewUser 만들기](./media/active-directory-saas-druva-tutorial/ic795099.png "NewUser 만들기")
   
   a. Hello에 **전자 메일 주소** 텍스트 상자와 같은 사용자의 전자 메일을 hello 입력  **brittasimon@contoso.com** 합니다.
   
   b. Hello에 **이름** 텍스트 상자와 같은 사용자의 hello 이름 입력 **BrittaSimon**합니다.
   
   c. **사용자 만들기**를 클릭합니다.

>[!NOTE]
>다른 Druva 사용자 계정 만들기 도구를 사용할 수 있습니다 또는 Azure AD 사용자 계정을 tooprovision Druva에서 제공 된 Api입니다.

### <a name="assign-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당 합니다.

이 섹션에서는 tooDruva 액세스 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![Hello 사용자 역할 할당][200] 

**tooassign Britta Simon tooDruva hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **Druva**합니다.
...
Hello 액세스 패널에서에서 hello Druva 타일을 클릭할 때 자동으로 로그온 tooyour Druva 응용 프로그램을 구해야 합니다.
