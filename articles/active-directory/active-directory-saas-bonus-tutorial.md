...
이 자습서에 설명 어떻게 toointegrate Bonusly Azure Active directory (Azure AD).

다음 이점을 hello로 제공 Bonusly Azure AD와 통합:

- 액세스 tooBonusly을 지닌 Azure AD에서 제어할 수 있습니다.
- 프로그램 사용자 tooautomatically get 로그온 tooBonusly (Single Sign-on)와 Azure AD 계정 사용 하도록 설정할 수 있습니다.
...
다음 항목 hello가 필요 tooconfigure Bonusly와 Azure AD 통합 합니다.
...
1. Bonusly는 hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-bonusly-from-hello-gallery"></a>Bonusly는 hello 갤러리 추가
tooconfigure hello와의 통합 Bonusly Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 Bonusly tooadd가 필요합니다.

**hello 갤러리에서 Bonusly tooadd hello 다음 단계를 수행 합니다.**
...
4. Hello 검색 상자에 입력 **Bonusly**선택, **Bonusly** 결과 패널에서 클릭 **추가** tooadd hello 응용 프로그램 단추입니다.
...
Single sign on toowork에 대 한 Azure AD는 tooknow Bonusly에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자 및 Bonusly에 hello 관련된 사용자 간 링크 관계를 설정할 toobe가 필요 합니다.

Bonusly에서 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure 및 Bonusly 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.
...
3. **[Bonusly 테스트 사용자 만들기](#create-a-bonusly-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 Bonusly에 해당 하는 도구입니다.
...
이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 Bonusly 응용 프로그램에서 single sign on 구성 합니다.

**tooconfigure Azure AD single sign on, Bonusly와 hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **Bonusly** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **Bonusly 도메인 및 Url** 섹션를 hello 다음 단계를 수행 합니다.

    ![Bonusly 도메인 및 URL Single Sign-On 정보](./media/active-directory-saas-bonus-tutorial/tutorial_bonusly_url.png)

    Hello에 **회신 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://Bonus.ly/saml/<tenant-name>`

    > [!NOTE] 
    > hello 값이 실제 아닙니다. 업데이트 hello 값과 실제 회신 URL hello 합니다. 연락처 [Bonusly 지원 팀](https://Bonusly/contact) tooget hello 값입니다.
...
6. Hello에 **Bonusly 구성** 섹션에서 클릭 **Bonusly 구성** tooopen **sign on 구성** 창. 복사 hello **SAML 엔터티 ID, 및 SAML Single Sign-on 서비스 URL** hello에서 **빠른 참조 섹션.**

    ![Bonusly 구성](./media/active-directory-saas-bonus-tutorial/tutorial_bonusly_configure.png) 

7. 다른 브라우저 창에서 tooyour에 로그인 **Bonusly** 테 넌 트입니다.

8. 도구 모음의 hello hello 위쪽에 클릭 **설정**를 선택한 후 **통합 및 앱**합니다.
   
    ![Bonusly 소셜 섹션](./media/active-directory-saas-bonus-tutorial/ic773686.png "Bonusly")
9. **Single Sign-On**에서 **SAML**을 선택합니다.

10. Hello에 **SAML** 대화 상자 페이지를 hello 다음 단계를 수행 합니다.
   
    ![Bonusly Saml 대화 상자 페이지](./media/active-directory-saas-bonus-tutorial/ic773687.png "Bonusly")
   
    a. Hello에 **IdP SSO 대상 URL** 붙여넣기 hello 값의 텍스트 상자 **SAML Single Sign-on 서비스 URL**, Azure 포털에서 복사한입니다.
   
    b. Hello에 **IdP 발급자** 붙여넣기 hello 값의 텍스트 상자 **SAML 엔터티 ID**, Azure 포털에서 복사한입니다. 

    c. Hello에 **IdP 로그인 URL** 붙여넣기 hello 값의 텍스트 상자 **SAML Single Sign-on 서비스 URL**, Azure 포털에서 복사한입니다.

    d. 붙여넣기는 **지문** 값이 hello에 Azure 포털에서 복사 **인증서 지문** 텍스트 상자에 붙여넣습니다.
...
Tooenable Azure AD 사용자가 toolog tooBonusly에 주문 하 고에 Bonusly에 이들 프로 비전 해야 합니다. Hello Bonusly의 경우에서 프로 비전은 수동 작업입니다.

>[!NOTE]
>다른 Bonusly 사용자 계정 만들기 도구를 사용할 수 있습니다 또는 Api에서 제공 Bonusly tooprovision AAD 사용자 계정을 합니다.
>  

**tooconfigure 사용자 프로비저닝, hello 다음 단계를 수행:**

1. 웹 브라우저 창에서 tooyour Bonusly 테 넌 트에 로그인 합니다.

2. **설정**을 클릭합니다.
 
    ![설정](./media/active-directory-saas-bonus-tutorial/ic781041.png "설정")

3. Hello 클릭 **사용자 및 보너스** 탭 합니다.
...
6. Hello에 **사용자 추가** 대화 상자에서 hello 다음 단계를 수행 합니다.
    ...
    c. Hello에 **전자 메일** 텍스트 상자와 같은 사용자의 전자 메일을 hello 입력  **brittasimon@contoso.com** 합니다.

    d. **Save**를 클릭합니다.
   
     >[!NOTE]
     >hello Azure AD 계정 보유자 데이터베이스가 활성화 전에 tooconfirm hello 계정 연결을 포함 하는 전자 메일을 수신 합니다.
     >  

### <a name="assign-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당 합니다.

이 섹션에서는 tooBonusly 액세스 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![Hello 사용자 역할 할당][200] 

**tooassign Britta Simon tooBonusly hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **Bonusly**합니다.
...
Hello Bonusly 타일을 클릭할 때 hello 액세스 패널에서 자동으로 로그온 tooyour Bonusly 응용 프로그램을 얻어야 합니다.
