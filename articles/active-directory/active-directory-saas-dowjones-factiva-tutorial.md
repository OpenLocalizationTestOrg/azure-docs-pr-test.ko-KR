...
이 자습서에 설명 어떻게 toointegrate Azure Active Directory (Azure AD)와 jones 이면 특정 Factiva 되돌려집니다.

다운로드 jones 이면 특정 Factiva Azure AD와 통합 이점을 다음 hello로 제공 합니다.

- 액세스 tooDow jones 이면 특정 Factiva을 지닌 Azure AD에서 제어할 수 있습니다.
- Azure AD 계정을 사용 하면 사용자가 tooautomatically get 로그온 tooDow jones 이면 특정 Factiva (Single Sign-on)를 사용할 수 있습니다.
...
Azure AD 통합 되돌려집니다 jones 이면 특정 factiva tooconfigure 다음 항목 hello가 필요 합니다.
...
1. 다운로드 jones 이면 특정 Factiva hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-dow-jones-factiva-from-hello-gallery"></a>다운로드 jones 이면 특정 Factiva hello 갤러리 추가
tooconfigure hello와의 통합 되돌려집니다 jones 이면 특정 Factiva Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 다운로드 jones 이면 특정 Factiva tooadd가 필요합니다.

**hello 갤러리에서 다운로드 jones 이면 특정 Factiva tooadd hello 다음 단계를 수행 합니다.**
...
4. Hello 검색 상자에 입력 **되돌려집니다 jones 이면 특정 Factiva**합니다.

    ![Azure AD 테스트 사용자 만들기](./media/active-directory-saas-dowjones-factiva-tutorial/tutorial_dowjonesfactiva_search.png)

5. Hello 결과 패널에서 선택 **되돌려집니다 jones 이면 특정 Factiva**, 클릭 하 고 **추가** tooadd hello 응용 프로그램 단추입니다.
...
Single sign on toowork에 대 한 Azure AD는 tooknow 되돌려집니다 jones 이면 특정 Factiva에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자 및 다운로드 jones 이면 특정 Factiva에 hello 관련된 사용자 간 링크 관계를 설정할 toobe가 필요 합니다.

다운로드 jones 이면 특정 Factiva hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure 및 다운로드 jones 이면 특정 factiva Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.

1. **[Azure AD Single Sign-on 구성](#configuring-azure-ad-single-sign-on)**  -tooenable 사용자 toouse이이 기능입니다.
2. **[Azure AD 테스트 사용자 만들기](#creating-an-azure-ad-test-user)**  -tootest Azure AD single sign on Britta Simon 사용 합니다.
3. **[다운로드 jones 이면 특정 Factiva 테스트 사용자 만들기](#creating-a-dow-jones-factiva-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 되돌려집니다 jones 이면 특정 Factiva에 해당 하는 도구입니다.
4. **[Azure AD hello 테스트 사용자를 할당](#assigning-the-azure-ad-test-user)**  -tooenable Britta Simon toouse Azure AD에서 single sign-on입니다.
5. **[Single Sign-on 테스트](#testing-single-sign-on)**  -tooverify 구성 works를 hello 여부.

### <a name="configuring-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성

이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 사용 하도록 설정 및 다운로드 jones 이면 특정 Factiva 응용 프로그램에서 single sign on 구성 합니다.

**Azure AD에서 single sign-on 되돌려집니다 jones 이면 특정 factiva tooconfigure hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **되돌려집니다 jones 이면 특정 Factiva** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **되돌려집니다 jones 이면 특정 Factiva 도메인 및 Url** 섹션에서 hello 사용자 tooperform 모든 단계와 서로 hello 앱 Azure로 이미 사전 통합 되어 있습니다.
...
6. tooconfigure single sign on에서 **되돌려집니다 jones 이면 특정 Factiva** toosend hello 다운로드 해야 쪽에서는 **메타 데이터 XML** 너무[되돌려집니다 jones 이면 특정 Factiva 지원 팀](https://www.dowjones.com/contact/)합니다. 이 설정은 toohave hello 양쪽 모두에 제대로 설정 하는 SAML SSO 연결 설정 합니다.
...
이 섹션에서는 Dow Jones Factiva에서 Britta Simon이라는 사용자를 만듭니다. 다운로드와 협력 하세요 [jones 이면 특정 Factiva 지원 팀](https://www.dowjones.com/contact/) hello 되돌려집니다 jones 이면 특정 Factiva 플랫폼의 tooadd hello 사용자입니다.

### <a name="assigning-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당합니다.

이 섹션에서는 액세스 tooDow jones 이면 특정 Factiva 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![사용자 할당][200] 

**tooassign Britta Simon tooDow jones 이면 특정 Factiva hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **되돌려집니다 jones 이면 특정 Factiva**합니다.
...
Hello 액세스 패널에서에서 hello 되돌려집니다 jones 이면 특정 Factiva 타일을 클릭할 때 자동으로 로그온 tooyour 되돌려집니다 jones 이면 특정 Factiva 응용 프로그램을 구해야 합니다.
