...
이 자습서에 설명 어떻게 toointegrate Azure Active Directory (Azure AD)와 Projectplace 합니다.

Azure AD와 Projectplace 통합 이점을 다음 hello로 제공 합니다.

- 액세스 tooProjectplace을 지닌 Azure AD에서 제어할 수 있습니다.
- 프로그램 사용자 tooautomatically get 로그온 tooProjectplace (Single Sign-on)와 Azure AD 계정 사용 하도록 설정할 수 있습니다.
...
Projectplace와 Azure AD 통합 tooconfigure 다음 항목 hello가 필요 합니다.
...
1. Projectplace는 hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-projectplace-from-hello-gallery"></a>Projectplace는 hello 갤러리 추가
tooconfigure hello와의 통합 Projectplace Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 Projectplace tooadd가 필요합니다.

**hello 갤러리에서 Projectplace tooadd hello 다음 단계를 수행 합니다.**
...
Single sign on toowork에 대 한 Azure AD는 tooknow Projectplace에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자와 Projectplace에 hello 관련된 사용자 간 링크 관계를 설정 하는 toobe가 필요 합니다.

Projectplace에 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure와 Projectplace 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.

1. **[Azure AD Single Sign-on 구성](#configuring-azure-ad-single-sign-on)**  -tooenable 사용자 toouse이이 기능입니다.
2. **[Azure AD 테스트 사용자 만들기](#creating-an-azure-ad-test-user)**  -tootest Azure AD single sign on Britta Simon 사용 합니다.
3. **[Projectplace 테스트 사용자 만들기](#creating-a-projectplace-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 Projectplace에 해당 하는 도구입니다.
4. **[Azure AD hello 테스트 사용자를 할당](#assigning-the-azure-ad-test-user)**  -tooenable Britta Simon toouse Azure AD에서 single sign-on입니다.
5. **[Single Sign-on 테스트](#testing-single-sign-on)**  -tooverify 구성 works를 hello 여부.

### <a name="configuring-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성

이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 Projectplace 응용 프로그램에서 single sign on 구성 합니다.

**Azure AD tooconfigure single sign on와 Projectplace를 hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **Projectplace** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **Projectplace 도메인 및 Url** 섹션를 hello 다음 단계를 수행 합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-projectplace-tutorial/tutorial_projectplace_url.png)

    Hello에 **로그온 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<company>.projectplace.com`

    > [!NOTE] 
    > 이 값은 실제 값이 아닙니다. Hello로이 값을 업데이트 합니다. 실제 로그온 URL입니다. 연락처 [클라이언트 Projectplace 지원 팀](https://success.planview.com/Projectplace/Support) tooget이이 값입니다. 
...
7. tooconfigure single sign on에서 **Projectplace** toosend hello 다운로드 해야 쪽에서는 **메타 데이터 XML** 너무[Projectplace 지원 팀](https://success.planview.com/Projectplace/Support)합니다. 이 설정은 toohave hello 양쪽 모두에 제대로 설정 하는 SAML SSO 연결 설정 합니다.

>[!NOTE]
>hello single sign on 구성에 hello 수행한 toobe [Projectplace 지원 팀](https://success.planview.com/Projectplace/Support)합니다. Hello 구성이 완료 되는 즉시 알림을 받아볼 수 있습니다.
...
Tooenable Azure AD 사용자가 toolog Projectplace에 주문 하 고에 Projectplace에 이들 프로 비전 해야 합니다. Hello Projectplace의 경우에서 프로 비전은 수동 작업입니다.

**tooprovision 사용자 계정을 hello 다음 단계를 수행 합니다.**

1. Tooyour 로그인 **Projectplace** 회사 사이트에 관리자 권한으로 합니다.

2. 너무 이동**사람**, 클릭 하 고 **멤버**합니다.
...
4. Hello에 **멤버 추가** 섹션를 hello 다음 단계를 수행 합니다.
   
    ![새 멤버](./media/active-directory-saas-projectplace-tutorial/ic790233.png "새 멤버")
   
    a. Hello에 **새 멤버** tooprovision hello에 하려는 유효한 AAD 계정의 hello 전자 메일 주소를 입력 텍스트 상자 관련 텍스트 상자입니다.
   
    b. **보내기**를 클릭합니다.

   전자 메일 활성화 되기 전에 tooconfirm hello 계정 연결을 포함 하 여 toohello Azure Active Directory 계정 소유자를 전송 됩니다.

>[!NOTE]
>다른 Projectplace 사용자 계정 만들기 도구를 사용할 수 있습니다 또는 AAD 사용자 계정을 tooprovision Projectplace에서 제공 된 Api입니다.

### <a name="assigning-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당합니다.

이 섹션에서는 tooProjectplace 액세스 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![사용자 할당][200] 

**tooassign Britta Simon tooProjectplace hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **Projectplace**합니다.
...
Hello 액세스 패널에서에서 hello Projectplace 타일을 클릭할 때 자동으로 로그온 tooyour Projectplace 응용 프로그램을 구해야 합니다.
