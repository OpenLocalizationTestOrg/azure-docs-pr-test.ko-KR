...
이 자습서에 설명 어떻게 toointegrate LiquidFiles Azure Active directory (Azure AD).

다음 이점을 hello로 제공 LiquidFiles Azure AD와 통합:

- 액세스 tooLiquidFiles을 지닌 Azure AD에서 제어할 수 있습니다.
- 프로그램 사용자 tooautomatically get 로그온 tooLiquidFiles (Single Sign-on)와 Azure AD 계정 사용 하도록 설정할 수 있습니다.
...
다음 항목 hello가 필요 tooconfigure LiquidFiles와 Azure AD 통합 합니다.
...
1. LiquidFiles는 hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-liquidfiles-from-hello-gallery"></a>LiquidFiles는 hello 갤러리 추가
tooconfigure hello와의 통합 LiquidFiles Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 LiquidFiles tooadd가 필요합니다.

**hello 갤러리에서 LiquidFiles tooadd hello 다음 단계를 수행 합니다.**
...
Single sign on toowork에 대 한 Azure AD는 tooknow LiquidFiles에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자 및 LiquidFiles에 hello 관련된 사용자 간 링크 관계를 설정할 toobe가 필요 합니다.

LiquidFiles에서 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure 및 LiquidFiles 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.

1. **[Azure AD Single Sign-on 구성](#configuring-azure-ad-single-sign-on)**  -tooenable 사용자 toouse이이 기능입니다.
2. **[Azure AD 테스트 사용자 만들기](#creating-an-azure-ad-test-user)**  -tootest Azure AD single sign on Britta Simon 사용 합니다.
3. **[LiquidFiles 테스트 사용자 만들기](#creating-a-liquidfiles-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 LiquidFiles에 해당 하는 도구입니다.
4. **[Azure AD hello 테스트 사용자를 할당](#assigning-the-azure-ad-test-user)**  -tooenable Britta Simon toouse Azure AD에서 single sign-on입니다.
5. **[Single Sign-on 테스트](#testing-single-sign-on)**  -tooverify 구성 works를 hello 여부.

### <a name="configuring-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성

이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 LiquidFiles 응용 프로그램에서 single sign on 구성 합니다.

**tooconfigure Azure AD single sign on, LiquidFiles와 hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **LiquidFiles** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **LiquidFiles 도메인 및 Url** 섹션를 hello 다음 단계를 수행 합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-liquidfiles-tutorial/tutorial_liquidfiles_url.png)

    a. Hello에 **로그온 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<YOUR_SERVER_URL>/saml/init`

    b. Hello에 **식별자** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<YOUR_SERVER_URL>`

    c. b. Hello에 **회신 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<YOUR_SERVER_URL>/saml/consume`

    > [!NOTE] 
    > 이러한 값은 실제 값이 아닙니다. 이러한 값을 업데이트 hello 실제 로그온 URL, 식별자 및 회신 URL입니다. 연락처 [LiquidFiles 클라이언트 지원 팀](https://www.liquidfiles.com/support.html) tooget 이러한 값입니다. 
...
6. Hello에 **LiquidFiles 구성** 섹션에서 클릭 **구성 LiquidFiles** tooopen **sign on 구성** 창. 복사 hello **Sign-Out URL 및 SAML Single Sign-on 서비스 URL** hello에서 **빠른 참조 섹션.**

    ![Single Sign-on 구성](./media/active-directory-saas-liquidfiles-tutorial/tutorial_liquidfiles_configure.png)
 
7. 관리자 권한으로 로그온 tooyour LiquidFiles 회사 사이트입니다.

8. 클릭 **Single Sign On** hello에 **관리자 > 구성** hello 메뉴에서 합니다.

9. Hello에 **Single Sign On 구성** 페이지를 hello 다음 단계를 수행 합니다.
    ...
    d. Hello에 **IDP 인증서 지문** 텍스트 붙여넣기 hello **지문** Azure 포털에서 복사한 값으로...

    e. Hello 이름 식별자 형식 텍스트 상자에 입력 hello 값 **urn: oasis: 이름: tc: SAML:1.1:nameid-: emailAddress**합니다.

    f. 인증 컨텍스트 textbox hello에 hello 값 입력 **urn: oasis: 이름: tc: SAML:2.0:ac:classes:PasswordProtectedTransport**합니다.
...
hello이이 섹션의 목적은 toocreate Britta Simon LiquidFiles에서 호출 하는 사용자입니다. 작업을 프로그램 LiquidFiles 서버 관리자 tooget와 직접 tooyour LiquidFiles 응용 프로그램에에서 로그인 하기 전에 사용자로 추가 합니다.

### <a name="assigning-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당합니다.

이 섹션에서는 tooLiquidFiles 액세스 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![사용자 할당][200] 

**tooassign Britta Simon tooLiquidFiles hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **LiquidFiles**합니다.
...
Hello LiquidFiles hello 액세스 패널에서에서 타일을 클릭할 때 자동으로 로그온 tooyour LiquidFiles 응용 프로그램을 구해야 합니다.
