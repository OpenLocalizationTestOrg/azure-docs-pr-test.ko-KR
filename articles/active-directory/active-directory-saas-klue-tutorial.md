...
이 자습서에 설명 어떻게 toointegrate Klue Azure Active directory (Azure AD).

다음 이점을 hello로 제공 Klue Azure AD와 통합:

- 액세스 tooKlue을 지닌 Azure AD에서 제어할 수 있습니다.
- 프로그램 사용자 tooautomatically get 로그온 tooKlue (Single Sign-on)와 Azure AD 계정 사용 하도록 설정할 수 있습니다.
...
다음 항목 hello가 필요 tooconfigure Klue와 Azure AD 통합 합니다.
...
1. Klue는 hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-klue-from-hello-gallery"></a>Klue는 hello 갤러리 추가
tooconfigure hello와의 통합 Klue Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 Klue tooadd가 필요합니다.

**hello 갤러리에서 Klue tooadd hello 다음 단계를 수행 합니다.**
...
Single sign on toowork에 대 한 Azure AD는 tooknow Klue에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자 및 Klue에 hello 관련된 사용자 간 링크 관계를 설정할 toobe가 필요 합니다.

Klue에서 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure 및 Klue 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.

1. **[Azure AD Single Sign-on 구성](#configuring-azure-ad-single-sign-on)**  -tooenable 사용자 toouse이이 기능입니다.
2. **[Azure AD 테스트 사용자 만들기](#creating-an-azure-ad-test-user)**  -tootest Azure AD single sign on Britta Simon 사용 합니다.
3. **[Klue 테스트 사용자 만들기](#creating-a-klue-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 Klue에 해당 하는 도구입니다.
4. **[Azure AD hello 테스트 사용자를 할당](#assigning-the-azure-ad-test-user)**  -tooenable Britta Simon toouse Azure AD에서 single sign-on입니다.
5. **[Single Sign-on 테스트](#testing-single-sign-on)**  -tooverify 구성 works를 hello 여부.

### <a name="configuring-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성

이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 Klue 응용 프로그램에서 single sign on 구성 합니다.

**tooconfigure Azure AD single sign on, Klue와 hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **Klue** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **Klue 도메인 및 Url** tooconfigure hello 응용 프로그램에 필요한 경우 섹션 **IDP** 시작 모드:

    ![Single Sign-on 구성](./media/active-directory-saas-klue-tutorial/tutorial_klue_url1.png)

    a. Hello에 **식별자** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`urn:klue:<Customer ID>`

    b. Hello에 **회신 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://app.klue.com/account/auth/saml/<Customer UUID>/callback`
...
4. **고급 URL 설정 표시**를 선택합니다. Tooconfigure hello 응용 프로그램에 필요한 경우 **SP** 시작 모드:
    ...
    Hello에 **로그온 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://app.klue.com/account/auth/saml/<Customer UUID>/`
     
    > [!NOTE] 
    > 이러한 값은 실제 값이 아닙니다. Hello 실제 회신 URL, 식별자 및 로그온 URL와 이러한 값을 업데이트 합니다. 연락처 [Klue 클라이언트 지원 팀](mailto:support@klue.com) tooget 이러한 값입니다.

5. hello Klue 응용 프로그램 구성을 수행 해야 하면 tooadd 사용자 지정 특성 매핑을 tooyour SAML 토큰 특성을 특정 형식에서 hello SAML 어설션이 필요 합니다. Hello에서 이러한 특성의 hello 값을 관리할 수 있습니다 "**사용자 특성**" 응용 프로그램 통합 페이지에서 섹션. 
...
7. Hello에 **SAML 서명 인증서** 섹션에서 클릭 **Certificate(Base64)** hello 인증서 파일을 컴퓨터에 저장 합니다.
...
9. Hello에 **Klue 구성** 섹션에서 클릭 **구성 Klue** tooopen **sign on 구성** 창. 복사 hello **SAML Single Sign-on 서비스 URL 및 엔터티 ID가 SAML** hello에서 **빠른 참조 섹션.**

    ![Single Sign-on 구성](./media/active-directory-saas-klue-tutorial/tutorial_klue_configure.png) 

10. tooconfigure single sign on에서 **Klue** toosend hello 다운로드 해야 쪽에서는 **Certificate(Base64), SAML Single Sign-on 서비스 URL 및 엔터티 ID가 SAML** 너무[Klue 지원 팀](mailto:support@klue.com).
...
hello이이 섹션의 목적은 toocreate Britta Simon Klue에서 호출 하는 사용자입니다. Klue는 적시에 프로비전을 지원하며 기본적으로 사용하도록 설정합니다. 이 섹션에 작업 항목이 없습니다. 새 사용자는 아직 존재 하지 않는 경우 시도 tooaccess Klue 중 생성 됩니다.

>[!Note]
>Toocreate 수동으로 사용자에 게 문의 해야 할 경우 [Klue 지원 팀](mailto:support@klue.com)합니다.

### <a name="assigning-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당합니다.

이 섹션에서는 tooKlue 액세스 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![사용자 할당][200] 

**tooassign Britta Simon tooKlue hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **Klue**합니다.
...
Hello 액세스 패널에서에서 hello Klue 타일을 클릭할 때 자동으로 로그온 tooyour Klue 응용 프로그램을 구해야 합니다.
