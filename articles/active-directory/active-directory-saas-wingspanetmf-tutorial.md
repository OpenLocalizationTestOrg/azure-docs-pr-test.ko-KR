...
이 자습서에 설명 어떻게 toointegrate 날개 길이 eTMF Azure Active directory (Azure AD).

다음 이점을 hello로 제공 날개 길이 eTMF Azure AD와 통합:

- 액세스 tooWingspan eTMF 지닌 Azure AD에서 제어할 수 있습니다.
- 프로그램 사용자 tooautomatically get 로그온 tooWingspan eTMF (Single Sign-on)와 Azure AD 계정 사용 하도록 설정할 수 있습니다.
...
다음 항목 hello가 필요 tooconfigure 날개 길이 eTMF와 Azure AD 통합 합니다.
...
1. 날개 길이 eTMF hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-wingspan-etmf-from-hello-gallery"></a>날개 길이 eTMF hello 갤러리 추가
tooconfigure hello와의 통합 날개 길이 eTMF Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 tooadd 날개 길이 eTMF 필요합니다.

**hello 갤러리에서 tooadd 날개 길이 eTMF hello 다음 단계를 수행 합니다.**
...
Single sign on toowork에 대 한 Azure AD는 tooknow 날개 길이 eTMF에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자 및 날개 길이 eTMF에 hello 관련된 사용자 간 링크 관계를 설정할 toobe가 필요 합니다.

Hello hello 값을 할당 하 여이 링크 관계가 설정 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** 날개 길이 eTMF에 합니다.

tooconfigure 및 eTMF 날개 길이 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.

1. **[Azure AD Single Sign-on 구성](#configuring-azure-ad-single-sign-on)**  -tooenable 사용자 toouse이이 기능입니다.
2. **[Azure AD 테스트 사용자 만들기](#creating-an-azure-ad-test-user)**  -tootest Azure AD single sign on Britta Simon 사용 합니다.
3. **[날개 길이 eTMF 테스트 사용자 만들기](#creating-a-wingspan-etmf-test-user)**  -toohave에서 사용자의 연결 된 Azure AD toohello 표현인 날개 길이 eTMF Britta Simon 해당 하는 도구입니다.
4. **[Azure AD hello 테스트 사용자를 할당](#assigning-the-azure-ad-test-user)**  -tooenable Britta Simon toouse Azure AD에서 single sign-on입니다.
5. **[Single Sign-on 테스트](#testing-single-sign-on)**  -tooverify 구성 works를 hello 여부.

### <a name="configuring-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성

이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 날개 길이 eTMF 응용 프로그램에서 single sign on 구성 합니다.

**Azure AD tooconfigure single sign on 날개 길이 eTMF와 hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **날개 길이 eTMF** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **날개 길이 eTMF 도메인 및 Url** 섹션를 hello 다음 단계를 수행 합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-wingspanetmf-tutorial/tutorial_wingspanetmf_url11.png)

    a. Hello에 **로그온 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<customer name>.<instance name>.mywingspan.com/saml`

    b. Hello에 **식별자** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`http://saml.<instance name>.wingspan.com/shibboleth`

    c. Hello에 **회신 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<customer name>.<instance name>.mywingspan.com/`
     
    > [!NOTE] 
    > 이러한 값은 실제 hello 되지 않습니다. Hello 실제 고객 이름 및 인스턴스 이름을 포함 하 여 실제 로그온 URL, 식별자 및 회신 URL hello로 이러한 값을 업데이트 합니다. 연락처 [날개 길이 eTMF 클라이언트 지원 팀](http://www.wingspan.com/contact-us/) tooget 이러한 값입니다. 
...
6. tooconfigure single sign on에서 **날개 길이 eTMF** toosend hello 다운로드 해야 쪽에서는 **메타 데이터 XML** 너무[날개 길이 eTMF 지원](http://www.wingspan.com/contact-us/)합니다. 설정할 toohave hello SAML SSO 연결 양쪽 모두에 제대로 설정.
...
이 섹션에서는 Wingspan eTMF에서 Britta Simon이라는 사용자를 만듭니다. 작업할 [날개 길이 eTMF 지원](http://www.wingspan.com/contact-us/) hello 날개 길이 eTMF 응용 프로그램의에서 tooadd hello 사용자입니다. Single Sign-On을 사용하려면 먼저 사용자를 만들고 활성화해야 합니다.

### <a name="assigning-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당합니다.

이 섹션에서는 tooWingspan eTMF 액세스 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![사용자 할당][200] 

**tooassign Britta Simon tooWingspan eTMF hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **날개 길이 eTMF**합니다.
...
Hello 날개 길이 eTMF 타일을 클릭 hello 액세스 패널에에서 됩니다 tooOrganization 로그온 페이지 리디렉션. 성공적으로 로그인 한 후 로그온 tooyour 날개 길이 eTMF 응용 프로그램 수 있습니다. 액세스 패널에 대한 자세한 내용은 [액세스 패널 소개](https://msdn.microsoft.com/library/dn308586)를 참조하세요.
