...
이 자습서에 설명 어떻게 toointegrate Pantheon Azure Active directory (Azure AD).

다음 이점을 hello로 제공 Pantheon Azure AD와 통합:

- 액세스 tooPantheon을 지닌 Azure AD에서 제어할 수 있습니다.
- 프로그램 사용자 tooautomatically get 로그온 tooPantheon (Single Sign-on)와 Azure AD 계정 사용 하도록 설정할 수 있습니다.
...
다음 항목 hello가 필요 tooconfigure Pantheon와 Azure AD 통합 합니다.
...
1. Pantheon은 hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-pantheon-from-hello-gallery"></a>Pantheon은 hello 갤러리 추가
tooconfigure hello와의 통합 Pantheon Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 Pantheon tooadd가 필요합니다.

**hello 갤러리에서 Pantheon tooadd hello 다음 단계를 수행 합니다.**
...
Single sign on toowork에 대 한 Azure AD는 tooknow Pantheon에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자 및 Pantheon에 hello 관련된 사용자 간 링크 관계를 설정할 toobe가 필요 합니다.

Pantheon에서 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure 및 Pantheon 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.

1. **[Azure AD Single Sign-on 구성](#configuring-azure-ad-single-sign-on)**  -tooenable 사용자 toouse이이 기능입니다.
2. **[Azure AD 테스트 사용자 만들기](#creating-an-azure-ad-test-user)**  -tootest Azure AD single sign on Britta Simon 사용 합니다.
3. **[Pantheon 테스트 사용자 만들기](#creating-a-pantheon-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 Pantheon에 해당 하는 도구입니다.
4. **[Azure AD hello 테스트 사용자를 할당](#assigning-the-azure-ad-test-user)**  -tooenable Britta Simon toouse Azure AD에서 single sign-on입니다.
5. **[Single Sign-on 테스트](#testing-single-sign-on)**  -tooverify 구성 works를 hello 여부.

### <a name="configuring-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성

이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 Pantheon 응용 프로그램에서 single sign on 구성 합니다.

**tooconfigure Azure AD single sign on, Pantheon와 hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **Pantheon** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **Pantheon 도메인 및 Url** 섹션를 hello 다음 단계를 수행 합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-pantheon-tutorial/tutorial_pantheon_url.png)

    a. Hello에 **식별자** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`urn:auth0:pantheon:<orgname>-SSO`

    b. Hello에 **회신 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://pantheon.auth0.com/login/callback?connection=<orgname>-SSO`

    > [!NOTE] 
    > 이러한 값은 실제 값이 아닙니다. Hello 실제 식별자 및 회신 URL로이 값을 업데이트 합니다. 연락처 [Pantheon 지원 팀](https://pantheon.io/docs/getting-support/) tooget 이러한 값입니다.

4. Pantheon 응용 프로그램 있습니다 tooset hello hello 사용자의 전자 메일 주소가 있는 UserIdentifier 특성 값을 필요로 하는 특정 형식으로 hello SAML 어설션이 필요 합니다. 기본적으로 Azure AD UserIdentifier 특성에 대 한 hello UserPrincipalName을 사용합니다. 하지만 성공적으로 통합에 대 한 tooadjust이 값 toomatch 사용자의 전자 메일 주소를 사용 합니다. hello 통합 후에 올바른 매핑을 hello 에서만 작동 합니다.
...
7. Hello에 **Pantheon 구성** 섹션에서 클릭 **구성 Pantheon** tooopen **sign on 구성** 창. 복사 hello **SAML Single Sign-on 서비스 URL** hello에서 **빠른 참조 섹션.**

    ![Single Sign-on 구성](./media/active-directory-saas-pantheon-tutorial/tutorial_pantheon_configure.png) 

8. tooconfigure single sign on에서 **Pantheon** toosend hello 다운로드 해야 쪽에서는 **인증서** 및 **SAML Single Sign-on 서비스 URL** 너무[Pantheon 지원 팀](https://pantheon.io/docs/getting-support/)합니다.

     > [!Note]
     > 또한 해야 tooprovide hello 전자 메일 도메인 정보 및 날짜 시간 tooenable이이 연결을 사용할 때. [여기](https://pantheon.io/docs/sso-organizations/)에서 이에 대한 자세한 내용을 볼 수 있습니다.
...
이 섹션에서는 Pantheon에서 Britta Simon이라는 사용자를 만듭니다. 아래 단계 tooadd hello 사용자 Pantheon에 hello를 수행 하십시오. 

>[!NOTE] 
>SSO에 대 한 toowork 사용자 toobe Pantheon에서 먼저 작성 해야 합니다.

1. 관리자 자격 증명으로 로그인 tooPantheon 합니다.
...
5. Hello 사용자의 전자 메일 주소를 입력 합니다.

6. Hello 사용자의 역할을 선택 합니다.

7. **사용자 추가**를 클릭합니다.

### <a name="assigning-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당합니다.

이 섹션에서는 tooPantheon 액세스 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![사용자 할당][200] 

**tooassign Britta Simon tooPantheon hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **Pantheon**합니다.
...
Hello 액세스 패널에서에서 hello Pantheon 타일을 클릭할 때 자동으로 로그온 tooyour Pantheon 응용 프로그램을 구해야 합니다.
