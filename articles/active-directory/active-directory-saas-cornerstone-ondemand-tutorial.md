...
이 자습서에 설명 어떻게 toointegrate Cornerstone OnDemand와 Azure Active Directory (Azure AD).

Azure AD와 Cornerstone OnDemand를 통합 이점을 다음 hello로 제공 합니다.

- Azure ad 액세스 tooCornerstone OnDemand를 가진 제어할 수 있습니다.
- 프로그램 사용자 tooautomatically get 로그온 tooCornerstone OnDemand (Single Sign-on)와 Azure AD 계정 사용 하도록 설정할 수 있습니다.
...
Cornerstone OnDemand와 Azure AD 통합 tooconfigure 다음 항목 hello가 필요 합니다.
...
1. Cornerstone OnDemand hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-cornerstone-ondemand-from-hello-gallery"></a>Cornerstone OnDemand hello 갤러리 추가
tooconfigure hello와의 통합 Cornerstone OnDemand Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 Cornerstone OnDemand tooadd가 필요합니다.

**Cornerstone OnDemand hello 갤러리에서 tooadd hello 다음 단계를 수행 합니다.**
...
4. Hello 검색 상자에 입력 **Cornerstone OnDemand**합니다.

    ![Azure AD 테스트 사용자 만들기](./media/active-directory-saas-cornerstone-ondemand-tutorial/tutorial_cornerstoneondemand_search.png)

5. Hello 결과 패널에서 선택 **Cornerstone OnDemand**, 클릭 하 고 **추가** tooadd hello 응용 프로그램 단추입니다.
...
Single sign on toowork에 대 한 Azure AD는 tooknow Cornerstone OnDemand에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자와 Cornerstone OnDemand에 hello 관련된 사용자 간 링크 관계를 설정 하는 toobe가 필요 합니다.

Cornerstone OnDemand에 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure와 Cornerstone OnDemand와 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.

1. **[Azure AD Single Sign-on 구성](#configuring-azure-ad-single-sign-on)**  -tooenable 사용자 toouse이이 기능입니다.
2. **[Azure AD 테스트 사용자 만들기](#creating-an-azure-ad-test-user)**  -tootest Azure AD single sign on Britta Simon 사용 합니다.
3. **[Cornerstone OnDemand 테스트 사용자 만들기](#creating-a-cornerstone-ondemand-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 Cornerstone OnDemand에 해당 하는 도구입니다.
4. **[Azure AD hello 테스트 사용자를 할당](#assigning-the-azure-ad-test-user)**  -tooenable Britta Simon toouse Azure AD에서 single sign-on입니다.
5. **[Single Sign-on 테스트](#testing-single-sign-on)**  -tooverify 구성 works를 hello 여부.

### <a name="configuring-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성

이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 Cornerstone OnDemand 응용 프로그램에서 single sign on 구성 합니다.

**Azure AD tooconfigure single sign on와 Cornerstone OnDemand hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **Cornerstone OnDemand** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **Cornerstone OnDemand 도메인 및 Url** 섹션에서 단계 다음에 나오는 hello 수행:

    ![Single Sign-on 구성](./media/active-directory-saas-cornerstone-ondemand-tutorial/tutorial_cornerstoneondemand_url.png)

    a. Hello에 **로그온 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<company>.csod.com`

    b. **식별자** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<company>.csod.com`

    > [!NOTE] 
    > 이러한 값은 실제 값이 아닙니다. 이러한 항목을 업데이트 로그온 URL과 식별자 실제 hello로 값입니다. 연락처 [Cornerstone OnDemand 클라이언트 지원 팀](mailTo:moreinfo@csod.com) tooget 이러한 값입니다. 
...
6. Hello에 **Cornerstone OnDemand 구성** 섹션에서 클릭 **Cornerstone OnDemand 구성** tooopen **sign on 구성** 창. 복사 hello **Sign-Out URL 및 SAML Single Sign-on 서비스 URL** hello에서 **빠른 참조 섹션.**

    ![Single Sign-on 구성](./media/active-directory-saas-cornerstone-ondemand-tutorial/tutorial_cornerstoneondemand_configure.png) 

7. tooconfigure single sign on에서 **Cornerstone OnDemand** toosend hello 다운로드 해야 쪽에서는 **인증서**, **Sign-Out URL** 및 **SAML Single 서비스 URL 로그온** 너무[Cornerstone OnDemand 지원 팀](mailTo:moreinfo@csod.com)합니다. 이 설정은 toohave hello 양쪽 모두에 제대로 설정 하는 SAML SSO 연결 설정 합니다.
...
Tooenable Azure AD 사용자가 toolog Cornerstone OnDemand에 주문 하 고에 Cornerstone OnDemand에 이들 프로 비전 해야 합니다. Hello Cornerstone OnDemand의 경우에서 프로 비전은 수동 작업입니다.

tooconfigure 사용자 프로비저닝, 송신 hello 정보 (예:: 이름, 전자 메일) tooprovision toohello hello Azure AD 사용자에 대 한 원하는 [Cornerstone OnDemand 지원 팀](mailTo:moreinfo@csod.com)합니다.

>[!NOTE]
>다른 Cornerstone OnDemand 사용자 계정 만들기 도구를 사용할 수 있습니다 또는 AAD 사용자 계정을 tooprovision Cornerstone OnDemand에서 제공 된 Api입니다.

### <a name="assigning-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당합니다.

이 섹션에서는 액세스 tooCornerstone OnDemand를 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![사용자 할당][200] 

**tooassign Britta Simon tooCornerstone OnDemand hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **Cornerstone OnDemand**합니다.
...
Hello 액세스 패널에서에서 hello Cornerstone OnDemand 타일을 클릭할 때 자동으로 로그온 tooyour Cornerstone OnDemand 응용 프로그램을 구해야 합니다.
