...
이 자습서에 설명 어떻게 toointegrate UserEcho Azure Active directory (Azure AD).

다음 이점을 hello로 제공 UserEcho Azure AD와 통합:

- 액세스 tooUserEcho을 지닌 Azure AD에서 제어할 수 있습니다.
- 프로그램 사용자 tooautomatically get 로그온 tooUserEcho (Single Sign-on)와 Azure AD 계정 사용 하도록 설정할 수 있습니다.
...
다음 항목 hello가 필요 tooconfigure UserEcho와 Azure AD 통합 합니다.
...
1. UserEcho는 hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-userecho-from-hello-gallery"></a>UserEcho는 hello 갤러리 추가
tooconfigure hello와의 통합 UserEcho Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 UserEcho tooadd가 필요합니다.

**hello 갤러리에서 UserEcho tooadd hello 다음 단계를 수행 합니다.**
...
Single sign on toowork에 대 한 Azure AD는 tooknow UserEcho에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자 및 UserEcho에 hello 관련된 사용자 간 링크 관계를 설정할 toobe가 필요 합니다.

UserEcho에서 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure 및 UserEcho 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.

1. **[Azure AD Single Sign-on 구성](#configuring-azure-ad-single-sign-on)**  -tooenable 사용자 toouse이이 기능입니다.
2. **[Azure AD 테스트 사용자 만들기](#creating-an-azure-ad-test-user)**  -tootest Azure AD single sign on Britta Simon 사용 합니다.
3. **[UserEcho 테스트 사용자 만들기](#creating-a-userecho-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 UserEcho에 해당 하는 도구입니다.
4. **[Azure AD hello 테스트 사용자를 할당](#assigning-the-azure-ad-test-user)**  -tooenable Britta Simon toouse Azure AD에서 single sign-on입니다.
5. **[Single Sign-on 테스트](#testing-single-sign-on)**  -tooverify 구성 works를 hello 여부.

### <a name="configuring-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성

이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 UserEcho 응용 프로그램에서 single sign on 구성 합니다.

**tooconfigure Azure AD single sign on, UserEcho와 hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **UserEcho** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **UserEcho 도메인 및 Url** 섹션를 hello 다음 단계를 수행 합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-userecho-tutorial/tutorial_userecho_url.png)

    a. Hello에 **로그온 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<companyname>.userecho.com/`

    b. Hello에 **식별자** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<companyname>.userecho.com/saml/metadata/`

    > [!NOTE] 
    > 이러한 값은 실제 값이 아닙니다. 이러한 항목을 업데이트 로그온 URL과 식별자 실제 hello로 값입니다. 연락처 [UserEcho 클라이언트 지원 팀](https://feedback.userecho.com/) tooget 이러한 값입니다. 
...
6. Hello에 **UserEcho 구성** 섹션에서 클릭 **구성 UserEcho** tooopen **sign on 구성** 창. 복사 hello **Sign-Out URL 및 SAML Single Sign-on 서비스 URL** hello에서 **빠른 참조 섹션.**

    ![Single Sign-on 구성](./media/active-directory-saas-userecho-tutorial/tutorial_userecho_configure.png) 

7. 다른 브라우저 창에서 관리자 권한으로 tooyour UserEcho 회사 사이트에 로그인 합니다.

8. Hello 위에 hello 도구 모음에서 사용자 이름 tooexpand hello 메뉴를 클릭 한 다음 클릭 **설치**합니다.
...
11. Hello에 **Single sign-on (SAML)** 페이지 hello 다음 단계를 수행 합니다.
   
    ![Single Sign-on 구성](./media/active-directory-saas-userecho-tutorial/tutorial_userecho_09.png)
    
    a. **SAML 사용**을 **예**로 선택합니다.
    
    b. 붙여넣기 **SAML Single Sign-on 서비스 URL**, hello에 hello Azure 포털에서에서 복사한 있는 **SAML SSO URL** 텍스트 상자에 붙여넣습니다.
    
    c. 붙여넣기 **Sign-Out URL**, hello에 hello Azure 포털에서에서 복사한 있는 **원격 logoout URL** 텍스트 상자에 붙여넣습니다.
    
    d. 콘텐츠 복사 hello 메모장에서 다운로드 한 인증서를 열고 hello에 붙여 넣습니다 **X.509 인증서** 텍스트 상자에 붙여넣습니다.
...
hello이이 섹션의 목적은 toocreate Britta Simon UserEcho에서 호출 하는 사용자입니다.

**toocreate Britta Simon UserEcho의 라는 사용자를 만들면 hello 다음 단계를 수행 합니다.**

1. 관리자 권한으로 로그온 tooyour UserEcho 회사 사이트입니다.

2. Hello 위에 hello 도구 모음에서 사용자 이름 tooexpand hello 메뉴를 클릭 한 다음 클릭 **설치**합니다.
   
    ![Single Sign-on 구성](./media/active-directory-saas-userecho-tutorial/tutorial_userecho_06.png)

3. 클릭 **사용자**, tooexpand hello **사용자** 섹션.
...
6. Hello에 **새 사용자를 초대** 대화 상자에서 hello 다음 단계를 수행 합니다.
   
    ![Single Sign-on 구성](./media/active-directory-saas-userecho-tutorial/tutorial_userecho_13.png)

    a. Hello에 **이름** textbox Britta Simon 같은 hello 사용자의 형식 이름입니다.
    
    b.  Hello에 **전자 메일** textbox, 사용자의 hello 전자 메일 주소를 입력 같은 Brittasimon@contoso.com합니다.
    
    c. **초대**를 클릭합니다.

TooBritta UserEcho를 사용 하 여 자신의 toostart 매핑함으로써 전달 합니다. 

### <a name="assigning-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당합니다.

이 섹션에서는 tooUserEcho 액세스 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![사용자 할당][200] 

**tooassign Britta Simon tooUserEcho hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **UserEcho**합니다.
...
Hello 액세스 패널에서에서 hello UserEcho 타일을 클릭할 때 자동으로 로그온 tooyour UserEcho 응용 프로그램을 구해야 합니다.
