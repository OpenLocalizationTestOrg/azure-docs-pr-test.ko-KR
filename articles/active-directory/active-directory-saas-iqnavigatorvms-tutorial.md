...
이 자습서에 설명 어떻게 toointegrate IQNavigator VM을 Azure Active Directory (Azure AD).

다음 이점을 hello로 제공 IQNavigator VM을 Azure AD와 통합:

- 액세스 tooIQNavigator VM을 지닌 Azure AD에서 제어할 수 있습니다.
- 프로그램 사용자 tooautomatically get 로그온 tooIQNavigator (Single Sign-on)는 VM의 Azure AD 계정으로 사용 하도록 설정할 수 있습니다.
...
다음 항목 hello가 필요 tooconfigure IQNavigator VM과 Azure AD 통합 합니다.
...
1. Hello 갤러리에서 IQNavigator VM 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-iqnavigator-vms-from-hello-gallery"></a>Hello 갤러리에서 IQNavigator VM 추가
tooconfigure hello 통합 IQNavigator VM의 Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 IQNavigator VM tooadd가 필요합니다.

**hello 갤러리에서 VM IQNavigator tooadd hello 다음 단계를 수행 합니다.**
...
Single sign on toowork에 대 한 Azure AD는 tooknow IQNavigator VM에서 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자 및 IQNavigator VM에서 hello 관련된 사용자 간 링크 관계를 설정 하는 toobe가 필요 합니다.

IQNavigator VM에서 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure 및 IQNavigator VM과 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.

1. **[Azure AD Single Sign-on 구성](#configuring-azure-ad-single-sign-on)**  -tooenable 사용자 toouse이이 기능입니다.
2. **[Azure AD 테스트 사용자 만들기](#creating-an-azure-ad-test-user)**  -tootest Azure AD single sign on Britta Simon 사용 합니다.
3. **[IQNavigator VM 테스트 사용자 만들기](#creating-a-iqnavigator-vms-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 IQNavigator VM에서 해당 하는 도구입니다.
4. **[Azure AD hello 테스트 사용자를 할당](#assigning-the-azure-ad-test-user)**  -tooenable Britta Simon toouse Azure AD에서 single sign-on입니다.
5. **[Single Sign-on 테스트](#testing-single-sign-on)**  -tooverify 구성 works를 hello 여부.

### <a name="configuring-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성

이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 IQNavigator VM 응용 프로그램에서 single sign on 구성 합니다.

**tooconfigure Azure AD single sign on IQNavigator VM과 hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **IQNavigator VM** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **IQNavigator VM 도메인 및 Url** 섹션를 hello 다음 단계를 수행 합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-iqnavigatorvms-tutorial/tutorial_iqnavigatorvms_url.png)

    a. Hello에 **식별자** textbox hello URL 입력:`iqn.com`

    b. Hello에 **회신 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<subdomain>.iqnavigator.com/security/login?client_name=https://sts.window.net/<instance name>`
...
4. 확인 **고급 URL 설정 표시**, hello 단계 다음에 수행:
    ...
    Hello에 **상태 릴레이** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`https://<subdomain>.iqnavigator.com`

    > [!NOTE] 
    > 이러한 값은 실제 값이 아닙니다. Hello 실제 회신 URL 및 릴레이 상태와 이러한 값을 업데이트 합니다. 연락처 [IQNavigator VM 클라이언트 지원 팀](https://www.beeline.com/iqn-product-support/) tooget 이러한 값입니다. 

5. **저장** 단추를 클릭합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-iqnavigatorvms-tutorial/tutorial_general_400.png)

6. toogenerate hello **메타 데이터** url hello 다음 단계를 수행 합니다.
    ...
    d. 이제의 toohello 속성 페이지를 이동 **IQNavigator VM** 및 복사 hello **응용 프로그램 Id** 를 사용 하 여 **복사** 단추를 메모장에 붙여 넣습니다.
...
7. IQNavigator 응용 프로그램에서는 hello 이름 식별자 클레임에 hello 고유한 사용자 식별자 값. 고객은 hello hello 이름 식별자 클레임에 대 한 올바른 값을 매핑할 수 있습니다. 이 경우 hello 사용자를 매핑하여 했습니다. Hello 데모 목적을 위해 UserPrincipalName 합니다. 하지만 tooyour 조직 관련 설정에 따라 매핑해야 hello 올바른 값에 대 한 합니다.   

    ![Single Sign-on 구성](./media/active-directory-saas-iqnavigatorvms-tutorial/tutorial_iqnavigatorvms_attribute.png)

8. Hello에 **IQNavigator VM 구성** 섹션에서 클릭 **IQNavigator VM 구성** tooopen **sign on 구성** 창. 복사 hello **Sign-Out URL, SAML 엔터티 ID, 및 SAML Single Sign-on 서비스 URL** hello에서 **빠른 참조 섹션.**

    ![Single Sign-on 구성](./media/active-directory-saas-iqnavigatorvms-tutorial/tutorial_iqnavigatorvms_configure.png) 

9. tooconfigure single sign on에서 **IQNavigator VM** 쪽 toosend hello 해야 **메타 데이터 URL**, **Sign-Out URL, SAML 엔터티 ID, 및 SAML Single Sign-on 서비스 URL** 너무 [IQNavigator VM 지원 팀](https://www.beeline.com/iqn-product-support/)합니다. 이 설정은 toohave hello 양쪽 모두에 제대로 설정 하는 SAML SSO 연결 설정 합니다.
...
hello이이 섹션의 목적은 toocreate Britta Simon IQNavigator VM에서 호출 하는 사용자입니다. 작업할 [IQNavigator VM 지원 팀](https://www.beeline.com/iqn-product-support/) hello IQNavigator VM 계정에서에서 tooadd hello 사용자입니다.

### <a name="assigning-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당합니다.

이 섹션에서는 액세스 tooIQNavigator VM을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![사용자 할당][200] 

**tooassign Britta Simon tooIQNavigator VM hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **IQNavigator VM**합니다.
...
Hello hello 액세스 패널에서에서 IQNavigator VM 타일을 클릭할 때 자동으로 로그온 tooyour IQNavigator VM 응용 프로그램을 구해야 합니다.
