...
이 자습서에 설명 어떻게 toointegrate (Azure AD) Azure Active Directory와 Zscaler ZSCloud 합니다.

Azure AD와 Zscaler ZSCloud 통합 hello 다음 이점을 제공 합니다.

- 액세스 tooZscaler ZSCloud 지닌 Azure AD에서 제어할 수 있습니다.
- 프로그램 사용자 tooautomatically get 로그온 tooZscaler ZSCloud (Single Sign-on)와 Azure AD 계정 사용 하도록 설정할 수 있습니다.
...
Zscaler ZSCloud와 Azure AD 통합 tooconfigure 다음 항목 hello가 필요 합니다.
...
1. Zscaler ZSCloud hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-zscaler-zscloud-from-hello-gallery"></a>Zscaler ZSCloud hello 갤러리 추가
tooconfigure hello와의 통합 Zscaler ZSCloud Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 Zscaler ZSCloud tooadd가 필요합니다.

**Zscaler ZSCloud hello 갤러리에서 tooadd hello 다음 단계를 수행 합니다.**
...
4. Hello 검색 상자에 입력 **Zscaler ZSCloud**합니다.

    ![Azure AD 테스트 사용자 만들기](./media/active-directory-saas-zscaler-zscloud-tutorial/tutorial_zscalerzscloud_search.png)

5. Hello 결과 패널에서 선택 **Zscaler ZSCloud**, 클릭 하 고 **추가** tooadd hello 응용 프로그램 단추입니다.
...
Single sign on toowork에 대 한 Azure AD는 tooknow Zscaler ZSCloud에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자와 Zscaler ZSCloud에 hello 관련된 사용자 간 링크 관계를 설정 하는 toobe가 필요 합니다.

Hello hello 값을 할당 하 여이 링크 관계가 설정 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** Zscaler ZSCloud에 합니다.

tooconfigure 및 Zscaler ZSCloud를 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.
...
3. **[Zscaler ZSCloud 테스트 사용자 만들기](#creating-a-zscaler-zscloud-test-user)**  -toohave Britta Simon 표현인 연결 된 toohello Azure AD 사용자는 Zscaler ZSCloud에 해당 하는 도구입니다.
4. **[Azure AD hello 테스트 사용자를 할당](#assigning-the-azure-ad-test-user)**  -tooenable Britta Simon toouse Azure AD에서 single sign-on입니다.
5. **[Single Sign-on 테스트](#testing-single-sign-on)**  -tooverify 구성 works를 hello 여부.

### <a name="configuring-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성

이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 Zscaler ZSCloud 응용 프로그램에서 single sign on 구성 합니다.

**Azure AD tooconfigure single sign on와 Zscaler ZSCloud hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **Zscaler ZSCloud** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **Zscaler ZSCloud 도메인 및 Url** 섹션를 hello 다음 단계를 수행 합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-zscaler-zscloud-tutorial/tutorial_zscalerzscloud_url.png)

     Hello에 **로그온 URL** 텍스트 상자에 사용자에 대 한 toosign tooyour ZScaler ZSCloud 응용 프로그램에서 사용 하는 hello URL 입력 합니다.
    
    > [!NOTE] 
    > 이 값이 hello로 tooupdate 되어 실제 로그온 URL입니다. 연락처 [Zscaler ZSCloud 클라이언트 지원 팀](https://support.zscaler.com/hc/articles/210172606-Zscaler-is-Expanding-Its-Zscloud-Global-Footprint) tooget이이 값입니다. 
...
6. Hello에 **Zscaler ZSCloud 구성** 섹션에서 클릭 **Zscaler ZSCloud 구성** tooopen **sign on 구성** 창. 복사 hello **SAML Single Sign-on 서비스 URL** hello에서 **빠른 참조 섹션.**

    ![Single Sign-on 구성](./media/active-directory-saas-zscaler-zscloud-tutorial/tutorial_zscalerzscloud_configure.png) 

7. 다른 웹 브라우저 창에서 tooyour에 ZScaler ZSCloud 회사 사이트는 관리자 권한으로 로그인 합니다.
...
11. Hello에 **SAML Single Sign-on 매개 변수 구성** 대화 상자 페이지에서 단계에 따라 hello를 수행 하 고 클릭 한 다음 **수행**
    ...
    a. 붙여넣기 hello **SAML Single Sign-on 서비스 URL** hello에 값 **hello SAML 포털 toowhich 사용자의 URL은 인증을 위해 전송** 텍스트 상자에 붙여넣습니다.
...
tooenable Azure AD 사용자가 toolog ZSCloud tooZScaler에 프로 비전 된 tooZScaler ZSCloud 이어야 합니다.  
Hello ZScaler ZSCloud의 경우에서 프로 비전은 수동 작업입니다.

### <a name="tooconfigure-user-provisioning-perform-hello-following-steps"></a>tooconfigure 사용자 프로비저닝, hello 다음 단계를 수행:

1. Tooyour 로그인 **Zscaler** 테 넌 트입니다.
...
5. Hello 사용자 추가 섹션에서에서 단계를 수행 하는 hello를 수행 합니다.
    ...
    a. 형식 hello **UserID**, **사용자 표시 이름**, **암호**, **암호 확인**를 선택한 후 **그룹**및 hello **부서** tooprovision 하려는 유효한 AAD 계정의 합니다.

    b. **Save**를 클릭합니다.

> [!NOTE]
> 다른 ZScaler ZSCloud 사용자 계정 만들기 도구를 사용할 수 있습니다 또는 AAD 사용자 계정을 tooprovision ZScaler ZSCloud에서 제공 된 Api입니다.

### <a name="assigning-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당합니다.

이 섹션에서는 액세스 tooZscaler ZSCloud 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![사용자 할당][200] 

**tooassign Britta Simon tooZscaler ZSCloud hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **Zscaler ZSCloud**합니다.
...
Hello Zscaler ZSCloud hello 액세스 패널에서에서 타일을 클릭할 때 자동으로 로그온 tooyour Zscaler ZSCloud 응용 프로그램을 구해야 합니다.
