...
이 자습서에 설명 toointegrate Azure Active Directory (Azure AD)와 Huddle 하는 방법입니다.

Azure AD와 Huddle 통합 이점을 다음 hello로 제공 합니다.

- 액세스 tooHuddle을 지닌 Azure AD에서 제어할 수 있습니다.
- 프로그램 사용자 tooautomatically get 로그온 tooHuddle (Single Sign-on)와 Azure AD 계정 사용 하도록 설정할 수 있습니다.
...
와 Huddle tooconfigure Azure AD 통합, 다음 항목 hello가 필요 합니다.
...
1. Huddle는 hello 갤러리 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-huddle-from-hello-gallery"></a>Huddle는 hello 갤러리 추가
tooconfigure hello와의 통합 Huddle Azure AD로 관리 되는 SaaS 앱의 hello 갤러리 tooyour 목록에서 Huddle tooadd가 필요합니다.

**Huddle hello 갤러리에서 tooadd hello 다음 단계를 수행 합니다.**
...
Single sign on toowork에 대 한 Azure AD는 tooknow Huddle에 어떤 hello 테이블에 해당 사용자가 Azure AD에서 tooa 사용자 필요 합니다. 즉, Azure AD 사용자와 Huddle 요구 toobe 설정에서 hello 관련된 사용자 간 링크 관계입니다.

Huddle에 hello hello 값을 할당 **사용자 이름** hello의 hello 값으로 Azure AD에서 **Username** tooestablish hello 링크 관계입니다.

tooconfigure와 Huddle 사용 하 여 Azure AD에서 single sign-on 테스트 구성 요소를 다음 toocomplete hello가 필요 합니다.

1. **[Azure AD Single Sign-on 구성](#configuring-azure-ad-single-sign-on)**  -tooenable 사용자 toouse이이 기능입니다.

2. **[Azure AD 테스트 사용자 만들기](#creating-an-azure-ad-test-user)**  -tootest Azure AD single sign on Britta Simon 사용 합니다.

3. **[Huddle 테스트 사용자 만들기](#creating-a-huddle-test-user)**  -toohave Britta Simon 사용자의 연결 된 Azure AD toohello 표현인 Huddle에 해당 하는 도구입니다.
...
이 섹션에서는 Azure AD에서 single sign-on hello Azure 포털에서에서 설정 및 Huddle 응용 프로그램에서 single sign on 구성 합니다.

**와 Huddle Azure AD에서 single sign-on tooconfigure hello 다음 단계를 수행 합니다.**

1. Hello hello에 Azure 포털에서에서 **Huddle** 응용 프로그램 통합 페이지에서 클릭 **Single sign on**합니다.
...
3. Hello에 **Huddle 도메인 및 Url** 섹션를 hello 다음 단계를 수행 합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-huddle-tutorial/tutorial_huddle_url.png)

    Hello에 **로그온 URL** 텍스트 상자에 패턴 hello를 사용 하 여 URL:`http://<company name>.huddle.com`

    > [!NOTE] 
    > 이 값은 실제 값이 아닙니다. Hello로이 값을 업데이트 합니다. 실제 로그온 URL입니다. 연락처 [클라이언트 Huddle 지원 팀](https://huddle.zendesk.com) tooget이이 값입니다. 
...
6. Hello에 **Huddle 구성** 섹션에서 클릭 **Huddle 구성** tooopen **sign on 구성** 창. 복사 hello **SAML 엔터티 ID, 및 SAML Single Sign-on 서비스 URL** hello에서 **빠른 참조 섹션.** 

    ![Single Sign-on 구성](./media/active-directory-saas-huddle-tutorial/tutorial_huddle_configure.png) 
    
7. 다운로드 toosend hello tooconfigure single sign on Huddle 쪽, 필요한 **인증서**, **SAML Single Sign-on 서비스 URL**, 및 **SAML 엔터티 ID** 너무[클라이언트 huddle 지원 팀](https://huddle.zendesk.com)합니다. 이 설정은 toohave hello 양쪽 모두에 제대로 설정 하는 SAML SSO 연결 설정 합니다.  
   
    >[!NOTE]
    > Single sign on toobe hello Huddle 지원 팀에서 사용 하도록 설정 해야 합니다. Hello 구성이 완료 되 면 알림을 받게 됩니다. 
    > 
...
tooenable Azure AD 사용자가 toolog tooHuddle에서 프로 비전 해야 Huddle에 있습니다. Hello Huddle의 경우에서 프로 비전은 수동 작업입니다.

**tooconfigure 사용자 프로비저닝, hello 다음 단계를 수행:**

1. Tooyour 로그인 **Huddle** 회사 사이트에서 관리자 권한으로 로그인 합니다.
2. **작업 영역**을 클릭합니다.
3. **피플 \> 피플 초대**를 클릭합니다.
   
   ![사람](./media/active-directory-saas-huddle-tutorial/IC787838.png "사람")

4. Hello에 **새 초대 만들기** 섹션를 hello 다음 단계를 수행 합니다.
   
   ![새 초대](./media/active-directory-saas-huddle-tutorial/IC787839.png "새 초대")
   
   a. Hello에 **선택 팀 tooinvite 사람 toojoin** 목록에서 **팀**합니다.

   b. 형식 hello **전자 메일 주소** 유효한 Azure AD의 계정에 포함 하려는 tooprovision 너무**Enter 전자 메일 주소를 위한 tooinvite 원할 것** 텍스트 상자에 붙여넣습니다.

   c. **초대**를 클릭합니다.   
   
    >[!NOTE]
    > hello Azure AD 계정 소유자 따라 활성화 tooconfirm hello 계정 연결을 포함 하 여 전자 메일을 받게 됩니다. 
    > 

>[!NOTE]
>다른 Huddle 사용자 계정 만들기 도구를 사용할 수 있습니다 또는 Azure AD 사용자 계정을 tooprovision Huddle에서 제공 된 Api입니다. 
> 

### <a name="assigning-hello-azure-ad-test-user"></a>Azure AD hello 테스트 사용자를 할당합니다.

이 섹션에서는 tooHuddle 액세스 권한을 부여 하 여 Azure에서 single sign-on Britta Simon toouse를 사용 합니다.

![사용자 할당][200] 

**tooassign Britta Simon tooHuddle hello 다음 단계를 수행 합니다.**

1. Hello Azure 포털에서에서 hello 응용 프로그램 보기를 열고 다음 toohello 디렉터리 보기를 탐색 및 너무 이동**엔터프라이즈 응용 프로그램** 클릭 **모든 응용 프로그램**합니다.

    ![사용자 할당][201] 

2. Hello 응용 프로그램 목록에서 선택 **Huddle**합니다.
...
Hello 액세스 패널에서에서 hello Huddle 타일을 클릭할 때를 받아야 하며 자동으로 Huddle 응용 프로그램의 로그인 페이지.
