...
작업을 수행 하는 가장 일반적으로 사용 된 id (즉, Azure AD B2C의 동작을 지시 하는 미리 정의 된 구성 파일 사용자 등록, 로그인, 암호 재설정) 및 해당 관계에 Azure AD B2C 미리 정의 하는 신뢰할 수 있는 당사자와 상호 작용 (예: Facebook id 공급자, LinkedIn, Microsoft 계정, Google 계정).  나중에 일반적으로 Azure Active Directory Premium, Active Directory/ADFS, Salesforce ID Provider와 같은 엔터프라이즈 영역에 있는 ID 공급자의 사용자 지정을 위해 기본 제공 정책을 제공할 수도 있습니다.
