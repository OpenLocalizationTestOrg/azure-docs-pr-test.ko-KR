...
Toohello 소프트웨어 버전을 해당 하는 업데이트 0.6 **10.0.10293.0**합니다.

> [!IMPORTANT]
> - 업데이트는 작업 중단 업데이트이며 장치를 다시 시작합니다. I/O가 진행 중인 경우 hello 장치 가동 중지 시간이 발생 합니다. Tooapply 업데이트 hello 하는 방법에 자세한 내용은 이동 너무[설치 업데이트 0.6](storsimple-virtual-array-install-update-06.md)합니다.
>
> - 중요 보안 수정이 포함되어 있으므로 즉시 업데이트 0.6을 설치하는 것이 좋습니다.


## <a name="whats-new-in-hello-update-06"></a>Hello 0.6 업데이트의에서 새로운 기능
업데이트 0.6은 중요 업데이트이므로 즉시 배포되어야 합니다. 이 업데이트에 따라 수정 하는 hello를 포함 되어 있습니다. 

- **Windows 보안 수정** - 이 릴리스에는 **Windows 중요 보안 수정**이 있습니다. Hello 보안 업데이트 hello 보안 문제에 대 한 자세한 내용은 다음을 검토 하 고 관련된 수정 프로그램 hello:
...
- **수정 프로그램을 복원** -이전 버전에서 hello 복원을 완료 하는 것을 하지 못하게 하는 버그가 발생 했습니다. 이 버그는 이 릴리스에서 수정되었습니다.


## <a name="issues-fixed-in-hello-update-06"></a>Hello 0.6 업데이트에서에서 해결 된 문제
...
| 2 |복원| 복원 하는 동안 경합 상태를 예방 하는 hello 복원 작업을 완료 했습니다. hello 버그 수정이 경합 상태를 해결합니다.|


## <a name="known-issues-in-hello-update-06"></a>Hello 업데이트 0.6의에서 알려진된 문제
...
| **17.** |업데이트 |오류가 발생 한 경우 코드: 2359302 (16 진수 0x240006) tooinstall 통해 핫픽스 하려고 할 때 로컬 UI hello 다음이 인해 해당 hello 핫픽스가 장치에 이미 설치 되어 있습니다.   | |
