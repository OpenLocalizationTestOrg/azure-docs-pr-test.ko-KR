...
- **향상 된 기능 tooSupport 패키지** -hello 향상 된 기능은 디스크, CPU, 메모리, 네트워크 및 클라우드 hello 지원 패키지의 장치 문제를 진단/디버깅 hello 프로세스 향상에 대 한 hello 통계 로그인을 포함 합니다.

- **제한 로컬로 고정 iSCSI 볼륨 too200 GB** -로컬 고정된 볼륨에 대 한 StorSimple 가상 배열에서 tooa 200GB iSCSI 볼륨을 제한 하는 권장 합니다. 계층화 된 볼륨에 대 한 로컬 예약 hello hello toobe 10% 볼륨 크기를 프로 비전 있지만 200GB 제한 계속 합니다. 

- **버그 수정 백업 관련** -이전 버전의 소프트웨어에서 백업 실패를 발생 시키는 문제 관련된 toobackups 않았습니다. 이러한 버스는 이 릴리스에서 해결되었습니다.


## <a name="issues-fixed-in-hello-update-04"></a>Hello 0.4 업데이트에서에서 해결 된 문제

hello 다음 표에서이 릴리스에서 수정 된 문제에 대 한 요약.

| 아니요. | 기능 | 문제 |
| --- | --- | --- |
| 1 |백업 성능|Hello 이전 릴리스의 경우 많은 수의 파일을 포함 하는 hello 백업을 오랜 시간이 toocomplete 순서에 따라 hello 일 수행 됩니다. 이 릴리스에서 hello 전체 및 증분 백업을 모두 hello 시간 toocompletion 크게 줄일을 참조 합니다. |
| 2 |지원 패키지|디스크, CPU, 메모리, 네트워크 및 클라우드 통계 이제 hello 지원 패키지를 모든 장치 문제를 해결 하는 데 매우 효율적으로 만드는 toohello 지원 로그에 기록 됩니다.|
| 3 |백업 |이전 버전에서 백업은 장기 실행 백업 오류를 일으키는 hello 장치에 공간 수치 연산에서 발생할 수 있습니다. 이 버그는 한 번에 5 개 이하의 백업을 tooqueue 허용 하 여이 릴리스의 처리 됩니다.|
| 4 |iSCSI | 이전 릴리스에서 hello 계층화 또는 로컬 고정 볼륨에 대 한 로컬 예약은 hello 프로 비전 된 볼륨 크기의 10%입니다. 이 릴리스에서 모든 iSCSI 볼륨 (로컬로 고정 된 또는 계층)에 대 한 로컬 예약 hello는 제한 too10% 최대 최대 200GB (계층화 된 볼륨이 2TB 보다 큰)에 대 한 함으로써 hello 로컬 디스크에 더 많은 공간을 해제 합니다. 이 릴리스에서 hello 로컬로 고정 볼륨 제한 too200 GB 되도록 하는 것이 좋습니다.|


## <a name="known-issues-in-hello-update-04"></a>Hello 0.4 업데이트의에서 알려진된 문제
