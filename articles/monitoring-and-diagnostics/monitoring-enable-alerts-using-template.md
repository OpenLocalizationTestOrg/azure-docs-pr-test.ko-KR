...
이 문서에서는 사용 하는 방법을 보여 줍니다.는 [Azure 리소스 관리자 템플릿](../azure-resource-manager/resource-group-authoring-templates.md) tooconfigure Azure 메트릭 경고 합니다. 이렇게 하면 tooautomatically 모든 리소스가 제대로 모니터링 tooensure 생성 될 때 리소스에 대 한 경고를 설정 합니다.

hello 기본 단계는 다음과 같습니다.

1. 템플릿을 toocreate 경고 hello 하는 방법을 설명 하는 JSON 파일을 만듭니다.
2. [배포 방법을 사용 하 여 hello 템플릿을 배포](../azure-resource-manager/resource-group-template-deploy.md)합니다.

아래 설명 방법을 toocreate 먼저 단독으로 경고에 대 한 리소스 관리자 템플릿 다른 리소스의 hello 생성 중 경고에 대 한 다음 합니다.

## <a name="resource-manager-template-for-a-metric-alert"></a>메트릭 경고를 위한 Resource Manager 템플릿 
형식의 리소스를 만들 리소스 관리자 템플릿을 사용 하 여 경고 toocreate `Microsoft.Insights/alertRules` 관련 된 모든 속성을 입력 합니다. 다음은 경고 규칙을 만드는 템플릿입니다.
...
경고 규칙에 대 한 hello 스키마 및 속성 설명은 [여기](https://msdn.microsoft.com/library/azure/dn933805.aspx)합니다.

## <a name="resource-manager-template-for-a-resource-with-an-alert"></a>경고가 있는 리소스에 대한 Resource Manager 템플릿
Resource Manager 템플릿에서의 경고는 리소스를 만드는 동안 경고를 만들 때 가장 유용합니다. 예를 들어, tooensure 수도 하는 "CPU % > 80" 가상 컴퓨터를 배포할 때마다 규칙을 설정 합니다. toodo이 VM 템플릿에 대 한 hello 리소스 배열에 리소스로 hello 경고 규칙을 추가 하 고 hello를 사용 하 여 종속성을 추가 `dependsOn` 속성 toohello VM 리소스 id입니다. Windows VM을 만들고 hello CPU 사용률이 80% 이상이 되는 경우 구독 관리자에 게 알려 주는 경고를 추가 하는 전체 예제는 다음과 같습니다.
...
* [진단 설정을 추가](monitoring-enable-diagnostic-logs-using-template.md) tooyour 리소스 관리자 템플릿
