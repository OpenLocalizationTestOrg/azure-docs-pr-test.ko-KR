...
---
# <a name="use-powershell-toosync-between-an-azure-sql-database-and-a-sql-server-on-premises-database"></a>Azure SQL 데이터베이스와 SQL Server 온-프레미스 데이터베이스 간의 toosync PowerShell을 사용 하 여

PowerShell 예제에는 Azure SQL 데이터베이스와 SQL Server 온-프레미스 데이터베이스 간의 데이터 동기화 toosync를 구성합니다. 
