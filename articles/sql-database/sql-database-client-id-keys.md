...
---
# <a name="get-hello-required-values-for-authenticating-an-application-tooaccess-sql-database-from-code"></a>코드에서 응용 프로그램 tooaccess SQL 데이터베이스를 인증 하기 위해 필요한 hello 값 가져오기
toocreate 및 Azure 리소스 생성 된 hello 구독에서 hello Azure AAD (Active Directory) 도메인에 앱을 등록 해야 코드에서 SQL 데이터베이스를 관리 합니다.

## <a name="create-a-service-principal-tooaccess-resources-from-an-application"></a>응용 프로그램에서 서비스 보안 주체 tooaccess 리소스 만들기
최신 toohave hello 필요한 [Azure PowerShell](https://msdn.microsoft.com/library/mt619274.aspx) 설치 하 고 실행 합니다. 자세한 내용은 참조 [어떻게 tooinstall Azure PowerShell을 구성 하 고](/powershell/azureps-cmdlets-docs)합니다.
...
* [데이터베이스를 사용 하 여 Azure Active Directory 인증 여 tooSQL 연결](sql-database-aad-authentication.md)
