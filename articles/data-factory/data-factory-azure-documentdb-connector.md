...
1. **질문:** 복사 작업은 기존 레코드의 업데이트를 지원합니까?
    ...
    **대답:** 아니요.
2. **질문:** 레코드 복사 이미 처리 하도록 Azure Cosmos DB에 복사본을 다시 시도 하면 어떻습니까?

    **대답:** 레코드에 "ID" 필드가 있고 복사 작업이 동일한 ID를 가진 레코드를 삽입하려고 시도하는 경우 복사 작업에서 오류가 발생합니다.  
3. **질문:** 는 Data Factory 지원 [범위 또는 해시를 기반으로 데이터 분할](../documentdb/documentdb-partition-data.md)?

    **대답:** 아니요.
4. **질문:** 테이블에 대 한 하나 이상의 Azure Cosmos DB 컬렉션 지정할 수 있습니까?
