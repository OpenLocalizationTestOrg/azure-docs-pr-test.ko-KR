...
---
# <a name="add-an-application-certificate-tooa-service-fabric-cluster"></a>응용 프로그램 인증서 tooa 서비스 패브릭 클러스터를 추가 합니다.

이 샘플 스크립트 hello 지정 된 Azure 키 자격 증명 모음에 자체 서명 된 인증서를 만들고 tooall 노드의 hello 서비스 패브릭 클러스터를 설치 합니다. 또한 hello 인증서 tooa 로컬 폴더를 다운로드합니다. hello 다운로드 한 인증서의 hello 이름 hello 주요 자격 증명 모음에 hello 인증서의 hello 이름과 달라 서 hello 됩니다. 필요에 따라 hello 매개 변수를 사용자 지정 합니다.
...
이 스크립트 명령 뒤 hello 사용: hello 테이블의 각 명령이 toocommand 특정 문서에 연결 합니다.

| 명령 | 참고 사항 |
|---|---|
| [Add-AzureRmServiceFabricApplicationCertificate](/powershell/module/azurerm.servicefabric/Add-AzureRmServiceFabricApplicationCertificate) | 새 응용 프로그램 인증서 toohello 가상 컴퓨터 크기 집합 hello 클러스터를 구성 하는 추가 합니다.  |
